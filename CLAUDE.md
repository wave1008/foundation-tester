...
- **シナリオは画面の解像度に依存しない形で書く**(ユーザー方針 2026-09-05)。**背の高い
  ...
  **確かめ方**: 変更したシナリオは**背の低い実機と背の高いシミュレータの両方**で回す
  ...
  `e2e.sh` は実機用を一度作ってある機械ならソースの鮮度で作り直すが、`e2e.sh` を通さずに SUT を変えたら
  `E2EAppIOS/scripts/build-ios-device.sh` を先に打つ(古いアプリのまま足した `#id` が not found で赤になる)。
...
- **対象未指定のピンチをどこへ当てるかは `FTCore.PinchRegion` の1箇所**(DSL・MCP・ライブ操作が
  ...
  **縮小だけ枠の長辺の両端から閉じる**ので、端に手前のものが載っていると1本を取られ、
  ...
  縦スクロールの recognizer が指を取る)/ **③端の規則は `closingTouchPoints` が唯一の定義元**で、
  ブリッジ(`BridgeRouter.coordinatePinch`)は同じ規則で少し内側に置く(片方だけ変えない)/
...
`run --fleet` の製品概念**に取っておく。見出しの文言(「デバイス」「実行ログ」)は中身を表すので据え置く
