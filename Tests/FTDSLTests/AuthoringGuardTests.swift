...
final class AuthoringGuardTests: XCTestCase {
    ...
    func testEveryAssertionCommandIsCounted() {
        ...
        scenario {
            scene(1, "全検証コマンドを1回ずつ") {
                FTDSL.expectation {
                    ...
                    select("#field").textIsNot("x")
                    select("#field").textContains("x")
                    select("#field").textContainsNot("x")
                    select("#field").textStartsWith("x")
                    select("#field").textStartsWithNot("x")
                    select("#field").textEndsWith("x")
                    select("#field").textEndsWithNot("x")
                    select("#field").textMatches("x")
                    select("#field").textMatchesNot("x")
                    select("#field").textMatchesDateFormat("yyyy/MM/dd")
                    select("#field").textIsEmpty()
                    select("#field").textIsNotEmpty()

                    select("#field").valueIs("x")
                    select("#field").valueIsNot("x")
                    select("#field").valueContains("x")
                    select("#field").valueContainsNot("x")
                    select("#field").valueStartsWith("x")
                    select("#field").valueStartsWithNot("x")
                    select("#field").valueEndsWith("x")
                    select("#field").valueEndsWithNot("x")
                    select("#field").valueMatches("x")
                    select("#field").valueMatchesNot("x")
                    select("#field").valueMatchesDateFormat("yyyy/MM/dd")
                    select("#field").valueIsEmpty()
        ...
        let assertionCategories: Set<String> = ["existence", "id", "text", "value"]
