// LedgerWriteRole の opt-in 配線をソース走査で固定する。
//
// **書くのは opt-in した production の入口だけ**(LedgerWriteRole.swift 冒頭)。この集合を
...
final class LedgerWriteRoleWiringTests: XCTestCase {
    ...
    func testEveryFMCallingEntryPointOptsIn() throws {
        ...
        for path in entryPoints {
            ...
            XCTAssertTrue(text.contains("LedgerWriteRole.enableForProduction()"),
                          "\(path) が起動時に LedgerWriteRole.enableForProduction() を呼んでいない" +
