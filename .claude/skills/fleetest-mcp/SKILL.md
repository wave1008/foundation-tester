...
- **冪等に**: 既に済んでいる状態(clone 済み・`.mcp.json` の `fleetest` が**同じ TOOL_ROOT** を指す)を
  検出したらスキップする。`fleetest` キーが**別のパス**を指していたら、スキップせず今回の TOOL_ROOT で
...
### 1. fleetest-mcp をビルド(疎通確認)

**TOOL_ROOT で** `swift build --product fleetest-mcp` を実行する(初回は数分。products 未宣言でも暗黙 product
として通る)。**exit code で成否を判定**(パイプで grep に繋がない —— tsc/swift の失敗を握りつぶす事故を避ける)。
これで `TOOL_ROOT/.build/debug/fleetest-mcp` が揃い、`.mcp.json` の rebuild-on-start が即座に返るようになる。
...
```
<ABS_TOOL_ROOT>/.build/debug/fleetest doctor --roots-only
```
...
- Claude Code が **fleetest MCP サーバの承認**を求めたら**許可する**。
- 承認後、`ft_status` などが応答すれば疎通完了。以後 `ft_tap`/`ft_screenshot`/`ft_snapshot`/`ft_run_scenario`
  等が使え、`/fleetest-scenario` も MCP 経由で動く。

疎通しないとき: サーバ起動失敗はたいてい PATH かビルド。TOOL_ROOT で `swift build --product fleetest-mcp` が
単体で通るか、`<ABS_TOOL_ROOT>` が絶対パスで正しいかを確認する。

### 4. この先

- テスト対象アプリ・デバイス・実行プロファイルの用意は `/fleetest-profiles`。
- シナリオ(.swift)の作成は `/fleetest-scenario`。
- VSCode 拡張・プロジェクト作成まで含むフル導入が必要になったら `/fleetest-setup`。
