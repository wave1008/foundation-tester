...
| スクロールしながら探す | `tap(sel, scroll: .down)` / `exist(sel, scroll: .down)`(別名 `tapWithScrollDown` / `existWithScrollDown`)/ ブロックで囲む `withScrollDown { … }` と、打ち消し: 1コマンドだけなら `scroll: .noScroll`(`exist(sel, scroll: .noScroll)`)・ブロックなら `withoutScroll { … }`。**画像系(`findImage` / `existImage`)は別名が無く `scroll:` だけ** |
