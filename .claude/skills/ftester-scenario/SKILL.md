...
- **マップ・キャンバス系の画面は `ft_pinch` / `ft_double_tap` / `ft_drag` で実際に試してから書く**。
  ただし**ここだけは「MCP で効かない = シナリオでも効かない」ではない**: `ft_*` は iOS では
  常に XCUITest 経路で動く一方、シナリオ実行は profile のエンジン(既定 hybrid)で動く。
  **iOS の Compose のダブルタップと iOS の Flutter のピンチは MCP からは効かないがシナリオでは通る**
  (理由と実測は docs/commands.md)。MCP で無反応でも、この2つに限っては書いてよい ——
  代わりに**実機実行(ステップ5)で必ず確かめる**。
