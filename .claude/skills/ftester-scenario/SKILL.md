...
- `祖先 >> 子孫` — **スコープ**。`#list >> .Cell[2]` = `#list` の子孫だけを候補にし、序数もその中で数える
  ...
  **使う前に実スナップショットで入れ子を確認する** — 効くかはフレームワークで割れる(実測):
  Compose / SwiftUI+UIKit / View-XML は入れ子になるが、**Flutter は葉しか残らないので `>>` は使えない**。
  同じ Compose でも Button/Cell は入れ子・Box は iOS で平坦化される
