...
- `.型:right(セレクタ)` — **方向**(`:left` / `:above` / `:below` も同型)。アンカーから見て
  その方向にある候補だけに絞る。**id が無い要素を隣のラベルから指す**ための記法
  (`.switch:right(通知)` = ラベル `通知` と同じ行にあるスイッチ)。規則は3つだけ:
  ①候補の中心がアンカーの帯(right/left なら y 範囲、above/below なら x 範囲)に入る
  ②候補の中心がアンカーの中心よりその方向 ③その中で最も近い(同距離はツリー順先頭)。
  **該当が無ければ失敗**する(近いものを勝手に選ばない)。可視要素にのみ有効
- `=ラベル` — `#` や `.` で始まる**生ラベル**を label として扱うエスケープ
  (`>>` や `:right(` を含むラベルもこれで書く)
- **綴り誤り・未対応記法は実行前にエラー**になる(`:near` `:parent` 等の他ツール記法、`[abc]`、
  閉じない括弧)。黙ってラベル扱いにはならないので、通れば構文は正しい
...
- **短いラベルは誤マッチする**。`"許可"` が `"通知を許可"` に、`"ディスプレイ"` が別項目の要約
  ...
  (`"ディスプレイとタップ"`)に寄せるか、`#id` / `.型=ラベル` で型を絞る。id は常に完全一致。
