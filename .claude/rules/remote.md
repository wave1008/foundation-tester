...
- **共有(複数ユーザー)の規律**(docs/remote-runner.md §18.7。M2 実装済み): **占有を知るために
  ...
  **①「不明」と「空き」を混ぜない**(子が落ちたら `observed:false`。**控えは消さない** ——
  ...
  不明の間は**配信を畳んだまま・保持者は名乗らない**(判定は拡張側の
  `vscode-fleetest/src/machineLockModel.ts` の `lock.held || !lock.observed` の1箇所。
  **Swift 側は `observed` を NDJSON へ載せるだけ**で畳む判断をしない)。
...
- **1マシンで同時に走る run は1本**(ユーザー決定 2026-09-21「高負荷になりすぎてテストが不安定に
  ...
  **④ローカルのロックの回収は pid(+ `acquiredAt` との開始時刻の照合 = pid の再利用を弾く。
  `ProcessLiveness.isAliveAndNotStartedAfter`)**(リモートの pgrep はローカル run を見つけられず、掛けると
