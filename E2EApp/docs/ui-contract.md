...
**レイアウトの制約(これを崩すと swipe 検証が落ちる)**: ブリッジの `swipe` は要素を狙わず**画面比率の
固定座標**で撃たれる — iOS(`InAppBridge.swipeVector`)は縦 0.15h↔0.85h・横 0.15w↔0.85w(y=0.5h)、
Android(`BridgeRouter.handleSwipe`)は縦 0.3h↔0.7h・横 0.2w↔0.8w(y=0.5h)。
