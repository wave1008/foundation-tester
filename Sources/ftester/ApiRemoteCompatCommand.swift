...
// `ftester api remote-compat --project <P> --profile <X>`: 拡張の実行前チェック専用
// (人が読む表は `ftester remote status`)。プロファイルのリモートホスト集合を求め、
...
struct ApiRemoteCompatCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Report git revision / toolchain compatibility of a run profile's remote hosts,"
            + " for the extension's pre-run check (the human-readable table is `ftester remote status`)")
    ...
    @Option(name: .customLong("remote-dir"),
            help: ArgumentHelp("Runner-only base directory override for every host"
                + " (default: each host registry entry, or ~/ftester-runner)"))
...
/// リモートホスト集合を求める純粋関数(I/O 抜き。`Tests/FTesterTests/ApiRemoteCompatTests.swift` 対象)
