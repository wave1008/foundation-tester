...
// **1台ぶんの画面配信を、その機械の上で始める**(`ftester api device-stream`)。
//
// 手元のデバイスなら拡張が配信ヘルパー(ftester-simstream / ftester-androidstream /
// ftester-devicepoll)を直接起こす。**リモートのデバイスにはそれができない** —— udid も
// adb serial も向こうの機械のものだから。そこで拡張は代わりに
// `ftester remote exec <host> -- api device-stream --device-host <host> --platform … --name …`
...
// 契約(v1 MJPEG / v2 H.264 のレコード形式)は vscode-ftester/src/deviceStream.ts が唯一の定義元。
//
...
struct ApiDeviceStreamCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: "Resolves the device on this machine and execs the matching stream helper, so"
            + " stdout is byte-for-byte what the helper writes (see vscode-ftester/src/deviceStream.ts"
    ...
    private func helperArgv(target: MonitorTarget, state: DeviceRuntimeState) throws -> [String] {
        ...
        if target.spec.isPhysical {
            ...
            let helper = try helperPath("ftester-devicepoll")
        ...
        if target.platform == "ios" {
            ...
            return [try helperPath("ftester-simstream"), "--udid", udid] + sizeArgs + codecArgs
        }
        guard let serial = state.androidSerial else {
            throw ValidationError("\(name) is not running on this machine")
        }
        return [try helperPath("ftester-androidstream"), "--serial", serial,
                "--adb", try AndroidDriver.findADB()] + sizeArgs + codecArgs
    }

    /// 配信ヘルパーは `ftester` と同じディレクトリに置かれる(拡張の config.ts の
