...
//
// 手元の `ftester api monitor` は simctl/adb を叩くので**この機械のデバイスしか観測できない**。
...
final class RemoteMonitorFanout: @unchecked Sendable {
    ...
    /// 起動直後の失敗が何秒未満なら「すぐ死んだ」とみなすか(ssh の接続確立 + ftester の起動で
    ...
    func start() {
        for host in hosts {
            ...
            thread.name = "ftester-monitor-fanout-\(host)"
            thread.start()
        }
    }
        ...
        while !isStopping() {
            ...
            guard quickFailures < Self.quickFailureLimit else {
                log("[monitor] Giving up on \(host): the remote monitor died \(quickFailures) times right"
                    + " after starting (is its ftester up to date? `ftester remote setup \(host)`)."
    ...
    private func runChild(host: String) {
        ...
        stderrThread.name = "ftester-monitor-fanout-err-\(host)"
        stderrThread.start()
