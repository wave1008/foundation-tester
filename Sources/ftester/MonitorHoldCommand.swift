// `ftester monitor pause/resume/status` — 拡張が尊重するモニター停止指示(計測条件づくり)。
...
struct MonitorCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
        Use before an unattended --performance run: `ftester monitor pause --for 30` stops the \
        ...
        `ftester monitor resume` releases the hold; an expired --for releases itself.
        """,
        subcommands: [Pause.self, Resume.self, Status.self])

    static func stateDir() throws -> URL {
        try RepoRoot.find().appendingPathComponent(".ftester")
    }
