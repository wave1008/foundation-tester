...
// (実害: remote setup の verify)。--device/--device-machine を既に持つ呼び出し
...
public enum RemoteDispatchDeviceScope: Equatable, Sendable {
    /// 全デバイスが machine 未指定 = プロファイル丸ごと(受け側が自分の台として解釈する)
    case wholeProfile
...
/// 手元の UDID をランナー機で探して `no simulator with that UDID` で落ちる(受け手報告:
