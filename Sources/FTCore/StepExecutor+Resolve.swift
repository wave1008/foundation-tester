...
extension StepExecutor {
    ...
    public static func candidates(_ locator: FlowLocator, elements: [ElementInfo],
        ...
        // **`#x` は identifier だけでなく placeholder も引く**(ユーザー指示)。
    ...
    /// テキスト比較の判定。**正規化を通す**: 素の `==` / `contains` だと、実データに紛れた
    /// ゼロ幅1文字で `textIs` が落ちる —— セレクタ側には正規化があるのにアサーション側には
    /// 無いと、**同じ画面について経路ごとに答えが違う**ことになる。
    ///
    ...
    /// 不一致で失敗するときに添える「**どちらの規則なら一致したか**」(ユーザー指示)。
    ///
    /// 読み手の次の一手がこれで決まる:
    ...
    /// —— 部分文字列一致だけ・装飾葉を除かず・文書順の先着3件という単純な規則では、
    ...
    ///
    /// Web の本文は `<span>` や強調で普通に割れる(受け手報告:
    ...
    public static func splitTextHint(for locator: FlowLocator,
        ...
        for size in 2...Self.splitTextMaxPieces {
            ...
            for start in 0...(sequence.count - size) {
                ...
                // 勧めると、利用者はそれを探して時間を捨てる(受け手データで判明)
