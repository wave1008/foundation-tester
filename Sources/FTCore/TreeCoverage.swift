...
public enum TreeCoverage {
    ...
    ///
    /// 根拠(固定コーパスの webView を持つ4画面): 取りこぼしのある Chrome の1枚が
    ...
    ///
    /// 実測(実機 Pixel 4a 1080x2340・同一ページの `wv_text`/`wv_link` 間の
    ...
    /// (debug ビルド): `underreports` 1回が 120 要素のブラウザ画面で 1.1ms、
    ...
    /// 最大の1本だけを返していた頃、実測(Yahoo!天気の週間画面を Android Chrome で)
    ...
    ///
    /// **witness(監査ラウンド)**: Chrome の `DNS_PROBE_FINISHED_NXDOMAIN`
    ...
    public static func emptyBands(inside container: ElementInfo,
        ...
        // 実際は本文で埋まっている帯を空白と誤報告していた(レビューで確定。
    ...
    /// ページ本体が1つも公開されていないと疑う空白の割合。根拠(固定コーパス):
    ...
    /// **この木は web ページを載せているか**。
    ///
    /// **木の中身だけで判定してはいけない**(2度外した)。実測で確かめた事実:
    ...
    /// アドレス欄ごと木から消すので、**悪い形ほど検知が外れる**。実測(実機
