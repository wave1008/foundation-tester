...
// 目的: 並列度を上げてもスループットは並列度5で飽和し、それ以上は modelmanagerd のモデル積み降ろし
// (unloadIfNeededToMakeRoom)とレイテンシが増えるだけなので、呼び出し側で待ち行列を明示して止める。
// run 中に見える約1回/秒は FM の天井ではなく**枠1 のロックの上限**(doctor --fm-load で枠を通さず実測)。
//
// **既定が 1 である理由**(ユーザー決定): 一部の機械は並列度 2 以上で FM が壊れる
// (M1 Ultra: 失敗率 26〜31% → 枠1 で 0%。docs/remote-runner.md §19)ため、既定は壊れない側に
// 置く。速さが要る機械は設定タブ / 登録簿の `fmConcurrency` で 5 に上げられる。
//
// **枠数 5 の根拠**(3機の掃引 `doctor --fm-load` + E2E 実測。詳細は docs/performance-tuning.md §3.5):
// 飽和する並列度は機械間で 3〜5 に揃う一方、絶対性能は 1.9 倍違う
// (local 7.62 / M1Ultra 5.50 / M1Max 4.11 回/秒)—— 変動するのは天井であって膝ではないので、
// **機械ごとに枠数を校正・適応する機構は置かない**(新しい世代で膝が動いたら掃引をやり直す)。
// E2E(8レーン・occlusion guard 有効)で「ゲート待ち + FM 実働」の総時間は
// 枠1 318.4s / 枠3 238.3s(待ち 56.9s 残る)/ 枠5 217.0s —— **待ちとレイテンシは
// トレードオフなので掃引だけで決めない**。枠を上げた代償(待ちの解消)は
// `fm.gateWait*` / `fm.skipped` で確認する(docs/results-json.md)。
//
...
// **旧版(単数の fm.lock)とは枠を共有しない** —— 版混在は直列化の効果を薄めるが、
// 混在自体を避けるのが前提(CLAUDE.md の版合わせ)。
//
...
public enum FMLock {
    ...
    /// `swift test` で `acquire(timeoutSeconds: 1)` が2回落ちた(単独では緑)。
