...
public enum SnapshotRenderer {
    ...
    ///
    /// `flagging` に入れた ref の行末には印を付ける(既定は空)。MCP が
    ...
    ///
    /// **群まるごとの all-or-nothing にしない**(D-2。実機で確定): 条件を外した
    ...
    /// **実データの罠**(Yahoo!天気 Safari 表示): 複数テキストノードを改行で
    ...
    /// 先頭切り(placeholder は対象外 — URL であることはまず無く判定コストだけ増える)
    ...
    /// endsWith セレクタを渡し7スクロール空振りした事故がある。
    ...
    /// 「木に居るのに waitFor が当たらない」= 照合のバグに見える(実アプリで実測。
