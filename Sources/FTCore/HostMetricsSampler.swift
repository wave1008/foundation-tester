...
public struct HostMetricsSample: Encodable {
    ...
    /// このサンプリング間隔中に完了した Vision / Core ML(FM 以外)の呼び出し数
    /// (この機械の全プロセス合計)。供給元は VisionUsageLedger。数えるのは3種: OCR の `recognize` 1回
    /// (`RegionText.resolve` の拡大はしごは最大 `RegionText.upscaleLadder.count` 回まで撃つので、
    /// 1回のガードで最大3件になりうる)/ 画像分類器の推論1回 / 学習1回。3欄とも null = 控えを読めなかった(不明)、
    ...
    public init(outputURL: URL, interval: TimeInterval = 1.0, logFailure: @escaping (String) -> Void) {
        ...
        let thread = Thread {
            ...
            while !stopFlag.isSet {
                ...
                let vision = VisionUsageLedger.drain(previous: &visionPrevious)
