...
//
// なぜ要るか(jma.go.jp を横スクロールした後の iOS Safari で実測):
...
public enum DuplicateRegion {
    ...
    ///
    /// 根拠(37枚の固定コーパス。下の y/x 制約を入れた後の実測):
    ...
    ///
    /// 根拠(witness `ios-browser_jma_hscroll` の複製10ペアを実測。
    ...
    /// 最長の複製区間。無ければ nil。
    ///
    /// **単純な「(type,label,value) の最長共通連続列」では誤検知する**(実装してすぐ撤回した):
    /// witness フィクスチャの最長一致(11)の正体は、**別々の2つの表が同じ8列の
    ...
    /// **O(n²) だが最適化しない**(実測してから決めた): debug ビルドで n=233 が
    ...
    /// **単に「相方が1つ居るか」だけの門では緩すぎる**: ラベル無し同型要素の
    /// 行(写真グリッド・アイコン列・ページドット)は全要素が互いの相方になるので毎回門を通り、
    /// **毎タップ** O(n²) の DP を払ってほぼ常に nil を得る。窓に「2種類以上のキー」を
