...
// E2E-CMP の `-06` が「画面は真っ黒・a11y ツリーは健全・タップだけ届かない」
...
// 回復を注入しない呼び出し(`recover` 省略)は「弾いて直し方を出す」だけ。
...
public enum BlankWorkerTriage {
    ...
    ///
    /// **窓は約10秒**(1.5秒から延ばした)。1.5秒では**アプリが最初のフレームを
    ...
    ///   - 「本物の wedge」= 入力しても戻らない
    /// 実測では、黒かった5台のうち本物は1台だけだった。この判別だけが全問正解した。
    /// `nudge` を渡さない呼び出しは一様が続けば凍結と判定する。
    ...
    /// ログ用。根拠が既定(一様 blank)だけのときは label だけを並べ、
