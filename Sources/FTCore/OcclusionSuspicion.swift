// occlusion-guard: FM/スクショを使わない Tier-0 の幾何ヒューリスティック。
...
//
// `OcclusionGeometry.occluder` とは別軸: こちらは面積重なり率(0.4)で FM 起動要否を決め、
// あちらは中心点の包含で「撃つと実際に何に当たるか」を撃つ前に名指しする。
...
public enum OcclusionSuspicion {
    ...
    /// ツリー順の近似 —— **近似のときだけ過検出寄り**なので、
    /// ステップを落とす根拠には使わないこと。
    ///
    /// 実測(固定コーパス): ツリー順のままだと Google マップの詳細画面で
    ...
    public static func covering(element: ElementInfo, in elements: [ElementInfo],
        ...
        // **いちばん手前を返す**(レビュー指摘)。配列順で最初に見つけたものを返すと、
