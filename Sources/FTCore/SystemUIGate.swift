...
//
// **`UIApplication.applicationState` は使えない**(実測。再提案しない)。
...
public enum SystemUIGate {
    ...
    /// 待ち切れなかったときの失敗の言い分。
    ///
    /// ここに来る時点で登録は必ずある(門が登録の有無で閉じる)。一致しなかったのだから、
    /// 次の一手に要るのは **「登録した名前」と「実際にそのアラートに在る名前」の両方**。
    /// ボタンは `GET /systemalert` が題名と同じ1往復で返しているので出せる。
