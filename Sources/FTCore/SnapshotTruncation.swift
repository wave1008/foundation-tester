...
//
// なぜ判定を共有するか: 打ち切りの注記は3箇所に別々に書かれていて、
...
public enum SnapshotTruncation {
    ...
    ///
    /// **`elements.count` をそのまま上限と比べてはいけない**: bulk 群(同一 id ×20 以上の非操作の
    /// 葉)は**予算の外で送られる**ので(`BridgeAPI` の版 61。安全弁 `bulkExemptCeiling` は 400)、
    /// `elements.count` は上限 + 最大 400 まで膨らむ。既定 120 で読んだ木に bulk が 280 件乗った
    /// だけで「もう天井だ」と誤判定し、**上げれば取れる要素に「上げても無駄」と言う**。
    ///
    /// **bulk 群の件数は申告(`bulkExemptCount`)を信用せず、最終配列から数え直す**
    /// (分類器は予算計算の唯一の定義元 `BridgeSnapshotThinning.bulkExemptCount` を再適用するだけで、
    /// 規則をここへ複製しない): 申告と配列がずれる生産者が2つある。①in-app の WebView-DOM マージ
    /// (`InAppBridge.mergeWebViewDOM`)はマージ後の集合で bulk 免除を**やり直す**のに、返す
    /// `bulkExempt` はマージ前(native 単独)の値のまま —— 申告が実際より小さくなり
    /// `budgetedCount` が**過大**になる。②Safari のホスト側 DOM 注入
    ...
    /// (**過少**方向)。最終配列そのものから数え直せば、どちらの生産者でも実配列と一致する
    ///
    ...
    /// 免除しない」という申告として読み、`elements.count` をそのまま使う
    ///
    /// `max(0,)` は残すが、**数え直した bulk 件数は必ず `elements.count` の部分集合**
    /// (`BridgeSnapshotThinning.bulkExemptCount` は同じ配列をフィルタするだけ)なので、
    /// この経路では数学的に到達しない(旧実装は「申告が正しい」という前提が崩れて②で実際に
    /// 到達していた)。到達しないので変異テストでは殺せない
    ///
    /// **表示も判定もこの `budgetedCount` と `bulkExemptPresentCount` の2つから組み立てる** ——
    /// 呼び手が切り詰めの文脈で `snapshot.elements.count` を直接印字すると、bulk が乗った木では
    /// 「truncated at 420 elements」のように、勧める上限(予算ぶんの120基準)より大きい数字を
    /// 出して読み手に矛盾として映る
