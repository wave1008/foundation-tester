...
//
// **効くのはプライマリ・フォールバックのどちらも解決できなかった失敗経路だけ**
// (StepExecutor+Actions.swift の解決分岐で、プライマリ・フォールバックの後に置く)。
// 成功経路の解決順(プライマリ → フォールバック)には一切触れないので、
...
// (「根拠のない定数を置かない」方針)。0件・複数件一致は不採用(ロケータ未解決の失敗)
