...
// 中継する(fleetest-simstreamのAndroid版)。既定はVideoToolboxでデコードしてJPEGをstdoutへ流す
...
// stdoutレコード形式はSources/fleetest-simstream/main.mと同一契約(パーサは共通:
// vscode-fleetest/src/deviceStream.ts)。変更する場合は3ファイルとも直すこと。
...
@autoreleasepool {
    ...
    if (serial.length == 0 || (![codec isEqualToString:@"mjpeg"] && ![codec isEqualToString:@"h264"])) {
        fprintf(stderr, "usage: fleetest-androidstream --serial <adb-serial> [--fps <n>] [--max-width <px>] [--adb <path>] [--codec h264|mjpeg]\n");
        return 2;
    }
    gFps = (fps > 0) ? fps : 12;
    ...
    // stdin EOF = 親がパイプを閉じた = 終了指示(常駐プロセス共通規約。fleetest-simstreamと同じ)。
