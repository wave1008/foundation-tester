...
public final class SystemUIDriver: AppDriver {
    ...
    ///
    /// **ここに因果を書かないこと**(失敗の実例): 一度は「hybrid へ寄せたら `notExist` が
    ...
    /// 「接続拒否」で落ちる(iPhone 13 で 18/34 赤。USB トンネルでは隠れる)。
    ...
    /// アラート操作・tapAppIcon が 401 になる(iPhone SE3 で E2E-iOS 15/35 赤。LAN では隠れる)
