...
- この1行が `.claude/skills/` に **`ftester-setup`(初回導入)と `ftester-update`(更新)** の両スキルを置く
  (この時点では clone しない=大きな取得の前にレビューできる)。版を固定するなら `FTESTER_REF=<tag>` を前置。
- `/ftester-setup`: foundation-tester を `git clone` → `swift build`(ftester CLI)→
  `npm run install-local`(VSCode 拡張)→ 自分のプロジェクト作成・マシン/アプリのプロファイル設定まで、
  検証ゲートと人間チェックポイント付きで実行する。
