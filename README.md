...
| **Swift DSL** | `Scenarios/*.swift` | テスト資産。どの入口で作っても同じ形式で保存・実行される |
...
# 3. FM エージェントにテストを作らせる → Scenarios/Generated/*.swift が生成される
...
| `explore <bundle-id> --goal "..."` | FM 探索による Swift シナリオ生成(`--max-steps` `--out`) |
| `run [--scenario <id>...]` | シナリオの決定的実行(`--heal` 自己修復、`--report-dir`、`--ports` 並列、`--skip-build`) |
...
の3層構造を持つ。`Scenarios/` に .swift を置いて `swift build` すれば自動発見される。
...
- レポートは成否問わず `reports/scenario-*.md` に出力(scene → CAE → ステップ階層、
  トリアージ、失敗スクリーンショット、**修正提案**)
- **自己修復とヒールキャッシュ**: `--heal` 時、壊れたセレクタは FM が修復して続行し、
  結果は `.ftester/heal-cache.json` に保存される。**2回目以降は FM なしで決定的に通過**し、
  レポートに「`Scenarios/LoginTest.swift:17` — セレクタ "#email_input" を "#email||.TextField[0]"
  に変更してください」のようなソース位置付き修正提案を出し続ける(ソースの自動書換はしない。
  人がソースを直すとキー不一致でキャッシュは自然に無効化される)
- **dry-run**: `swift run ftester-scenarios run --scenario <id> --dry-run` でデバイスに触れず
  ステップ列挙だけ行える(Shirates の No-Load-Run 相当。レビュー・生成コードの確認用)
...
- **シナリオ実行** — Scenarios/ の一覧(platform・実行状態バッジ)、
  実行+自己修復トグル、ライブ実行ログ、失敗時トリアージ表示。
  「再読込」でシナリオをビルドして一覧を更新(コンパイルエラーはそのまま表示)。
  **「全実行」は稼働中の全デバイスへシナリオを動的に分配して並列実行**
...
- **FM探索** — ゴールを書いて探索開始 → ExplorerAgent の進捗をライブ表示 →
  Swift シナリオが Scenarios/Generated/ に生成・ビルド検証され、一覧からそのまま実行できる
  (対象デバイスの platform がシナリオに記録される)
- **設定** — 下記
...
| `ft_list_scenarios` / `ft_run_scenario` | シナリオ一覧 / 決定的実行(`heal` オプション付き。自動ビルド込みで、コンパイルエラーはそのまま返る=エージェントが直せる) |
...
```
ftester CLI / GUI / MCP ──(サブプロセス)──▶ ftester-scenarios(Scenarios/ のシナリオを発見・実行)
...
```
Scenarios/         テストシナリオ(Swift DSL。コミットして資産化する)
...
Sources/
  ...
  FTScenarioRunner/ ftester-scenarios の CLI 実装(list / run・NDJSON イベント)
  FTCore/          ステップモデル / AppDriver / StepExecutor(FM 非依存・外部依存ゼロ)
...
- **explore が中断した** → 到達分のシナリオは `// TODO: 探索未完了` コメント付きで
  Scenarios/Generated/ に生成されるので、Swift を直接編集して仕上げられる(id セレクタ推奨)
- **シナリオのコンパイルエラーで実行できない** → `swift build --product ftester-scenarios` の
  エラーを修正する。explore の生成不良は Scenarios/_disabled/ に自動隔離される
