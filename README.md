...
| 入口 | 起動 | 向いている用途 |
|---|---|---|
| **CLI** `ftester` | `ftester ...`(mint 導入)/ `swift run ftester ...`(clone) | CI・回帰テストの定期実行(決定的・無料・exit code) |
...
## Claude Codeを使ってインストール

以下の指示をコピペしてClaude Codeに指示してください。

```
このプロジェクトに foundation-tester (https://github.com/wave1008/foundation-tester) を
消費者(Tier 2)パッケージとして導入してください。以下の手順で進めてください。

【パラメータ】
- プロジェクト名(--name): MyProject
- 対象アプリ bundle ID(--app): <com.example.ecapp / 不明なら後で設定でOK>
- 対象プラットフォーム: <iOS / Android / 両方>
- ftester バージョン: 最新版　または　ユーザー指定

【手順】
1. 前提ツール確認: mint / xcodegen / swift / Xcode / brew の有無をチェック
2. ftester 導入: `mint install wave1008/foundation-tester@0.1.0`
   （実行時は `export PATH="$HOME/.mint/bin:$PATH"` を通す）
3. scaffold:
   `ftester init --name <NAME> --app <BUNDLE_ID> \
     --ftester-url https://github.com/wave1008/foundation-tester --ftester-version 0.1.0`
4. `ftester doctor` で前提を確認し結果を報告
5. `swift build --product ftester-scenarios-<NAME>` でビルドが通るか確認
6. .gitignore を整備(.build/ .swiftpm/ Package.resolved reports/ results/ .DS_Store)
7. 生成物(Package.swift, Projects/<NAME>/) を確認し、残作業
   (実バンドルID設定, `ftester machine set`, デバイスマッピング)を箇条書きで報告

【方針】
- バンドルIDが不明な場合はプレースホルダで scaffold を進め、後から
  Projects/<NAME>/profiles/apps/*.json を編集する前提で報告する
- コミットは指示があるまで行わない
- インストールやビルドなど時間のかかる処理はバックグラウンド実行してよい
```

## 導入(使う: mint)

**自分のアプリのテストを書くだけ**なら、clone せず [mint](https://github.com/yonaskolb/Mint) で
`ftester` を導入し、自分の Swift パッケージから SPM 依存として使える。
手順の全体像・自分のアプリの登録・トラブルシュートは [docs/getting-started.md](docs/getting-started.md)。

```bash
brew install mint

# 最新版タグを取得(手動確認は https://github.com/wave1008/foundation-tester/tags)。
# CLI(mint)と依存(--ftester-version)に同じ版を使い回し、ブリッジと scenario runtime の版を揃える
VER=$(git ls-remote --tags --refs --sort=-v:refname \
  https://github.com/wave1008/foundation-tester | head -1 | sed 's#.*/##')

mint install wave1008/foundation-tester@"$VER"       # ftester CLI を導入
export PATH="$HOME/.mint/bin:$PATH"

mkdir my-app-tests && cd my-app-tests
ftester init --name MyApp --app com.mycompany.myapp \
  --ftester-url https://github.com/wave1008/foundation-tester --ftester-version "$VER"
swift build --product ftester-scenarios-MyApp
ftester run --project MyApp --profile ios            # シナリオは自分の repo に住む
```

VSCode 拡張の UI も `~/.mint/bin` を PATH に入れておけば `ftester` を自動発見する。
拡張は現状 marketplace 未公開のため、`vscode-ftester/` を `npm run install-local` で入れる
(下記/[vscode-ftester/README.md](vscode-ftester/README.md))。

以降の「セットアップ(クローン直後)」以下は、**ツール本体を clone してその中でシナリオを管理する、
または本体を改造する**場合の手順。リリース手順は [docs/releasing.md](docs/releasing.md)。
...
| `init` | 受け手の外部パッケージを scaffold(上記「導入(使う: mint)」参照) |
