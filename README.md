...
| `#list >> .Cell[2]` | **スコープ**(祖先 >> 子孫)。序数はスコープ内で数えるので画面クロムやスクロール位置でずれない(**Flutter は不可** — 下記) |
...
- **スコープ `>>` は UI フレームワークで効き方が割れる**(4 SUT 実測): Compose / SwiftUI+UIKit /
  View-XML は入れ子になるが、**Flutter はセマンティクスを畳んで葉になるため使えない**。
  使う前に実スナップショットで入れ子を確認する(`notExist`/`countIs`/`:near` は全フレームワーク共通で動く)
