...
`ftester` を導入し、自分の Swift パッケージから SPM 依存として使える(Tier 2)。
手順の全体像・自分のアプリの登録・トラブルシュートは [docs/getting-started.md](docs/getting-started.md)。

```bash
brew install mint
mint install wave1008/foundation-tester@0.1.0        # ftester CLI を導入
...
または本体を改造する**場合(Tier 1)の手順。リリース手順は [docs/releasing.md](docs/releasing.md)。
...
Projects/SampleApp/
├── profiles/
│   ├── apps/sampleapp.json        # アプリ: appName/autoInstall は common、bundle ID(app)と appPath は ios/android
