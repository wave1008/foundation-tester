...
- 注意: **コールドブート直後のシミュレータはアクセシビリティ IPC がタイムアウトしやすい**
  (kAXErrorIPCTimeout でランナーが落ちる)。`bridge up` 後に一度 `launch`+`snapshot` で
  ウォームアップしてから並列実行するのが安全
