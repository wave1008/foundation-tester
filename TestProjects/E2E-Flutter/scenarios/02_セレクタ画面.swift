...
// (旧: 02_セレクタ_id指定 / 03_セレクタ_ラベルと部分一致 / 04_セレクタ_型と序数 / 16 の S0010)。
// 旧シナリオ境界は tap("#tab_home") でホームへ戻ってから #nav_selector を叩き直す形に置き換えてある
// (AppShell はタブ切替で子画面の State を破棄するため、result="-" 等の初期値はこれだけで戻る。
// タブを跨がず #btn_back で戻る場合も _homeChild が破棄され同じ効果になるが、境界は #tab_home に揃える)。
// **各境界の直後に exist("#txt_home_marker", requireVisible: false) を挟む**: Flutter は起動/再着地の
// 数百 ms、a11y ツリーは完成しているのにポインタ入力を取りこぼすことがある(Android で実測)。
// これは可視性の検証ではなく着地確認の同期用(requireVisible: false)。
//
// **Flutter は型語彙が OS で非対称**(実測):
//   - ボタン(`Semantics(button: true)` を持つノード)は iOS/Android とも `Button`
//   - **テキストは iOS = `StaticText` / Android は葉+contentDesc の正規化で `StaticText`**
//     (Flutter は canvas 描画で Android 側の className が android.view.View のままになるため)
// → 型セレクタを使ってよいのは Button だけ。テキストの検証は必ず `#id` + `textIs` で書く。
//
// セレクタ画面の Button 順(E2EAppFlutter/docs/ui-contract.md。iOS/Android で同じ並び):
...
class セレクタ画面の機能一式が正しく動くこと {
    ...
    func S0010() {
        scenario {
            ...
            scene(16, ".button&&共通ラベル は #txt_shared_label ではなく #btn_shared_label に着地") {
                action {
                    tap(".button&&共通ラベル")
                }.expectation {
                    select("#txt_selector_result").textIs("result=shared")
