...
// (旧: 07_スクロール の S0010〜S0050・S0070 / 14 の S0010 / 16 の S0020・S0030)。
...
// (AppShell はタブ切替で子画面の State を破棄するため、selected="-" 等の初期値はこれだけで戻る)。
// **S0060/S0080/S0090/S0091/S0100 は重量級(scrollFrame 本丸・古いツリー検出・横縦同居・状態不変性)の
// ため統合しない**(各自 launchApp を残す独立 @Test のまま)。
// **14.S0010→16.S0020→16.S0030 は2 @Test に分割**(S0020/S0030)。1本に畳むと Android で 27.0s・
// iOS で 41.5s かかり、8 レーンあってもこの1本がスイート全体の壁時計を決めていた
...
class スクロールで折り返し下の要素に到達できること {
    ...
    @Test("scrollTo 到達・Cell ラベル・非スクロール契約・swipeElementToElement・notExist(scroll:)・固定ヘッダ")
    func S0010() {
        scenario {
            ...
            scene(8, "07.S0040: swipeElementToElement でリストがスクロールする") {
                condition {
                    ...
                    // notExist で行を追い出して確かめると ListView の cacheExtent に依存し、
    ...
    @Test("textContains / textMatches が動的な文字列を検証できる・Shirates 準拠のスクロールコマンドと tap(scroll:)")
    func S0020() {
        scenario {
            ...
            scene(4, "16.S0020: Shirates 準拠のスクロールコマンドと tap(scroll:)") {
                condition {
                    tap("#tab_home")
                    // 起動直後のポインタ取りこぼし対策(理由は scene 1)
                    exist("#txt_home_marker", requireVisible: false)
                    tap("#nav_scroll")
                }.action {
                    // 狙うのは一覧末尾(07 と同じ #row_40)。tap(scroll:) は
            ...
            scene(7, "スクロール探索の別名族(tapWithScrollDown / existWithScrollUp)と scroll: .noScroll") {
                action {
                    ...
                    withScrollDown {
                        ...
                        exist("#txt_row_selected", scroll: .noScroll)
                        tap("#btn_scroll_top", scroll: .noScroll)
                    }
                }.expectation {
                    exist("#row_01")
                }
