...
// CMP 版(TestProjects/E2E-CMP/scenarios/16_フィルタORと否定と対称アサーション.swift)の移植。
// **thisIs 系と doUntilTrue は移植しない**
// (デバイスに触れないホスト側機能で CMP 版とユニットテストが固定済み)。
// **@Test は3本に分ける**(セレクタ系 / スクロールコマンド / 探索を伴うスクロール)。1本に畳むと
// Android で 27.0s・iOS で 41.5s かかり、8 レーンあってもこの1本がスイート全体の壁時計を
// 決めていた(2026-08-01 実測。レーン追加では縮まない = docs/performance-tuning.md §3.6)。
        ...
        scenario {
            ...
            scene(7, "`exist(scroll:)` は画面外の要素を探索してから検証する") {
                expectation {
                    ...
                    // (07_スクロール.swift 参照)。画面を送ってしまうのでこの @Test の最後に置く
