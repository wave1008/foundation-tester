    ...
    func S0010() {
        scenario {
            scene(1, "セレクタ画面への URL でプロセスを起動する") {
                condition {
                    launchApp(url: "fte2eflutter://screen/selector")
                }.expectation {
            ...
            scene(2, "戻ってライフサイクル画面で受信 URL が記録されていることを確認") {
                action {
                    ...
                    select("#txt_last_deeplink").textIs("deeplink=fte2eflutter://screen/selector")
                }
            }
        }
    }
    ...
    func S0020() {
        scenario {
            ...
            scene(2, "クエリ付き URL を起動済みアプリへ送るとライフサイクル画面へ飛び、URL全体が記録される") {
                action {
                    openURL("fte2eflutter://screen/lifecycle?tag=a&n=1")
                }.expectation {
                    select("#txt_screen_title").textIs("ライフサイクル")
                    select("#txt_last_deeplink").textIs("deeplink=fte2eflutter://screen/lifecycle?tag=a&n=1")
                }
            }
        }
    }
    ...
    func S0030() {
        scenario {
            ...
            scene(2, "未知のルートを送っても画面は変わらず、受信したことだけ記録される") {
                action {
                    openURL("fte2eflutter://unknown/route")
                }.expectation {
                    select("#txt_screen_title").textIs("ライフサイクル")
                    select("#txt_last_deeplink").textIs("deeplink=fte2eflutter://unknown/route")
                }
            }
        }
    }
