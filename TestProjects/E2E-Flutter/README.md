...
| `03_テキスト入力.swift` | `type` と入力値 echo / `clearInput` / キーボード表示状態 / `pressEnter`・末尾改行の IME アクション(旧 05 の S0010〜S0030、18) |
| `04_ジェスチャ.swift` | `tap` 連打 / `press`(長押し)と通常タップの区別 / `swipe` 4方向 / `swipePointToPoint` / `pinchOut`/`pinchIn` / `doubleTap` / `swipeBy`(旧 06 の S0010〜S0020、22) |
| `05_スクロール.swift` | `scrollTo` と「`scroll:` を付けない探索・検証は現在画面のみ」の契約 / `swipeElementToElement` / `notExist(scroll:)` / 固定ヘッダの `scrollFrame` / `textContains`/`textMatches` / Shirates 準拠のスクロールコマンド(旧 07 の S0010〜S0050・S0070、14 の S0010、16 の S0020〜S0030。S0060/S0080/S0090/S0100 は重量級のため独立のまま) |
| `06_待機とタイムアウト.swift` | 暗黙待ち(既定タイムアウト再試行)と `timeout:` 引数 / `notExist`/`countIs` / 相対セレクタとスコープ / 状態フィルタ / `appIs`/`screenshot`/`waitForDisplay`/`verify`(旧 08 の S0010、12、21 の S0010。S0020 は独立のまま) |
| `07_条件分岐とダイアログ.swift` | `ifCanSelect` と `select`(掴めなければ空要素) / `back`(Android のみ) / `repeatWhileCanSelect` / `waitForClose`(旧 09 の S0010・S0020・S0040、14 の S0020、21 の S0060) |
| `08_ライフサイクルとプラットフォーム分岐.swift` | `restartApp`、`ios {}` / `android {}`、コントロールの状態遷移、`enabledIsFalse`/`enabledIsTrue`、`clearAppData`(旧 10 の S0020・S0030、11。S0010/S0040 は独立のまま) |
| `09_ID無し画面.swift` | 相対セレクタ(`基準:rightSwitch` / `基準:leftButton`)だけで id の無い画面を操作・検証 |
| `10_イレギュラーハンドラ.swift` | `irregularHandler`(出るか不定なアプリ内メッセージの検出・自動終了) |
| `11_WebView.swift` | WebView(Web コンテンツ)の中を操作・検証できること |
| `12_フリック.swift` | `flick`(画面基点の生ジェスチャ・8方向) |
