...
// 文字として挿入されている = 退行)。
// `#field_single` は RN の `TextInput`(iOS では `RCTUITextField` = UITextField のサブクラス、
// Android では実体が `EditText`)。2026-08-08 実測: iOS は in-app でも UITextField 経路
// (textFieldShouldReturn:)がそのまま発火し ime=+1(Flutter のような engine 私有 API は不要)。
// Android は a11y の ACTION_IME_ENTER で発火。いずれも本文に改行は入らない。
