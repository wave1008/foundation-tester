...
// 持ち越されないことの検証。
// SUT は React Native の E2EAppRN(iOS/Android 共通 JS バンドル)。プロセス起動ごとに
// React state が初期値へ戻るため「起動時は必ずホームタブのルート」契約が成立する
// (E2EAppCMP/docs/ui-contract.md)。platform 未指定 = ios/android 両プロファイルで回す。
        ...
        scenario {
            ...
            scene(4, "情報タブへ着地しアプリ ID が表示される") {
                action {
                    ...
                    select("#txt_about_app").textIs("app=com.ftester.e2e.rn")
                }
            }
        }
    }
