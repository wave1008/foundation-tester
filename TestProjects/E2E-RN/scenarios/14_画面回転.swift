...
// SUT は React Native の E2EAppRN(iOS は uikit 経路・Android は View 階層)。
// 回転の再レイアウトが RN のブリッジ越しでも a11y ツリーに反映されることの回帰。
// 回した向きはシナリオ終了時に自動で戻る。platform 未指定 = ios/android 両方で回す。
