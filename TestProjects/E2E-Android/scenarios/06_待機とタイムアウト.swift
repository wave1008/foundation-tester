// 06_待機とタイムアウト.swift
// fleetest 機能: 暗黙待ち(exist/textIs の既定タイムアウト再試行)と `waitSeconds:` 引数 / notExist・countIs・
...
// セレクタ解決は a11y ツリーの形に依存するので、記法が4フレームワーク共通で通ることも確かめる。
...
class 待機とタイムアウトが正しく効くこと {
    ...
    @Test("既定タイムアウト内の遅延表示・否定/個数/方向/スコープの各セレクタ・appIs/screenshot/waitForDisplay/verify")
    func S0010() {
        scenario {
