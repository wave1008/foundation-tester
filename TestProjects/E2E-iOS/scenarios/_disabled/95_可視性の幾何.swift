// 95_可視性の幾何.swift
// **陽性対照**(既定スイートには載せない): `requireVisible`(実行プロファイル `textVisualCheck: true`)の
// 幾何 Tier-0 =「木に居ても収まる軸の中心が画面外なら不可視」がデバイスで効くことを確かめる。
// 既定プロファイルは textVisualCheck が OFF なので、緑の run ではこの経路は1度も実行されない。
// 回し方: scenarios/ 直下へ一時的に出し、`fleetest run --project E2E-iOS --profile ios-fpc --scenario 可視性の幾何`
// (ios-fpc = textVisualCheck: true・1台)。**S0010 は落ちるのが正常**(失敗理由に
