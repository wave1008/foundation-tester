...
// 持ち越されないことの検証。
// SUT は iOS ネイティブ(SwiftUI+UIKit)の E2EAppIOS。プロセス起動ごとに @State が初期値へ戻るため
// 「起動時は必ずホームタブのルート」契約が成立する(E2EAppIOS/docs/ui-contract.md)。
        ...
        scenario {
            ...
            scene(4, "情報タブへ着地しアプリ ID が表示される") {
                action {
                    ...
                    select("#txt_about_app").textIs("app=com.ftester.e2e.ios")
                }
            }
        }
    }
