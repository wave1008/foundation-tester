...
class スクロールで折り返し下の要素に到達できること {
    ...
    func S0010() {
        scenario {
            ...
            scene(5, ".clickable&&行 03 でラベル指定タップできる(ラベルは Cell 側に集約してある)") {
                action {
                    tap(".clickable&&行 03")
                }.expectation {
            ...
            scene(8, "07.S0040: swipeElementToElement でリストがスクロールする") {
                condition {
                    ...
                    // 「スクロールした」の確認は #txt_scroll_top(先頭に見えている行)で行うため、
                    // 1回の送りで #row_01 を越えれば足りる(再利用窓から出ることは条件でない)。
                    swipeElementToElement("#row_05", "#row_01", durationSeconds: 0.5)
                }.expectation {
                    // notExist で行を追い出して確かめると UITableView の再利用窓に依存し、
                    // 慣性の小さい遅い台で #row_01 が木に残って誤って赤になる(docs/ui-contract.md
                    // §スクロール画面)。先頭に見えている行のラベルで確かめる。
    ...
    func S0090() {
        scenario {
            ...
            scene(4, "`scrollLeft` で横カルーセルを戻すと先頭タグに届く") {
                action {
                    // 送りの距離は往路と復路で対称ではない(12_フリック の同型 scene と同じ実測)ので、
