// 06_待機とタイムアウト.swift
// fleetest 機能: 暗黙待ち(exist/textIs の既定タイムアウト再試行)と `waitSeconds:` 引数の検証・
...
// **S0020(waitSeconds: の明示検証)は重量級(8秒待ち)のため統合しない**(独立 @Test のまま)。
...
class 待機とタイムアウトが正しく効くこと {
    ...
    func S0010() {
        scenario {
            ...
            scene(8, "3秒後表示を waitSeconds: 4.5(小数)で待てる") {
                action {
                    tap("#btn_delay_3")
                }.expectation {
                    exist("#txt_delayed", waitSeconds: 4.5)
                    select("#txt_delay_state", waitSeconds: 1.5).textIs("state=done", waitSeconds: 1.5)
                }
            }
