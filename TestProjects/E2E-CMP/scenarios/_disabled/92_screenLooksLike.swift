...
//
// ftester 機能: screenLooksLike(スクリーンショットと自然言語記述の FM 照合)。
...
class 画面全体をFMで検証できること {
    ...
    @Test("ホームと入力画面を screenLooksLike が見分ける")
    func S0010() {
        scenario {
            ...
            scene(2, "テキスト入力画面を screenLooksLike で検証(別画面と混同しないこと)") {
                action {
                    tap("#nav_input")
                }.expectation {
                    exist("#field_single")
                    screenLooksLike("テキスト入力用の入力欄が複数縦に並んでいる画面。"
