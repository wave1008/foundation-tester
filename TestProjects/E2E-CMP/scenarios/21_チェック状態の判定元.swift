...
// **この SUT に置く意味**: Compose は自前描画で、iOS の in-app ではタップ直後の絵が木より遅れる
// (ブリッジ v117 が撮る前に追いつきを待つ)。分類器優先の場面2がその witness。
// a11y 側は、Compose iOS の Checkbox / Radio がオンしか報告しない(オフは「報告なし」= checkIsOFF は通る)。
