...
**埋めるなら**: E2E-CMP の内蔵 HTML に「見出しを a11y へ出さない表」を足す。
コストは **5 SUT ぶんの webview HTML + `E2EAppCMP/docs/ui-contract.md` + 5 本のシナリオ**で、
5 アプリの再ビルドとフル E2E が要る(CLAUDE.md「片方だけ変えない」)。**ライブの web を叩く
タスクで代用しない**(盤面が毎日変わり、手数の差が注記の効果と混ざる。下の決定は維持)。

**やり方は 2026-08-13 に実機で確定した**(5 SUT を触る前に、127.0.0.1 の使い捨てページで検証):
...
- 動く雛形は `scratchpad/grid-prototype.html`(この検証に使ったもの)
...
## ここで測れないもの(2026-08-12 時点の穴 / 2026-08-16 に一部解消)

**a11y ツリーが実際に取りこぼす形**(格子の見出し行の欠落・アドレスバーのリダイレクト)は、
それを再現する offline の盤面がまだ無い。`gridWithoutHeaderNote` / `addressBarNote` はこのため
