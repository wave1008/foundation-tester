...
enum Tags {
    ...
    /// **occlusion-guard の FM 経路(反転)の witness**: テキストを
    ...
    /// **対象は必ずテキスト**(`OcclusionEligibility` は staticText 等のテキスト型だけを
    /// FM に回す。button はアイコンのラベルが説明文になりがちで約50%誤反転するため対象外)
    static let txtPaintTarget = "txt_paint_target"
    ...
    static let txtScrollTop = "txt_scroll_top"
