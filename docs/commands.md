...
| `irregularHandler(検出sel, dismiss: 閉じるsel?)` | **出るか不定のアプリ内メッセージ**(お知らせ・キャンペーン)を宣言すると、以降どのステップでも出た時点で自動的に閉じる。`dismiss` 省略時は検出したものをタップ。setUp で 1 回宣言するのが定石。閉じたことはステップの注記に残る(**1ステップで最大3回**まで閉じる。長いステップの最中に2度目が湧いても閉じ切れる。2回閉じても同じものが残っていれば「閉じられていない」と判断して打ち切り、注記に残す)。**OS のシステムダイアログ(権限の許可等)はこれでは閉じない** —— 下の §システムダイアログ(iOS)参照 |
