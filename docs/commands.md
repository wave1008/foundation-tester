...
- **OS のシステムダイアログ(権限の許可等)はこれでは止まらない** —— あちらは実行プロファイルの
  `iosSystemAlertButtons` による自動押下で別の機構(§システムダイアログ)。
...
### 自動で押す(実行プロファイル `iosSystemAlertButtons`)

```json
{ "iosSystemAlertButtons": ["Appの使用中は許可", "許可", "OK"] }
```

**並べた順に試し、先に見つかったものを押す**。押すのは「シナリオが要求した要素が、アプリ側でも
SpringBoard 側でも解決できなかった」ときだけ —— アラートを `tap("許可")` や `ifCanSelect` で
自分で操作しているシナリオからは奪わない。省略/空 = 何もしない(従来どおり)。
...
**エントリは2形**(2026-08-22)。素のラベルと、アラートの名指し:

```json
{ "iosSystemAlertButtons": [
    { "alert": "トラッキング", "button": "許可" },
    "Appの使用中は許可"
] }
```

- **素のラベル**(`"Appの使用中は許可"`)= どのアラートに出ても押してよい。**消費されない** ——
  `許可` のような汎用の文言は複数のアラートが共有するので、1度で使い切る扱いにすると
  「位置情報で使ったら、後から出る ATT に押すラベルが残らない」形になる(受け手報告
  2026-08-22)。同じアラートが本当に二度出るなら、人と同じように二度押す。
  **素のラベルがある限り、監視は run の間ずっと続く**(何が何回来るかを言っていないので、
  やめてよい根拠が無い)
- **名指し**(`{"alert": "<題名の一部>", "button": "<ラベル>"}`)= 題名がその文字列を含む
  アラートに**だけ**押す。**「このアラートが1回出る」という表明**なので、処理したら消費される。
  **名指しだけを並べた場合、全部処理し終えた時点でそのシナリオの監視は解除され、
  以後は判定の往復(約 73ms/ステップ)も払わない**。`alert` は部分一致(題名にはアプリ名が
  埋め込まれるため)・`button` は完全一致。題名が読めないアラートには当てない

順序は書いた順で、先に成立したものを押す。消費はシナリオ単位で、次のシナリオでは元に戻る。

**アラートが複数ある画面は、エントリを複数並べればよい**(位置情報の直後に ATT など)。
1枚閉じたら**戻らずに確かめ直す**ので、重なっていても順に閉じてから進む。
**押しても消えない画面で無限に回ることはない**(ステップの `timeout` が打ち切る)。
