...
されている(`tap(.id("login_btn"))` 等。意味・記録・ヒールは文字列版と同一)。**例外は `scrollFrame:` 引数で、こちらは
...
| `requireVisible: false` | 可視性確認を省く。**`exist` は見えていないと失敗へ反転し、`select` は空要素を返す**(意味が違う)。既定 true。確認が実際に走るのは実行プロファイルの `textVisualCheck`(**既定 true**。2026-09-03 にオプトインをやめた)が有効な run。確認は2段: **①幾何(FM 不要・決定的)** — 木に居ても**収まる軸の中心が画面外**なら不可視(iOS の木は画面外の要素も frame ごと残すので、通り過ぎた要素への `exist` がこれで止まる。`scroll:` 探索の「見つかった」判定と同じ述語)/ **②視覚照合** — 端末の OCR(Vision)が期待テキストを丸ごと読めれば通り、読めなければ FM が**描かれている文字を転写**し、期待テキストと突き合わせる(FM に期待テキストは渡さない。覆い・空白・別の文字はここで赤になる)。**先頭だけが描かれている**ときは、末尾に省略記号(`…`。日本語フォントの点の列も含む)があればアプリの意図した省略として緑 + 注記 `text-ellipsized`、無ければ描かれた割合が半分より多いと緑 + 注記 `text-partially-hidden`・半分以下で赤(`most of the text is hidden`)。FM が判定を返さなかったステップ(実呼び出しの失敗・ブレーカ開・macOS 26)は、OCR の段が読んだ文字(何も読めなければ領域のインク量)で同じ規則を当て、見えていないと判定したら注記 **`ocr-only-would-flip`** を残して**緑のまま通す**(新しい検知は警告から。赤にはまだしない)。FM に実際に訊いて答えが無かった回は結果 JSON の `notes` に **`visibility-guard-skipped`** も残る(「検証したつもりで検証していない緑」を run 横断で拾える。macOS 26 / `textVisualCheck:false` の静的に無効な構成では出ない)。**`launchApp` / `restartApp` の直後だけ猶予がある** —— 画面がまだ launch storyboard(全画素同一の未描画フレーム)なら、この待ち時間を**もう一度だけ**払って待ち直し `first-frame-pending` を残す(スプラッシュのあるアプリで起動直後の `exist` が赤くならないため)。それでも一様色のままなら赤にして `first-frame-timeout` を添える |
...
**門は「焦点が無い」ではなく「焦点が叩いた要素の外にある」**(2026-09-11)。Android は容器を
叩いても前の欄の焦点を外さないので、「全体で焦点が無いときだけ救う」形だと `type` が前の欄へ
入って緑になった(F13)。**MCP の `ft_tap(ref)` → ref なし `ft_type` も同じ救済を通る**
(判定は `InputFocusRescue` を共有。一意に決まらなければ撃った上で警告し、どこへ入ったかを
名指しする)。
...
`.noScroll`(`withScroll*` の中でも**この1コマンドだけ送らない**)を渡します。省略すると、ブロックの文脈に従います
(文脈も無ければ現在画面だけ)。型は `FTScrollOption` で、`scrollTo(direction:)` や `withScroll*` が取る
`FTScrollDirection` とは別の型です(向きを取る側に `.noScroll` は書けません)。
**関数名でスクロールを指定する別名(`tapWithScrollDown` / `existWithScrollUp` / `selectWithScrollLeft` /
`findImageWithScrollDown` / `tapWithoutScroll` など)は1つも置いていません。** Shirates の名前を書いても、
コンパイルエラーが正しい書き方を指します(`tapWithScrollDown(sel)` → `Write tap(selector, scroll: .down) instead`)。
...
**端と確定するのは、画面が最後に動いてから 1.0 秒経った後**(`StepExecutor.edgeClaimGraceAfterMove`)。
一度で端へ寄せる経路(上の2行)は本当の端に着いたときに最大 1.0 秒払う(実スワイプは1本で1秒近くかかるので
ふつう払わない)。端へ飛ぶと続きを**窓の外に非同期で描き足す**リスト(RN の `FlatList`: 描き足しまで実測
0.4〜0.7 秒)で、描き足す前の「余地が無い」を端と読んで途中で止まらないため。in-app ブリッジは端送りで
`contentOffset` を動かしたら `atEdge: false` を返す(v116。飛ぶたびにセルが同じ座標に並ぶと、木の署名では
動いたと読めない)。**Compose / Flutter の AX の scroll は返さない**(Compose は端でも受理する)。
...
スクロール内容の下端が**タブバーや固定フッタの下に潜る**画面があります(実アプリで頻出。
背の高い端末では余白に収まり、小さい端末でだけ露見します)。この形をそのまま撃つと
タップは帯のほうに当たり、失敗は数ステップ後の無関係な場所に出ます。

`tap` は**撃つ前に容器を1回だけ送って外そうとします**。外せたらそのまま対象を撃ち、
ステップの注記に `scrolled the container to bring the target out from under <要素>` が残ります。
**外せないと分かる形では送りません**(従来どおり警告を付けて撃ちます):

- 覆いが操作可能でない(暗幕・装飾)
- 覆いが容器の半分以上を占める(全画面のモーダル)
- 覆いが容器の中心線を跨ぐ(中央のダイアログ。どちらへ送っても外れない)

**拒否はしません** —— 覆われた要素をわざと叩いて「反応しないこと」を確かめる書き方を
壊さないためです。払うのは注記が出る画面だけで、ドラッグ1回と木1枚です。
...
- **覆われた背面は木から消える** —— 覆いの下にある要素の `exist` は落ち、タップは
  「見つからない」で失敗します。**覆われているのに緑になる**(誤った緑)を残さないためです
...
> 逆にホーム画面は日本語 3/3・英語 0/3)、さらに ②**同じ画面の別の撮り方で裏返る**
> —— 目には同じ 1206x2622 の入力画面2枚で、同じ説明文が **5/5 通る画像と 0/5 落ちる画像**があった
> (同一画像に対しては決定的)。**待ちを足しても変わらない**。
...
- **`fleetest run --dry-run`(MCP は `ft_dry_run`)ならデバイス無しで判定できます**。
  デバイスを触る前にここで落とすのが安上がりです。**dry-run は `ifCanSelect` の成立側と
  `.ifElse` 側の両方を列挙する**(デバイス実行では片側しか走らないが、列挙なら両側のセレクタ
  構文誤り・台帳に無い `#id` をここで返せる)ので、両側とも検証が無ければ警告します
...
`fleetest run --app-id <bundleID>` で渡すか、`--profile` を使う。
`installApp()` の `appPath` も同じくアプリプロファイルの `<platform>.appPath` から解決される。

| コマンド | 説明 |
|---|---|
| `launchApp(bundleID?, url:?)` | 起動(省略時は**この run の既定アプリ** = 実行プロファイル → アプリプロファイルの `<platform>.app`。`@TestClass(app:)` が書かれていればそちらが勝つ)。**起動済みでも前面化ではなく、常にプロセスを終了してから起動し直す**(Android はブリッジが force-stop+起動、iOS は terminate 込み launch。エントリー画面から始まる)。`url:` を渡すと起動直後にその URL を配送する(配送の詳細・制約は `openURL` を参照)。**配送は最初の画面が描かれてから**(利用者が触れる要素が木に載るまで既定の待ち時間だけ待つ。React Native は JS が listener を登録するまで warm な URL を捨てるので、ブリッジが答えた直後に撃つと届かない。載らないまま配送したときは注記 `launch-url-before-interactive-ui`) |
...
| `@TestClass(platform:)` / `@Test(platform:)` | **対象 OS の宣言**(`"ios"` / `"android"`)。両方あるとメソッド側が勝つ。宣言した OS を回さない実行プロファイルでは**そのシナリオを実行せず skipped(対象外)として記録する**(失敗ではなく、exit code も汚さない)。「iOS では意味がないテスト」(Wi-Fi プロキシ設定など)を、`ios { }` を空にして緑にする代わりに意図として残すためのもの。**実行プロファイルを使う run でだけ効く** —— `--port` / `--serial` の直指定は「この run が回す OS の集合」を宣言しないので従来どおり |
...
| `irregularHandler(検出sel, dismiss: 閉じるsel?, maxDismissals: 10)` | **出るか不定のアプリ内メッセージ**(お知らせ・キャンペーン)を宣言すると、以降どのステップでも出た時点で自動的に閉じる。`dismiss` 省略時は検出したものをタップ。**セレクタは `\|\|` / `(a\|b)` の代替も効く**(`"#promo\|\|お知らせ"`。照合は先に書いたほうが勝つ)。setUp で 1 回宣言するのが定石。閉じたことはステップの注記に残る(**1ステップで最大10回**まで閉じる。`maxDismissals:` で宣言ごとに変えられる。長いステップの最中に2度目が湧いても閉じ切れる。2回閉じても同じものが残っていれば「閉じられていない」と判断して打ち切り、注記に残す)。**OS のシステムダイアログ(権限の許可等)はこれでは閉じない** —— 下の §システムダイアログ(iOS)参照 |

### 独自コマンド(`@FTCommand`)

DSL コマンドで足りない繰り返し手順(ログイン・共通の前処理・自作ジェスチャの組み合わせ等)は、
scenarios/ 配下に**素の Swift 関数**として書く(`func setUp()`/`tearDown()` と同じ場所でよい)。
これは Shirates の `macro` に相当するが、独自の機構ではなく**ただの Swift 関数**(コード生成は無い)。
...
`@FTCommand("説明")` を付けると `fleetest api dsl-commands` / MCP の `ft_dsl_commands` の索引に
`[project: ファイル:行]` 付きで載る(**付けなくても呼び出せるが、索引に出ないので AI エージェントが
「存在しない」と判断して使わない**)。守る規律:

- 付けられるのは **トップレベルの関数**、または **`extension FTElement` のメソッド**だけ
  (`extension FTElement { @FTCommand("…") func dismissToast() { … } }` は索引上
  `select(selector).dismissToast()` の形で呼べることが分かる)
- 引数は空でない文字列リテラル1つ(`@FTCommand("summary")`)。既存の DSL コマンドと同じ名前を
  付けると索引が「組み込みと紛らわしい」警告を出す(改名すること)
- `private`/`fileprivate` を付けた関数は他ファイル(シナリオ)から呼べないため索引には載らず、
  代わりに警告になる
- 索引の一覧であって**コード生成ではない**(展開は何も生成しない、ただのマーカー)。
  `ft_batch` では実行できない(バッチは DSL の1行を直接実行する仕組みで、シナリオファイル内の
  関数までは踏み込まない)
...
| **アドホックな MCP 実行**(エージェントが調べる・動かす) | セレクタを優先するが、**座標のほうが早く解けるならそれでよい**。実行時間の短縮を優先する |
...
`*x*` = 部分。題名にはアプリ名が埋め込まれる(「“アプリ名”が…」)ので、
