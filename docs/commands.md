...
**宣言は毎回そのまま使う**(2026-08-22)。押したラベルを消費したりはしない ——
`許可` のような**汎用の文言は複数のアラートが共有する**ので、消費すると
「位置情報で1度使ったら、後から出る ATT に押すラベルが残らない」形になる
(受け手報告 2026-08-22)。同じアラートが本当に二度出るなら、人と同じように二度押す。

**アラートが複数ある画面は、複数のラベルを並べればよい**(位置情報の直後に ATT など)。
