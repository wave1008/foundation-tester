...
同じアラートは1つのシナリオで二度出ないものとして扱うので、宣言を全部使い切った時点から
**そのシナリオは以後1往復も払わない**。消費はシナリオ単位で、次のシナリオでは元に戻る。

**アラートが複数ある画面は、複数のラベルを並べればよい**(位置情報の直後に通知など)。
...
**消費済みのラベルは二度と押さない。** 同じアラートが再び出た場合は押さずに止まる
(その前提を置かない設計なら、消費せず毎回押す形になり監視も解除できない)。
