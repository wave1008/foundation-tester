...
```json
{ "common": { "appName": "MyApp", "app": "com.mycompany.myapp" },
  "ios":    { "appPath": "~/builds/MyApp.app", "autoInstall": true },
...
- `Projects/MyApp/Scenarios/` に `.swift` を置く（`_Main.swift` は編集不要のエントリポイント）。
- または拡張の **FM 探索**（オンデバイス・無料でシナリオ自動生成）や、CLI の
  `swift run ftester explore ...` で生成する。
