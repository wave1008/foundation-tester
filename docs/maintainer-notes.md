...
### 50.6 座標ジェスチャを細かく割ると XCTest が時間を守らない(ブリッジ iOS v128)

ライブ操作の軌跡(pointermove ごと = 16〜20ms 間隔の点)を Apple マップで再生すると地図が動かなかった。
原因はランナーの `CoordinatePinch` がキーフレーム間を **1/60 秒刻みに割って**送っていたこと ——
**1つの move が短いと XCTest(`XCSynthesizedEventRecord`)は時間を守らない**。実測(シミュレータ・
3 秒の直線): 間隔 8ms → 0.45 秒 / 16.7ms → 4.4 秒 / 20ms → 5.4 秒 / 33ms 以上 → 3.4〜3.7 秒。
形(直線/ジグザグ)には依らず、間隔だけで決まる。細かい軌跡は実時間の約 1.7 倍に伸び、指が遅く動く
= マップはパンではなく**長押し(ピンを立てる)**として受け取った。粗い軌跡は逆に縮んだ(11 秒 → 約 6 秒)。

- **XCTest は1つの move の中を自分で補間する**(4 秒の1区間でもドラッグとして届く)ので、細かく割る必要は無い。
  直し: `resample` = ①50ms(`minimumSegment`)未満の間隔の点を間引く(先頭・末尾・静止区間の端は残す)
  ②0.1 秒(`maximumSegment`)を超える移動だけ割る(ピンチの「数点に割る」の趣旨)。
  直した後は間隔 8〜100ms・点 17〜551 の全部で要求 +0.4〜1 秒(シミュレータ)/ +0.9〜2 秒(実機 SE3・USB)に揃った
- **前回(§50 本体)の「時間どおり」の確認は穴だった** —— 使った witness の軌跡は区切り点4つで大半が静止区間。
  静止は割らないので守られていた。**細かく動く区間の時間を測る対照が無かった**
- **マップで「動かない」を見たら先に長押しを疑う** —— ゆっくり動かす合成軌跡(30pt/秒程度・始点で速度 0 の
  余弦波)は、直した後でもマップがピンを立ててメニューを開き、**以降の測定がそのメニューに当たって全部無効**になる。
...
ライブ操作の画面内の門は画面の大きさを**直近の観測の控え(`LiveScreenMemo`)で先に見る** —— パネルのクリックは
座標の tap なので、毎クリック木を読むと負荷下で命令ごとに数秒足す。控えで外れたときだけ撮り直して確かめる
(回転で広がった画面を古い控えで断らない)
...
時間切れ(拒否ではない)のどれかなら止めない。**準備完了の実績(`.fleetest/bridge-<port>.ready`)は中身に
ランナーの pid を持ち、今の `.pid` と一致するときだけ数える** —— ポートの台帳は同じ番号で建て直されるので、
在否だけだと前世代の ready が「起動しきれない新しいランナー」を永久に守ってしまう(レビューで発見)
...
## 52. CLAUDE.md の領域固有の規律を `.claude/rules/` へ移した(2026-09-26)

**なぜ**: CLAUDE.md が 160 KB(1,436 行)まで育ち、全セッションと全サブエージェントが起動時に
毎回まるごと読んでいた。実測で起動時の読み込みは約 85k トークン(うち CLAUDE.md が約 67k)。

**仕組み**: Claude Code のパス限定の規則(`.claude/rules/*.md` の frontmatter `paths:`)。
**2026-09-26 に 2.1.212 で実地確認した読み込み条件**: 対象のファイルを **Read ツールで**読んだときだけ
読み込まれる。何も読まない・`cat`・grep・別のファイルの Read・**読まずに新しいファイルを書く**では
読み込まれない。**サブエージェントが対象を Read すると届く**。**書き誤った `paths:` は黙って無視される**
(= 移すときは各パターンが実在のファイルに当たるかを `git ls-files ':(glob)<pat>'` で確かめる。
移設時は 598 パターン全部が当たることを確認した)。

**移し方**: 箇条書きを**1文字も変えずに**移し、元の182個がちょうど1回ずつ CLAUDE.md か規則ファイルに
残ることを機械で確かめた。CLAUDE.md に残したのは、どの作業でも効く規則と「どこであれ新しいものを
足すとき」の規律(ファイルが決まらない = パス限定では読み込まれない)と、規則ファイルの索引。

**精度の計測**(保守作業に近い問い10問。規律を移した領域8問 + 残した領域の対照2問。自動メモリは切った):

| 条件 | 移す前 | 移した後(索引に固有名を足した最終版) |
|---|---|---|
| ファイルを読める(各2回) | 移した領域 92.7% / 対照 100% | 100% / 100% |
| ファイルを読めない(4問) | 10/12 | 11/12 |
| 起動時の読み込み | 85,377 トークン | 43,085 トークン |

索引に固有名を足す前の版は、ファイルを読めない条件で 8/12 に落ちた(`ParentBoundCommand`・
`installStepSync` が抜けた)。**読まずに答えると抜ける固有名は索引の一行に置く**のが効いた。
...
- **読み替えは置かず、記録を書き換えた**(1回きりのスクリプト。キーは引用符つきの完全一致だけを置換・冪等)。手元と
  リモート3機で流して旧キー 0 を確かめた後、スクリプトは消した(`63ec86c4` に残っている)。**リモートは align と同時に
  流す** —— 先に書き換えると、旧いバイナリが逆に読めなくなる
...
## 55. ダッシュボード「デバイスの健全性」で踏んだもの(2026-09-27)

### 55.1 重い計測をモニターの周期の中で待っていた

iOS Simulator のストレージ(データディレクトリの `du`)を、モニターの配信周期の中で並列に await していた。
`du` は 1 台 27〜35 秒(実測・データ 30〜37GB)かかり、起動直後は 8 台が同時に期限を迎えるので、その間タイルの
状態・凍結の判定が止まる(しかも同期の子プロセスを Swift の協調スレッドに載せていた)。**周期は控えを読むだけにし、
計測は専用のキューで 1 台ずつ・この Mac で run が動いている間は撃たない**(`DeviceStorageSampler`。
`taskpolicy -b` で I/O も後回し)。`getattrlistbulk` でも所要は変わらなかった(カーネル側のファイルごとのコスト)。
ついでに分かったこと: データの 7 割は PosterBoard のスナップショットキャッシュだった(§12.4.2)

### 55.2 run 前の点検の結果を、回復前の一覧の label で引き直していた

`WorkerAnomalyRecord.preRunTriage` が、点検の結果(label の配列)を**点検前の一覧**で引いて台の鍵に直していた。
iOS の `BlankWorkerTriage` は回復でポートが変わるので、結果の `repaired` は**回復後の label**で、点検前の一覧には
