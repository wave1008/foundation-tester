...
### Compose の探索直後タップ(未解決・2026-08-03)

**症状**: CMP(Compose Multiplatform)で、**スクロール探索の直後のタップが数行ずれる**。
`tap("#btn_scroll_top")` の直後に探索してタップすると ios-xcuitest では決定的に、
`withScrollDown(scrollFrame:) { tap("#row_30") }` の形では android で間欠的に(3回中1〜2回)、
狙った `#row_30` ではなく `#row_40` / `#row_35` が選ばれる。
**E2E-iOS / E2E-Android / E2E-Flutter では同じ形が安定して通る = Compose 固有**。

**分かっていること**:
- 「先頭へ」のスクロール自体は**550ms 以内に完了**している(ブリッジ直叩きで実測。
  tap の応答 349ms + 次の snapshot で既に先頭)。整定の取りこぼしとは別の経路
- ホスト側の「探索1周目は静止を待つ」修正(2026-08-02)を入れた後も再現する
- 整定署名に**可視要素の id を混ぜても直らない**(仮想リストの行入れ替えを署名が
  見落としている説は否定された)
- 画面下部に横カルーセルを足した後から決定的になった(それ以前は通っていた)

**再現**: `Projects/E2E/Scenarios/07_スクロール.swift` に他 SUT の S0080 と同じ形
(`scrollTo("#row_40")` → `tap("#btn_scroll_top")` → `withScrollDown { tap("#row_30") }`)を
書いて `--project E2E --profile ios-xcuitest`(決定的)/ `--profile android`(間欠)で実行する。
**CMP にだけ「探索直後にタップする」シーンを置いていない**のはこのため
(他 3 SUT では回帰テストとして機能している)。

**次に見るべき所**: 探索が終端で行う整定(`StepExecutor.settleAfterScroll`)と、その署名
(型 + 座標)。Compose の仮想リストは行を使い回すため、**減速中でも「同じ y に同じ型」が並んで
署名が変わらない**可能性がある。ただし可視要素の id を署名へ混ぜる実験では iOS 側は直らなかった
(2026-08-03)ので、別の経路も疑うこと。
...
- **`git stash` での A/B は版を下げるので Android が拒否する**。HEAD へ戻すと
  `expectedBridgeVersionCode` も下がり、Android は versionCode のダウングレードを拒否するため
  **全機で「cannot connect (no response to status)」= 40/40 失敗**になる(実測)。
  A/B の前に必ず全機で `adb uninstall` すること
...
- **まず両者の入力が本当に同じかを実機で測る**(DOM と XCUITest の矩形を並べる。
