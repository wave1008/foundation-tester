...
### Compose の探索直後タップ(機構確定・回避策あり・2026-08-03)
...
**機構(2026-08-03 に確定)**: 領域を指定しない探索は**全画面スワイプ = 1回の移動量がリストの
ビューポートを超える**。Compose iOS は**画面外の要素の frame をビューポート内へクランプして報告する**
([[compose-ios-ax-frame-clamp]])ので、飛び越した行が「見えている」ように見え、探索がそこで止まって
**実際には別の行が居る座標をタップする**。だから狙った行の何行も先(#row_40 / #row_35)が選ばれる。

**回避策(iOS で実証済み)**: **`scrollFrame` で領域を指定する**。刻みが容器の 0.5 に収まって
飛び越しが無くなり、CMP/ios-xcuitest は 3/3 で安定した(未指定は 0/3)。

**Android は領域を指定しても間欠的に残る**(2/3 で失敗)。あちらに frame クランプは無いので
別要因 —— スワイプ後の慣性が続いている間に解決している疑いが濃い。Android の a11y ノードは
キャッシュ供給で、**読む前に `refresh()` しないと古い座標を返す**既知の性質があり
(design.md §Android のテキスト注入の規律)、スナップショット側は同じ手当てをしていない。
次に試すならそこ(コストは 1 snapshot あたり約 +65ms なので、探索の整定に限って使う)。

**自動では直せない理由**: 未指定時に容器を自動で選ぶには `ElementInfo.scrollable` の申告が要るが、
**Compose の容器は xcuitest では `other` として出て申告できない**(実測: `#list_rows` も
`#carousel_tags` も scrollable なし)。

**「自己発見」も試して撤回した(2026-08-03・再提案しない)**: 前後 snapshot で 4pt 以上動いた
要素が2つ以上あれば、その**外接矩形**を容器とみなす案。iOS は 0/3 → 3/3 に直ったが、
**リスクが2つ現実化**して収束しなかった:
...
`settleAfterScroll` に `snapshot(bypassingCache:)` を入れる案も**効かなかった**(Android の
間欠ずれは a11y キャッシュの鮮度では説明できない)。

**正式な対処は「Compose では `scrollFrame` を書く」**(実証済み 3/3)。commands.md に明記してある。
