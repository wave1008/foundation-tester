...
2026-09-02 と 09-14 に回して E2E-CMP / E2E-iOS / E2E-Flutter は全緑、**E2E-RN だけが2本**
(`hello123`→`hllo123` / `persist99`→`prsist99`)**打鍵の中抜け**で落ち、記録は全プロファイル成功が
条件なので digest が更新されなかった。

**2026-09-14 に `TypeReadback.plan` へ `.retype` を足して直した(ブリッジ v104)**: 実際値が期待値の
順序を保った部分列で、落ちた文字に可視文字を含むとき、消してから全文を打ち直す(ランナーは
delete × 実際値の文字数 + 期待値 / ホストは clearInput + type)。守る規律3つ:
- **前方一致の後にしか置けない**(`hel` は `hello123` の部分列でもある。先に見ると今自己修復できている
  「末尾の欠落 → 追送」を全文打ち直しに変える)
- **打ち直しは `TypeReadback.maxRetypes`(1)回まで** —— 打ち直しても同じ形で欠けるなら、それは
  打鍵の落ちではなくアプリ側の加工(英字を捨てる数字欄)。2回目は v104 より前と同じく受理する。
  上限が無いと「消す→打つ→同じ値」が停滞に達して緑だった step が 422 になる
- **ホスト側の `readbackTarget` は前方一致の説明を部分列の説明より先に採る** —— ヒントが value に
...
仕組みが無く、赤を抱えたまま「検証済み」を名乗る装置になる(e2e.sh のコメントにある通り)。

## 再起動直後の最初の xcuitest 供給は冷えたブートがランナーの予算に混ざる(2026-09-14)

Mac の再起動直後(シミュレータ 0 台)に `--ios-xcuitest` を回すと、8 台のランナーが**何も書かないまま**
`startupTimeoutSeconds`(180 秒)を跨いで全滅する(台帳 §19.25)。xcodebuild は設定を書き出した後、
テスト本体が始まるまでログに何も書かず、しかも Shutdown のシミュレータを**自分でブートしてから**始めるので、
8 台同時のブート(3 分超)がそのままランナーの予算に載る。ツール側は 3 つで受ける:
①ランナーを起動する前に `SimulatorBoot.ensureBooted`(`simctl bootstatus -b`)でブートを待ち切る(予算の外)
②冷えたシミュレータを起こす起動は in-app の注入と同じく**同時 2 台**(`BridgeProvisioner.launchWidth`)
③起動待ちはログの伸び・**ランナーアプリのプロセスがシミュレータに現れた**・`Test Suite started` の 3 つを進みとして
...
## モニターと E2E(**止めるのは性能を測るときだけ**。2026-08-21 に運用変更)
...
`an unreachable bridge` ならこの壊れ方の署名(アサーション失敗は別の話)。
...
**2026-09-20 の 3 時間負荷テストでも同じ向きの差が出た(壊れ方は「凍結」)。** 手元の Mac で
フリート(シミュレータ 8 + エミュレータ 8)+ 実機 4 台 + MCP 2 セッションを同時に回し、
途中 30 分だけ `monitor pause` して 3 区間を取った:

| 区間 | 配信 | 時間 | 凍結で離脱したワーカー | 頻度 |
|---|---|---|---|---|
| 周回 1〜4 | あり | 56 分 | 12 | 0.21/分 |
...
**配信を止めると load average も 34 → 14 に落ちる**ので、配信は「負荷の大きな一部」であって
唯一の原因ではない(配信なしの区間でも 1 件出た)。
...
**キーウィンドウにしない別 UIWindow** に載るモーダルを出す(SDK のアプリ内メッセージ相当)。
2形あるのが要点 —— **全画面モーダル**(背面は隠れる)と**上部バナー**(背面は隠れない)。
片方だけだと「手前の窓だけ見せる」実装でも緑になり、**判定を分けられない**。

回帰は `TestProjects/E2E-iOS/scenarios/15_別ウィンドウのモーダル.swift` の4本
(木に載る / `irregularHandler` が閉じる / バナーは背面を隠さない /
**条件判定が覆いを閉じてから答える**)。4本目は 2026-08-20 に追加 ——
`ifCanSelect` は `perform` を通らないので、操作・検証側を直しても自動では守られない。
**この形の失敗は赤くならない**(分岐が黙って飛ぶ)ので、witness 側で不成立を赤くしてある。
**覆いに関する変更を入れたらこの3本を回す**(修正前は3本とも落ち、修正後は3本とも緑、が対照)。

**操作側の確認は木では取れない**: 合成タッチの宛先が正しい窓かは、**座標タップ**で確かめる
(`POST /tap {"x":…,"y":…}` でモーダルが閉じるか)。木に載っているのに閉じられない、が
2026-08-20 に実際に残った形。**スクリーンショットも見る** —— モーダルが写らない画像を
証跡として残していた。

## 「待ってから取り直す」を模すフェイクは**動く木**にする(2026-08-20)

割り込みを閉じた直後の取り直しを「整定を待つ」に変えた回、変異(整定を外して素の
`driver.snapshot()` に戻す)が**生き残った**。原因はテストのフェイクで、閉じるアニメーションを
**毎回同じ木**で模していた —— 整定は「同じ署名が2回」で成立するので、**静止した木では
待っても待たなくても同じ結果**になり、判定になっていなかった。

直し方は2つとも要る:
- **フェイクの木を動かす**(消えていくカードの frame をスナップショットごとにずらす)
- **長さを窓に入れる** —— 操作側のリトライ予算(3回)より**長く**、整定のポーリング上限
  (6回)より**短く**。短いと「どちらでも通る」、長いと「どちらでも落ちる」で、
  やはり判定にならない

一般化: **「待つ/待たない」の差は、待っている間に状態が変わるフェイクでしか出ない**。
整定・ポーリング・リトライを触るときは、フェイクが**時間とともに変わるか**を先に見る。
関連: 変異が生き残ったときの切り分け(フィルタ・先に落ちたテスト・変異式・テスト内容)。
...
- `curl -s http://127.0.0.1:<port>/status` の `protocolVersion` が期待どおりか
...
## ブリッジの「誤った赤」を疑う手順(成功しているのに失敗と報告される)
...
1. **まずホスト側から状態を撮る**。失敗した直後に `/snapshot` を撃ち、期待値になっていれば
   誤った赤が確定する(「入っていない」のか「読めていない」のかはここで分かれる)
...
**失敗レポートだけで誤った赤を確定できることがある**(2026-08-01・Android ネイティブ画面)。
...
### 待つ間の読み直しは、Android の Compose ではキャッシュを迂回する(2026-09-18)

「肯定形は待つ用途だから助かっている」には続きがあった —— **助かってはいるが 1 周期遅れる**。
E2E の Android で、1 秒後に表示される `#txt_delayed` の検出が CMP だけ一律に約 1 秒遅かった
(遅延ボタンを押してから: View/XML・Flutter 1.8 秒 / CMP 2.9 秒。差はすべて `waitMs` = ポーリングの待ち)。

- **ブリッジを HTTP で直接叩いて切り分けた**: `exist` と同じ間隔(`PollBackoff` の 100/200/400/800/1000ms)で
  読むと、素取得は押してから 1.79 秒の読みでまだ無く 2.80 秒で見つかる。`refresh=1` は 1.08 秒(4 回とも同じ)。
- **罠: 50ms 刻みで読み続けると遅れは出ない**(素取得でも 1.10〜1.15 秒で見つかる)。最初の計測はこれで
  「ブリッジは遅れていない」と誤って結論しかけた。**鮮度の計測は、実際の読み手と同じ間隔で読む**
- 迂回の費用は snapshot 1 回あたり約 +45ms(要素 19 個の Compose 画面で 8 → 50〜60ms)

**規則**(`StepExecutor.repollBypassesCache`): Android かつ自前描画(`isSelfRendered`)のアプリでは、
**待つ間の 2 回目以降の読み**をキャッシュ迂回にする。1 回目の読み(見つかれば終わるふだんの経路)では払わない。
効かせる場所は2つで、どちらも既定値を置かない —— アサーションの待ちは `AssertFreshRetry(bypassOnRepoll:)`、
操作の待ち(要素の出現を待つ再試行)は `SnapshotFreshness.repoll`。不明なフレームワークは迂回しない。

**配線の罠**: 最初の実装は単体テストが全部緑なのに E2E で**まったく効かなかった** —— シナリオ実行バイナリが
Android では `uiFramework` を決めていなかった(空打ちに無関係だから判定を省いていた)。単体テストは
`uiFramework: .compose` を直接渡すので通る。配線は `RepollCacheBypassTests` の走査で固定した。
**効果**(手元のエミュレータ 6 台・1 秒遅延の出現待ちの中央値): CMP 2876 → 1074ms、Flutter 1828 → 1063ms、
View/XML(一部 Compose)1823 → 1175ms。対象外の RN は 936 → 942ms で不変、他のステップの中央値も不変。
...
(未着手)。**計器のコメントは 2026-08-25 に直してある** —— 撤去済みの `FrozenEvidence.noPresent`
を指し、「wedge なら tick が止まる(未検証)」という**反証済みの前提**を残していた5箇所を、
「ホストは判定に使わない・採り直さないこと」へ書き換えた(コメントだけなので版は据え置き、
指紋の期待値だけ貼り替えている)。`BridgeDTO.swift` の**版履歴 64 は当時の記録として残す**。
...
E2E も、その経路が要る失敗(古い a11y を読む誤った赤)が出るまで沈黙した。
...
## 自前 SUT に witness が無い形は、受け手の環境で取れたら台帳に書く(2026-08-19)

**OS で bundle ID が違うアプリ**は自前 SUT に1つも無い(5 SUT すべて ios/android 同一 ID)。
そのため「対象アプリを実行プロファイルから解決する」(`FTCore.ScenarioAppResolution`)を入れた
...
**規律**: 自前 SUT で代表できない形は、**SUT を増やす前に受け手の環境で取れないかを見る**
(ここでは SUT の bundle ID を片方だけ変えるという手はあったが、ビルド構成に触る割に
得られるのは1形だけだった)。取れたら**どの環境で・いつ・何が緑になったか**をここに書く ——
書かないと、次に同じ箇所を触る人が「witness が無い」と読んで作り直しにかかる。
**受け手の報告は再現手順ごと控える**(こちらでは再実行できないので、報告が唯一の記録になる)。
...
**生き残りの最頻の型は「その節を1度も通らない入力しか書いていない」**(2026-08-15 に4件連続で観測)。
テストは呼ばれてもいるし緑でもあるので、フィルタや置き場所を疑っても何も出てこない。見るべきは
**変異した節に到達する前に、別のガードで落ちていないか**:

- 複数の条件を **and** で束ねた判定は、**片方だけが効く入力**を別に用意する。
  合成木の容器を画面いっぱいにしていたため「容器比」と「画面比」が同値になり、
  画面比を 0 にする変異を1つも殺せなかった(容器が画面の一部を占める木を足して解決)
- 絞り込みが**多段**なら、狙った段より前で落ちていないか確かめる。葉の判定は
  「scrollable でない」に加えて「容器より低い」も見ており、**容器と同じ高さの矩形を置いていた**
  ので前段で落ちて、後段を外す変異が素通しした
- **周期的・一様な入力は複数のガードに同時に当たる**。一様な並び(A,A,A…)は別のガードが
  先に落とすので、**そのガードだけが止めている形**(A,B,A,B… の周期)を別に用意する
...
**run 横断で推移(良くなったか)を出すときは分母も揃える** —— run の本数・シナリオの集合・
標本数・デバイス構成の4つ。レシピは docs/results-json.md §フレークの推移を見る。
...
- 2026-08-23 追加: E2E-RN/android `フリックが正しく動くこと.S0010` がフル E2E(8 並列)で 1 件
  (`scrollLeft ×3` 後の `#tag_01` 不在 = 同じ形)。直前 10 run は 10/10 緑、直後に同プロファイル
  26 本を再実行して 26/26 緑。母数: フル 1 回 + 再実行 1 回で 1 件
...
- **打ち切り**(`unmovedRoundsToStopSearch = 2`): 木が2周続けて1文字も変わらなければ、
  ...
  理由文には**実際の回数**を出す(上限を名乗ると「8回も振ったのに」と読めてしまう)。
  **文言は2形に分ける**(`contentEverMoved`。2026-08-15 の外部評価): 動いた末の停止は
  `the scroll area reached its end`、1度も動かなかった回は `nothing moved at all during the search`。
  どちらも `raising maxSwipes will not help` まで言い切る —— 旧文言 `stopped early` は
  「途中で諦めた」としか読めず、**実際は iOS の設定アプリでリストの末尾に着いていた**回を
  欠陥と受け取らせ、maxSwipes を上げた無駄な再試行を誘っていた
...
- **ディレクトリを消しても `.build` が戻る**のは、生きているプロセスが作り直しているから。復活させる
  ...
  **VSCode 終了で止まらないのは XCUITest ランナーだけ**(親から切り離してあり PPID=1)。
  順序と掃除の1行(`pgrep -fl` / `pkill -f`)は docs/user-docs/getting-started_ja.md「アンインストール」
...
- **occlusion の「FM に訊いて反転する」経路は、自前 SUT では踏めない**(2026-09-03 に実機で確認)。
  ...
  ① `E2EAppIOS` の別ウィンドウ全画面モーダル配下の背面要素 —— 覆いは
  `Color.black.opacity(0.55)` の**減光**で、規則どおり「判読できる減光は visible=true」。
  しかも減光越しに文字が見えるので Tier-1 のインク量も高く、**FM の手前で素通り**する。
...
- **Android 側の画面凍結(blank-screen)判定は実機では動かない**。閾値(30KB)が 1080x2424
  エミュレータ較正で、誤判定すると健全な実機に `adb reboot` を撃つため、事前トリアージ・
  事後プローブ・失敗時の blank 証跡判定のいずれからも実機を除外してある。
...
- **端末のロックを解除しておく**(Android と同じ前提)。ロックされていると xcodebuild が
  `Unlock <name> to Continue` で無言のまま止まる。**ビルドと起動の前にホストが現況を見て促す**
  (`IOSPhysicalDeviceLock`。`xcrun devicectl device info lockState` の **`passcodeRequired`** =
  「今パスコードの入力が要るか」で、静的な設定の有無ではない。実測 0.23 秒・消灯 true /
  解除 false・自動ロック 30 秒での遷移も観測)。**ビルド前は促すだけで待たない**(ビルドに解除は
  要らないので、その数分の間に解除してもらう)/ **起動の直前は解除まで待ち**(2 秒刻み・上限は
  `BridgeLauncher.startupTimeoutSeconds` と同じ 180 秒)、**待ち切ったら撃たずに落とす**
  (撃っても deviceprep に拒否され、もう 180 秒待って同じ理由で死ぬだけ)。
...
## 横カルーセルの戻しが数十 px 足りず flake していた(2026-08-30・**解決** — scrollToLeftEdge)

5 SUT の `フリックが正しく動くこと.S0010` が、`flickRightToLeft` → `scrollLeft(scrollFrame:, repeat: 3)` →
`exist "#tag_01"` で落ちていた(E2E-iOS ios-inapp で 25/108 = 23%。既存メモでは「FM の死が引き金」
と読んでいたが、それは負荷状態の交絡だった)。

**決め手はホスト側プローブ**(各 scrollLeft 後の `tag_0x` の x を記録)。**通った run でも
戻り切っていなかった**:

| | scrollLeft 1回 | 3回の合計 | 慣性距離 | 先頭タグの位置 | exist |
|---|---|---|---|---|---|
| 通った run | 221px | 663px | ≈ 700px | 38px 画面外(82px 見える) | 木に載る → ✅ |
| 落ちた run | 221px | 663px | ≈ 770px | 85px 画面外(17/19 で一定) | 木から落ちる → ❌ |

合否を分けていたのは**数十 px の慣性のばらつき**で、シナリオの「距離に依存させない戻し」という
前提が誤りだった(scrollLeft×N は距離依存)。**修正はシナリオ側** —— 戻しを距離非依存の
`scrollToLeftEdge(scrollFrame:)` に(5 SUT の 10 箇所。ユーザー決定。ツール側で刻み幅を
60%→85% に上げる案は「依然として距離依存で、全 scrollFrame スクロールの挙動が変わる」ため不採用)。
検証: 5 SUT の単発 7/7 緑 + E2E-iOS スイート 25周で **0/25**(修正前 23%)。

**教訓**: 「部分可視で木に載る/落ちる」境界の上に立っていた検証は、緑でも安定していない。
scrollLeft/Right の repeat で「端へ戻す」つもりの書き方は他にもある(`05_スクロール.swift` の
repeat 2/3。こちらは失敗履歴なし・意図未確認)。
