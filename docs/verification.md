...
**直し**: 一様でも拍動(`displayIdleSeconds`)が生きていれば凍結としない
(`FrozenVerdict.countsAsFrozen`。run とモニターで同じ規則)。**申告が無い(旧ブリッジ・
ブリッジ無し)ときは従来どおり一様=凍結**にして保護を外さない。
実測: E2E-Flutter/ios-inapp が **387.0s → 76.7s**(10台の再起動が消え、24/25 → 25/25)。

**拍動を主にしようとしたが、実験で反証した**(同日): 一度は「拍動が生きていれば凍結ではない」
という規則にした。その直後に**本物の wedge を故意に起こして観測**したところ
(`Scripts/freeze-heartbeat-probe.sh`)、画面が完全に固まっていても**拍動は 0.001〜0.016s で
回り続けた**。拍動が測っているのは「アプリが vsync を要求しているか」で、
**「表示が進んだか」ではない** —— コンパイタが wedge してもコールバックは来る。
この規則のままでは**本物を1件も検出できない**ので撤回した。
