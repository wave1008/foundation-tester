...
**コスト(2026-08-03 に A/B 実測): Android で +1.1%。劣化は `scroll` / `scrollToEdge` 系に集中する**。

| | base | fix | 差 |
|---|---|---|---|
| E2E android・共通53シナリオの中央値合計 | 347.15s | 350.95s | **+3.80s(+1.1%)** |
| `scrollToTop` | 2,789ms | 3,796ms | **+1,007ms** |
| `scrollRight` / `scrollDown` | 2,043 / 2,218ms | 2,210 / 2,332ms | +167 / +114ms |
| `scrollTo`(探索) | 5,472ms | 5,465ms | −8ms |
...
`scroll` / `scrollToEdge` は**繰り返しのたびに `settledSignature` を呼ぶ**ので、
1 呼び出し ≧2 枚 × +35ms が回数ぶん積み上がる。探索(`scrollTo`)は 1 周につき 1 回なので出ない。
iOS は `supportsCacheBypass=false` で素通しなので実費ゼロ。

**それでも払う**: 素取得だと `scrollToEdge` の「2回続けて変化なし = 端」が**遅れて公開された
古い位置で成立して途中で止まる**(沈黙の誤り)。+1s と黙って誤る危険は釣り合わない。
