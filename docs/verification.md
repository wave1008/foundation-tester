...
**コスト: スイート全体では計測できる差が出ない**(2026-08-03 に A/B 実測)。1 枚あたりは確かに
高い —— ブリッジ直叩きで **5.1ms → 39.9ms(+35ms)**(emulator-5554・要素 5.8KB の画面・各 15 回の中央値。
**refresh の費用はノード数に比例する**ので重い画面ではもっと掛かる)。にもかかわらず
**E2E android フル 50 シナリオの共通シナリオ中央値合計は +0.1%(+0.40s)**、スクロール系だけに絞った
NDJSON 計測でも **`snapshotMs` は -2.5%**(base 5,674ms / fix 5,531ms。run 間のばらつきの方が大きい)。
枚数が少ないうえ、**古い木で空振りしていたスワイプと再試行が消えた分が相殺している**と読める。
**「+65ms なので探索の整定に限って使う」という以前の見積りは、実測で棄却された** ——
迂回を惜しんで沈黙の誤りを残す取引は成立しない。iOS ブリッジは `supportsCacheBypass=false` で
素通しなので実費ゼロ(対照実験: E2E ios-xcuitest の同じ A/B は **−0.0%(−0.06s)**)。
