...
`#carousel_tags` も scrollable なし)。申告できるエンジン(SwiftUI の table・Android の
RecyclerView・in-app の UIScrollView)では未指定でも容器を対象にして刻みを抑えている。

**残る宿題**: 申告に頼らず容器を推定する(例: 1回目のスワイプで**実際に動いた要素の外接矩形**を
容器とみなす自己発見)。実装すれば Compose でも未指定のまま塞げる。
