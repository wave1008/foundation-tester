...
**残存(未解決)**: 修正後も**高負荷の並列実行で約5%落ちる**(2026-08-03 実測: 単独 10/10 /
別スイート並走下 9/10・12/12 / フルスイート中に1回 = 累計 43 回中 2 回)。値は `selected=-` で
**タップが飲まれる**形。修正前は決定的に落ちていたので**退行ではなく取りこぼし**。
計装つき 12 回では再現しなかった。**S0110 が落ちたらまずこれを疑う**(新しい退行と誤認しない)。
次に追うなら、`runScrollSearch` の FOUND(掴んだ要素・viewport・祖先)と
DRAG(空打ちの点・safe・taken)をファイルへ出す計装を、別スイート並走下で回す。
有力な未検証の仮説は「空打ちの点が手前の要素に取られて**丸ごと飛ばされ**、容器の1タッチが
肩代わりされない」—— いまは飛ばすだけだが、容器の矩形は分かっているので**別の安全な点を選ぶ**
余地がある(未実装・未検証)。
