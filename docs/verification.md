...
**さらに画像を判定から外した**(同日): 当初は「一様 AND 拍動停止」にしたが、拍動があるのに
画像も要求するのは**非決定的な材料に判定を依存させたまま**で、しかも**固着型
(最後のフレームが残る = 非一様)を原理的に取り逃がす**。規則を次に変えた:

| 拍動の申告 | 判定 |
|---|---|
| 停止(> `FrozenVerdict.displayIdleFrozenThreshold`) | **凍結**(画像は見ない) |
| 生きている | 凍結ではない(画像が一様でも) |
| **申告なし**(旧ブリッジ・ブリッジ無し) | 一様 = 凍結(**予備**。保護を外さない) |

しきい値は 5 秒。拍動はアプリのメインスレッド上の CADisplayLink / Choreographer なので、
メインスレッドの長時間占有で飢えうる —— 実測では Flutter の初回描画中でも 0.03〜0.10s だった。

**教訓**: 代理指標(proxy)は「その症状が出る別の原因」を必ず持つ。症状ではなく**直接の量**
(ここでは vsync 由来の拍動)を測れるなら、そちらを判定の主にし、代理指標は
**直接の量が取れない場合の予備**へ降格する。
なお **本物の凍結で拍動が止まる観測はまだ採れていない**(この期間に本物が1件も無かった)。
これが採れるまでは「拍動が止まらない wedge」の可能性が残る —— 採れたら
`FrozenEvidence.noPresent` のコメントを更新する。
