...
- **打ち切り**(`unmovedRoundsToStopSearch = 2`): 木が2周続けて1文字も変わらなければ、
  上限まで振らずに失敗させる。**8スワイプ 7.40s → 2スワイプ 2.05s**(実測)。
...
**まだ残っている**: 容器の**縁にまたがった行**へのタップは、Compose が focus 時に
bringIntoView で内容を動かすため隣の行に当たる(Android で約 50%・実測 135〜179px のずれ)。
「またぎまで掴み直しの対象を広げる」のは 2026-08-05 に S0110 が 2/10 → 5/10 に悪化して撤回済みなので、
**やるなら slowDrag での穏やかな寄せを S0110 で測ってから**。
