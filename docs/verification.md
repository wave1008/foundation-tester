...
**残存(未解決・約2%)**: 修正後も稀に落ちる。累計 **93 回中 2 回**(無負荷 52 回で 0 /
別スイート並走下 40 回で 1 / フルスイート中に 1)。値は `selected=-` で**タップが飲まれる**形。
修正前は決定的に落ちていたので**退行ではなく取りこぼし**。**S0110 が落ちたらまずこれを疑う**
(新しい退行と誤認しない)。

**計装での捕捉は断念した(2026-08-03)**: FOUND(掴んだ要素・viewport・祖先)/ DRAG(空打ちの
点・safe・taken)/ TAP(実際にタップした対象)をファイルへ出して **42 回(うち 30 回は
別スイート並走下)** 回したが**一度も再現しなかった**。2% を捕まえるには桁違いの反復が要る。
