...
- 実害: ランナー側の修正2件を入れ、`bridgeProtocolVersion` も上げ、`swift test` も緑にした状態で
  実機確認したところ、**snapshot は直る前の木を返し続けた**。原因は稼働中の旧ランナー(v52)の
...
- **ブリッジを直したときの検証手順**: 版を上げる → `ftester bridge down && ftester bridge up`
  → 実機で確かめる。**down/up を省くと必ず旧版を測る**。iOS のシミュレータはブリッジを
...
- **4 SUT は同じシナリオでも前提が違う**。画面に要素を1つ足しただけでリストの可視行数が変わり、
...
### 実機で確かめるときに嘘をつく2つの実験系(2026-08-05)

どちらも「実装は正しいのに実機で誤った結果を見る」形なので、原因をコード側に探すと迷う。
...
**スナップショットの検知は dry-run では当てられない**(木が要る)。2026-08-06 に MCP へ
「遮蔽されている要素」「同一矩形に積まれた要素」の警告を足したときは、**ビルドした
`ftester-mcp` に JSON-RPC を流すハーネス**を書き、4 SUT × iOS/Android の 6 構成 ×
主要 11 画面 = **72 画面**を機械的に巡って旗の立つ行を全部並べた。結果は
真陽性 34 行(スクロール残骸)・**誤検知 0**・タップ警告 1 件(既知の `#nav_heal`)。

この形の検知は**誤検知の履歴がある**(2026-08-06 に ghost 判定を拒否から警告へ後退させた
原因が5形の誤検知)ので、「単体テスト緑」では絶対に足りない。ハーネスは
`ftester-mcp` を1プロセス立ち上げっぱなしにして `ft_launch`/`ft_snapshot`/`ft_tap` を
順に流すだけでよい(セッション状態が要るので**1リクエスト1プロセスにしない**)。
...
| iOS | SwiftUI/UIKit | Compose | Flutter |
|---|---|---|---|
| XCUITest | ダブルタップ ✅ / ピンチ ✅ | ダブルタップ ❌ / ピンチ ✅ | ダブルタップ ✅ / ピンチ ❌ |
| in-app | 両方 ❌(合成タッチを受理しない) | 両方 ✅ | 両方 ✅ |
