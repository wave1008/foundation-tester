...
### Compose の探索直後タップ(2026-08-03: Android は修正済み・iOS は `scrollFrame` で塞ぐ)
...
#### 原因1(iOS・未修正・`scrollFrame` で塞ぐ): frame クランプ

領域を指定しない探索は**全画面スワイプ = 1回の移動量がリストのビューポートを超える**。
Compose iOS は**画面外の要素の frame をビューポート内へクランプして報告する**
([[compose-ios-ax-frame-clamp]])ので、飛び越した行が「見えている」ように見え、探索がそこで止まって
**実際には別の行が居る座標をタップする**。

**対処は `scrollFrame` で領域を指定する**。刻みが容器の 0.5 に収まって飛び越しが無くなり、
CMP/ios-xcuitest は 3/3 で安定した(未指定は 0/3)。commands.md に明記してある。

**自動では直せない**: 未指定時に容器を自動で選ぶには `ElementInfo.scrollable` の申告が要るが、
**Compose の容器は xcuitest では `other` として出て申告できない**(実測: `#list_rows` も
`#carousel_tags` も scrollable なし)。
...
`Projects/E2E/Scenarios/07_スクロール.swift` の **S0080**(`scrollTo("#row_40")` →
`tap("#btn_scroll_top")` → `withScrollDown(scrollFrame:) { tap("#row_30") }`)。
**CMP だけ `scrollFrame` を指定する形で置く** —— 未指定形は原因1(iOS の frame クランプ)で
決定的に外れたままだから。他 3 SUT の S0080 は未指定形で通る。
