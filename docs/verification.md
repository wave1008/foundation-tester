...
### xcuitest の digest 警告は 2026-09-02 時点で「鳴りっぱなし」(未検証の意味ではない)

**この警告が出ていても、xcuitest 経路が未検証とは限らない。** 現状はそうなっている:

- 記録済み digest は **2026-08-30 10:23** のもの。その後 `02114750`(2026-08-31・ブリッジ版 85。
  clear/読み返し・横向きの既定スワイプ・任意 ref の scrollFrame ほか)が入力集合を動かした
- **2026-09-02 に `--ios-xcuitest` を回して実質的な検証は済んでいる** ——
  E2E-CMP / E2E-iOS / E2E-Flutter は全緑で、**版 85 による退行は出なかった**
- ただし **E2E-RN が2本落ちる**: `hello123`→`hllo123` / `persist99`→`prsist99`。
  どちらも **打鍵の中抜け**(`TypeReadback.plan` が中央1文字の欠落を `.unverifiable` で
  黙って受理する既知の穴。2026-08-25 に HEAD でも同じ2本・同じ文字列で再現を確認済みで、
  **実装は同日ユーザー判断で見送り**)。**版 85 とは無関係**
- 記録は**全プロファイル成功が条件**なので、この2本が赤である限り **digest は永久に更新されない**

**したがって、この警告を見たら「xcuitest を回せ」ではなく、まずここを読むこと。**
回しても(既知の2本で赤になり)印は更新されず、同じ警告がまた出る。

**2本を `_disabled/` へ退避して印を通す案は採らない** —— この2本が**打鍵の中抜けを検出できる
唯一の場所**で(2026-09-02 の実行でも2回とも正しく赤にした)、消すと検出器を失う。
**記録条件を緩める案も採らない** —— 「既知の失敗」を表現する仕組みが無く、
赤を抱えたまま「検証済み」を名乗る装置になる(e2e.sh のコメントにある通り)。

**この状態が解消するのは打鍵の中抜けを直したときだけ。** それまでは鳴り続ける前提で、
**本当に xcuitest ブリッジを触ったときは digest ではなく「自分が何を変えたか」で判断する。**
