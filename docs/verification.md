...
対処は2段(どちらも探索ループと同じ判定 `clippingAncestor` を使う):
...
これを復元して見切れ判定の viewport に使う(`StepExecutor.clippingAncestor`)。
...
- **タップの空振り**: /tap が 200 を返したのにタップが処理されない。計測中に1件だけ観測
  ...
  **3・4例目と、初めて掴んだ安い再現条件(2026-08-05)**: 同じ S0010 の**シーン1**
  (`launch` → `tap("#nav_webview")` → `textIs("#txt_screen_title") == "WebView"`)が
  **10 周中 2 回** `actual "ホーム"` で落ちた —— **アイドルの1レーン**で、fix 側と base 側の
  両方で1回ずつ(= コード変更とは無関係)。過去 630 run のレポートには同じ形が1件も無い
  ので、効いているのは**同一シナリオを連続で回すこと**(毎回 launch し直す)らしい。
  発生率 2/10 は 95% 区間が 3〜56% と広く、まず**同じ回し方で 30〜50 周**して率を確定させるのが
  次の一手(これが本当に 20% 級なら、1/529 の E2E-iOS 例を追うより桁違いに安い)。
  **追うときに実行中の I/O を足さないこと** —— 計装で消えた前例がある(下の heisenbug)
...
**修正(2026-08-05)**: 失敗時の白/黒フレーム推定を **Android 限定から「実機以外」へ広げた**
(`FTRuntime` の `inferFrozenFromBlankFrame`)。これで iOS でも
「⚠️ The evidence screenshot is a blank frame (frozen device display)」がレポートに出て、
既存の `deviceFrozen` 経路(ワーカーを外して**別デバイスへ振り直す**)に乗る。
**挙動変更である**ことに注意 —— 従来 iOS では環境全滅が「テストの失敗」として無警告で記録されていた。
副作用は「失敗時のスクショが一様だと最大3回撮り直す(+7.5s)」「そのデバイスを1回外す」だけで、
成功パスには一切かからない。

**まだ無いもの(候補)**: Android にある**実行前トリアージと sleep/wake 修復**
(`ProfileWorkerFactory.excludeOrRepairBlankScreenWorkers`)の iOS 版。今は失敗して初めて分かる。
...
**Compose iOS も同じクラスだが規則が違う**: あちらは straddle した行で
「原点はクリップ前・高さはクリップ後」の混成を返し、**中心が可視領域の外**に落ちる
(S0110 の失敗 21 件中 11 件がこの形。`#row_30` が `y=206 h=26〜45`・容器の上端は 230)。
`clippingAncestor` は「祖先と交差しないこと」を条件にしているため、**またいでいる形は検出できない**
(= ghost 注記が付かないまま容器の外をタップする)。[[compose-ios-ax-frame-clamp]]
