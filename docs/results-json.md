...
今日のレポート / 生きているブリッジのログ / 稼働中ブリッジが開始した後の添付 / **生きているブリッジが
書き込み中の xcresult の束**。
...
同じ理由で、ブリッジを起こす `xcodebuild` には**結果の束と作業フォルダの置き場を必ず渡す**
(`-resultBundlePath <tool>/.fleetest/xcresult/bridge-<port>.xcresult` を起動のたびに作り直す・
`-derivedDataPath` はビルドと同じ置き場)。渡さないと Xcode が既定の DerivedData に起動ごとの新しい
フォルダを作り、誰も読まない結果の束を積んだ(実測 100 個・1.8 GB・1 日約 50 個)。**孤児**
(生きたランナーの居ないポートの束)は `BridgeLauncher.sweepOrphanResultBundles` が provision の
入口で無条件に消す。

**`xcresult` は生きているブリッジの分を消せない**(guarded)。束の中身は XCTest が起動から継続して
書き込む**セッションログ**(「終わらない UI テスト」の全操作 —— `XCTWaiter` / `XCTContext` の活動の
生ログ)で、`BridgeLauncher.captureSettings`(動画・スクショを止める設定)の対象外(2026-09-20 に
判明。実測: 実機ブリッジ1本を 75 分立てただけで 324 MB、8台規模の負荷試験では全体で 733 MB/時)。
**このログを起動時の設定で絞れる手掛かりは調べて見つからなかった** —— xcodebuild の
`-collect-test-diagnostics on-failure|never`(xctestrun の `DiagnosticCollectionPolicy`)は候補に
見えたが、その説明(「失敗検知時の sysdiagnose 風の追加診断」)どおり失敗時専用の別診断を切り替える
もので、このセッションログ自体は一度も失敗していないブリッジでも起動から継続して書かれる
(この Mac の実データで確認: 3時間以上生きているブリッジの束のログが、テスト失敗が一度も
無いまま伸び続けていた)。効果を A/B で確かめる手段(同時に2本のブリッジを長時間立てて
増分を比べる)がこのリポジトリの検証にはまだ無いため、確かめずに設定へは足していない。
掃除ができるのは**孤児**だけで(通常は `sweepOrphanResultBundles` がここへ来る前に消している)、
guarded だけで上限を超えたときは他のカテゴリと同じく**事実を言う**(何が容量を占めているか・
立てっぱなしのブリッジを落とせば消せること)。実機ブリッジや MCP セッションのように立てっぱなしに
する使い方では 1 台あたり約 6 GB/日で伸び続けるので、5 GiB という上限は**保持量を抑える線ではなく
気付かせる線**(単発のブリッジでも約 20 時間で届く)。

**掃除が見る場所は2つ**(`RetentionSweeper.Roots`)。録画・レポート・`install.sh` のログは
**シナリオのパッケージ**(受け手の WORK_DIR)、ブリッジのログと台帳は**ツールのクローン**にある。
受け手の外部構成(既定)では別の場所なので、背景の掃除には親が決めた2つを `FT_PACKAGE_ROOT` /
`FT_TOOL_ROOT` で渡す(子に解決し直させると取り違え、受け手の録画・レポートを1度も掃除しなかった)。
**保守者のクローン構成では2つが一致するので、この取り違えは手元の実データでは出ない**。
...
| freezeRetries | [String]? | 結果取り消し+振り直しの監査記録。表示用。**Android のドライバ不達(ブリッジ断)もここに載る**(2026-09-16 より。ワーカーは離脱させない) |
...
書かないので、回数欄は「実仕事」だけを表し続ける。**Vision に死活・プローブは無い**(回数欄だけ)。
