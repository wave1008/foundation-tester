...
(プロセスに1回)である。対策は2つ入っている: ①暖機は executor を作った時点で始める
②OCR 段に予算(`RegionText.occlusionBudget` = 1.3 秒 = 置き換える相手である FM 照合の実測下限)を
持たせ、超えたら FM へ落として注記 `ocr-budget-exhausted` を残す。
