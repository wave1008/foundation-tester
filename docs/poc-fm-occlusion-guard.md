...
## 5.10 DSL: `exist` を既定 occlusion-guard 化 + `present`(ツリー存在のみ)(2026-07-22)

occlusion-guard を利用側の既定挙動にし、**2 コマンドに整理**した(ユーザー決定)。`visible()` は削除。

```swift
exist("#msg")     // 既定: ツリー存在 + 実際に見えているかを FM で確認(見えなければ失敗)
present("#icon")  // ツリー存在のみ(見えているかは問わない・高速・アイコン/画像向け)
```

- **`exist`**: 存在検証に加え、ツリー一致で pass した直後に occlusion-guard を発火
  (足切り→低インク/幾何→FM→visible)。覆われ/切れ/不在なら偽陽性として失敗に反転。
- **`present`**: occlusion 確認を省いた版(= `exist` の occlusion 確認なしと等価)。FM を一切呼ばない。
- **配線**: `FlowStep.occlusionGuard: Bool?`([Flow.swift](../Sources/FTCore/Flow.swift))を `exist`=true /
  `present`=false で立て、[StepExecutor.occlusionFlip](../Sources/FTCore/StepExecutor.swift) が
  `step.occlusionGuard ?? executor 既定` で判定。executor 既定は false のまま(api/record 経路や
  raw FlowStep=nil は非ガード)。
- **安全策**: FM 未配線(不可)時は `exist` の guard も素通り(pass)= `present` と同一挙動。足切りで
  非テキスト型(アイコン/画像/絵文字)は自動除外、高インク領域は FM を呼ばない(コスト最小化)。
- **ソース契約**: 逆写像コード生成(guarded=`exist(...)` / 非guard=`present(...)`)・日本語説明
  (`exist`→「(覆われず)見えていること」/ `present`→「表示されること」)・VSCode パラメータ編集(timeout)を
  両コマンドで登録([ScenarioCodeGen](../Sources/FTDSL/ScenarioCodeGen.swift)・[StepDescription](../Sources/FTDSL/StepDescription.swift)・
  [StepCommandText](../Sources/FTDSL/StepCommandText.swift)・[StepCommandParams](../Sources/FTDSL/StepCommandParams.swift))。
- **テスト**: 反転/pass/オプトイン(raw exists=nil は FM 不呼出)を StepExecutorTests、`exist`/`present` の
  説明・パラメータ解析を StepDescriptionTests/StepCommandParamsTests で検証(全 green)。

> **コスト注意**: `exist` 既定 ON により、各 exist 通過で **スクショ 1 枚**(+疑わしい低インク領域のみ
> FM ~1.6s)が乗る。可視テキストが多い通常画面は高インクで FM を呼ばないため増分は小さいが、
> 大量に存在検証を回すシナリオでは可視性が不要な箇所を `present()` にするのを推奨。
...
| `present("レビュー")`(ガード無) | passed | ✓ ツリー存在のみで通過 |
