...
**ただし FM はホスト全体で直列化する**(スループットは並列度によらず約1回/秒、レイテンシは
並列度に正比例)。実行時間には「FM 呼び出し総数 × 約1秒」の下限ができ、**デバイスを増やしても
縮まない**。詳細な実測値と、発火が増えたときの絞り方は
[performance-tuning.md §3.5 / §4.2](performance-tuning.md)。
