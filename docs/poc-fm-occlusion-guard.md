...
モデルが**メモリに載っていない**とプロセスで最初の 1 回だけ 25〜47 秒かかる(以後は 40〜130ms)。
**ガードが実際に走った回**に背景で1回だけ小さな画像を読ませる(`RegionText.prewarmIfNeeded`。
off のときは撃たない)。生成のたびに暖機すると、ガードが一度も撃たれない executor
(MCP の操作用・`falsePositiveCheck` オフ・dry-run)でもモデルを読み込む。モデルが常駐している間の run では最初の 1 回も 54ms で済むが、
**常駐が切れていると暖機の完了前に撃たれたガードが同じ読み込みを待つ**(実測 47 秒)。
プロセスに 1 回だけで、読めた回はそのまま pass になる(締切超過で赤にはならない)ため、
待たせない機構は置いていない。
...
**利用者の口は実行プロファイルの `ocr`**(既定 true。拡張のプロファイルタブ「FM(Foundation Model)/OCR」
セクションの「OCRの機能を使用する」)。`false` にすると `--no-ocr` がランナーへ渡り、
