# PoC レポート: FM 視覚照合によるアサーションの誤った緑の排除(occlusion-guard)

ブランチ: `poc/fm-occlusion-verify` / 実施日: 2026-07-21 / 実行: メインセッション(Claude Code)

## 1. 背景と課題

`exists` / `textEquals` / `valueEquals` の判定は、スナップショット(アクセシビリティツリー)上に
要素が存在するかだけを見る([StepExecutor.matchDetailed](../Sources/FTCore/StepExecutor+Resolve.swift))。
可視性フィルタは「hidden・サイズ0・画面外」しか落とさず、**別要素に覆われている(occlusion)/
減光されている/切れている**要素はツリーに残るため、**視覚的に見えていないのにアサーションが成功する
誤った緑**が起こり得る。`hittable` は本来この検知に使えるが、①現行 wire DTO(`ElementInfo`)に
hittable は無く、②Compose iOS では `isHittable` 自体が壊れている([compose-ios-ax-frame-clamp])ため
採用できない。

そこで、**描画(スクリーンショット)を根拠に FM(オンデバイス Foundation Models)で可視性を照合し、
覆われ/切れ/減光/不在なら assert を失敗へ反転する**方式の実現可能性・正確性・速度を検証した。

## 2. 実装(PoC)

| 追加物 | 位置 | 役割 |
|---|---|---|
| `OcclusionVerifier` | [Sources/FTFoundationModels/OcclusionVerifier.swift](../Sources/FTFoundationModels/OcclusionVerifier.swift) | FM 視覚照合器。`@Generable VisibilityVerdict{ visible, state, observedText, reason }` を greedy で生成 |
| `ReplayDelegate.verifyElementVisible` | [Sources/FTCore/StepExecutor.swift](../Sources/FTCore/StepExecutor.swift) | FM 非依存の delegate フック(既定実装 nil)。FTCore を FM から切り離したまま結線 |
| `StepExecutor.occlusionGuard` + `occlusionFlip()` | ノブは同上。`occlusionFlip` の現在の実体は [Sources/FTCore/StepExecutor+Assert.swift](../Sources/FTCore/StepExecutor+Assert.swift) | ノブ。exists/textEquals がツリー一致した**一点で1回だけ** FM 照合し、`visible==false` なら `.failed("false positive (occlusion)…")` へ反転(誤った緑を赤へ) |
| 計測ハーネス | `ftester-poc-occlusion`(PoC ブランチ履歴・§8) | 正解ラベル付き合成フィクスチャで正確性・速度を計測 |

検証器は 2 アームを実装して比較した:

- **cropped(本命)**: スクショを要素 frame(+24px 余白)に**クロップ**して FM に渡す。座標を言葉で
  説明する必要がなく、視覚モデルは「この領域に期待テキストが明瞭に見えるか」だけに集中できる。
  frame は pt、スクショは px なので `screen` サイズからスケールを求めて px クロップする(Retina 対応)。
- **full(比較用)**: スクショ全体＋frame 座標を**言葉で**渡し、座標→ピクセル対応を FM に委ねる。

## 3. 計測方法

実機・シミュレータ不要。iPhone 相当の logical 画面(402×874pt)を **renderScale=2 のピクセル
(804×1748px)** で CoreGraphics 合成し、pt→px 換算経路も実地で通す。ナビバー・ディストラクタ文言を
常に描き「画面に文字があれば可視」と短絡できないようにした。3 テキスト(和/英/記号)× 6 カテゴリ =
**18 フィクスチャ**、各を両アームで判定(計 36 FM コール)。2 回反復。

| カテゴリ | 内容 | 期待(assert が pass すべきか) |
|---|---|---|
| visible | 期待テキストを明瞭に描画 | **見える(pass)** |
| coveredFull | 全面を不透明矩形で覆う | 見えない(reject) |
| coveredPartial | 左 65% を覆う | 見えない(reject) |
| dimmed | alpha 0.12 で薄く描画 | 見えない(reject) |
| absent | 領域に何も描かない(ツリーだけ存在を主張) | 見えない(reject) |
| mismatch | 別テキストを描画 | 見えない(reject) |

指標: **誤った緑の排除の再現率**(隠れ 15 件を reject できた割合=機能の価値)、**有害な誤反転率**
(正しく見えている 3 件を誤って reject=導入の害)、全体正解率、1 回あたりレイテンシ。

## 4. 結果

2 回とも**ほぼ同一**(greedy のため決定的。full アームの誤りも同じ 4 件が再現)。

### 正確性

| アーム | 全体正解率 | 誤った緑の排除の再現率 | 有害な誤反転率 |
|---|---|---|---|
| **cropped** | **18/18(100%)** | **15/15(100%)** | **0/3(0%)** |
| full | 14/18(78%) | 13/15(87%) | 2/3(67%) |

- **cropped は全カテゴリ完璧**。覆い・部分覆い・減光・不在・文言違いをすべて `visible=false` で弾き、
  正しく見えている 3 件は `visible=true` を維持。**誤った緑を 100% 排除し、正しい要素を一度も却下しなかった。**
- full は逆に**正しく見えているテキストを "covered" と誤判定して却下**(有害誤反転 67%)。座標を言葉で
  渡す方式は視覚モデルには機能せず、実用不可。

### 速度(1 回あたり、ms)

| アーム | min | median | p90 | max | mean |
|---|---|---|---|---|---|
| cropped(run1) | 1459 | 1618 | 1751 | 2937 | 1673 |
| cropped(run2) | 1454 | 1589 | 1782 | 2623 | 1655 |
| full(run1) | 1653 | 1911 | 2009 | 2201 | 1917 |
| full(run2) | 1696 | 1883 | 2036 | 2265 | 1909 |

- cropped は **median ≈ 1.6s / p90 ≈ 1.75s**。max(2.6〜2.9s)は各実行の**初回のみ**でモデルの
  コールドスタート。以降は定常。クロップで画像が小さい分 full より速い。

## 5. 分析

- **クロップが決定打**。座標を自然言語で渡す弱い手がかりを排し、視覚モデルに「この小領域を読めるか」
  という単純問題へ還元したことで精度が 78%→100% に跳ねた。設計判断(cropped を本命)は妥当。
- **`state` ラベルは不正確だが `visible` は正確**。cropped は coveredFull/Partial を "notRendered"、
  dimmed を "covered" と分類するなど state は当てにならない。だが**ガードが使うのは `visible` bool
  のみ**で、そこは 100% 正しい。state は診断メッセージ用の参考に留め、判定には使わない設計で問題ない。
- **コスト**は「ツリー一致した pass の一点で1回」。全 assert に付けると 1 ステップ +1.6s 程度。
  通常の in-app snapshot(~0.05ms)/操作に対し大きいので**常時 ON は非現実的**、ノブで**画面/シナリオ
  限定**が前提。

## 5.5 FM を「疑いがある時だけ」呼ぶ自動ゲート(追加検証)

全 assert で FM(+1.6s)を払うのは非現実的なので、**安価な事前シグナルで FM をゲート**する2段前段を
追加実装・計測した。FM に回すのは疑わしい時だけにする。

- **Tier-0 幾何**([OcclusionSuspicion.swift](../Sources/FTCore/OcclusionSuspicion.swift)・ツリーのみ・FM/スクショ不要):
  対象 frame を 40%以上覆う「記載順で後(手前寄り)の要素」/画面外はみ出し(切れ)/ロケータの
  部分一致解決、のいずれかで疑う。合成スナップショットで **5/5** 正しく発火/非発火。
- **Tier-1 ピクセル**([RegionInk.swift](../Sources/FTCore/RegionInk.swift)・スクショのみ・FM不要):
  対象 frame 領域の輝度 stddev。テキストがあれば中〜高、覆い/空/減光では低い。

ゲート規則([StepExecutor.occlusionFlip](../Sources/FTCore/StepExecutor+Assert.swift)):
**Tier-0 で疑い、または Tier-1 の stddev が閾値未満**のときだけ FM(cropped)へ。両方無罪なら FM 省略で pass。

### 事前フィルタの分離性(実測 stddev)

| カテゴリ | stddev | 判定 |
|---|---|---|
| visible | 60〜74 | 高(FM 省略で pass) |
| coveredFull / absent | **0.0** | 低(FM へ) |
| dimmed | 7〜9 | 低(FM へ) |
| coveredPartial | **31.5** | 高寄り(Tier-1 単独では取りこぼす) |
| mismatch | 48〜76 | 高(Tier-1 単独では取りこぼす) |

可視(≳60)と 全覆い/空/減光(≲9)は **大きなギャップで分離**。閾値 10〜30 のどこでも同挙動(既定 12)。

### ゲート後パイプライン(Tier-1 単独・閾値スイープ)

| 閾値 | FM呼出 | FM削減 | 見逃し | 有害誤反転 | 総合正解 |
|---|---|---|---|---|---|
| 12〜25 | 9/18 | **50%** | 6 | **0** | 67% |

- **FM 呼出を半減、有害な誤反転は 0**。全覆い/空/減光は Tier-1 が確実に FM へ回して捕捉。
- 見逃し 6 = coveredPartial×3 + mismatch×3。**Tier-1 単独の弱点は「部分覆い」と「文言違い」**
  (残テキストのインクが多く高分散に見える)。
  - mismatch は本来 occlusion ではなく、`textEquals` の完全一致・`exists(id)` の実在で別途弾かれる筋
    (このガードの対象外と見なせる)。
  - coveredPartial は **Tier-0 幾何**が補う: 覆う要素がツリーに載っていれば、高インクでも FM に回す。
    Tier-0/Tier-1 を OR で併用する実装にした理由がこれ。
- **Tier-0 で拾えない残余**(覆う要素がツリーに載らない部分覆い)は原理的に FM 前段では検知不能。
  ここだけは「全件 FM」でないと 100% にはならない。実運用では稀と見て、ゲート既定 ON を推奨する。

## 5.7 実 UI(Simulator)再計測 — 合成では見えなかった重大な誤反転(2026-07-21)

空きデバイス **A012ADD8**(iPhone 17 Pro/iOS 27、モニター占有外)に inapp ブリッジを注入起動し、
**sut-ec-mobile** の実画面で計測(ハーネス `ftester-poc-occlusion` の Live.swift・§8、
`ftester-poc-occlusion live <udid> <bundle> <port> <dir>`)。ground truth はスクショ目視で確定。

### 結果: unconditional FM は実 UI で**約50%の有害誤反転**

ランディング + おすすめ商品の実テキスト/アイコン 24 要素中、**視覚的には全て可視**なのに
cropped 検証器が `visible=false` を返した誤反転が **約12件(≈50%)**。レイテンシはデバイス上で
median≈1.5s(合成と同等)。誤反転はすべて次のパターン:

| 誤反転の型 | 例 | 原因 |
|---|---|---|
| **アイコン/画像**(a11y label は説明文) | ハート「お気に入りに追加」、商品画像 label=商品名 | 画面に描かれるのは絵で、label の文字列ではない → 「その文字が無い」と判定 |
| **絵文字** | 「📱」「👕」単体要素 | 絵文字を期待テキストとして読めず notRendered |
| **結合セマンティクス** | 「📱, 家電・電化製品」(タイル全体) | 絵文字+全文の合成 label。視覚とは別物 |
| **省略テキスト(…)** | label「家電・電化製品」→ 実描画「家電・電化**…**」 | 末尾省略で全文が見えない → 覆い/切れ扱い |

**決定的な対比**: 同じ行で label「ファッション」(省略なし)は `visible=true` ✓、
label「家電・電化製品」(実描画は「家電・電化…」)は `visible=false` ✗。**省略(ellipsis)が
誤反転の主因**であることがデバイス上で確定した。合成フィクスチャは省略/アイコン/画像/絵文字を含まなかった
ため、この失敗モードを完全に見落としていた。

### なぜ「疑い時だけ FM」ゲートが実運用では安全側に働くか

上記の誤反転要素の輝度 stddev は **すべて 23〜99(高インク)**。ゲート(閾値 12)は高インク領域で
**FM を呼ばず素通り(pass)**にするため、**これらの誤反転は一切発生しない**(gate ON なら実質
0 誤反転)。FM が起動するのは stddev<12 の**ほぼ無地**の領域だけ — そこは「覆い/空」が明白で、
FM が最も信頼できるケース。逆に言うと:

- **unconditional FM(全 pass で FM)は実 UI では採用不可**(誤反転が多すぎる)。
- **2段ゲートは、FM が不得手な高インク領域(アイコン/画像/省略)から FM を遠ざける**ことで
  安全性を確保している。ゲートは単なるコスト最適化ではなく**正しさの前提**。
- ただしゲートは低インク領域しか FM に回さないので、**高インク下に隠れる部分覆い**は依然
  取りこぼす(Tier-0 幾何で一部補うのみ)。

## 5.8 足切り+省略許容を実装して再計測 — 誤反転 50%→0%(2026-07-22)

§5.7 の誤反転(アイコン/画像/絵文字/結合/省略)への対策を実装し、同じ A012ADD8 で再計測した。

**対策1: FM 前の要素足切り**([OcclusionEligibility.swift](../Sources/FTCore/OcclusionEligibility.swift)・ツリーのみ)。
`StaticText`/`Text` 等のテキスト型のみを対象にし、**非テキスト型(Button/Image/Cell)・結合 label
(`, ` を含む)・文字を含まない label(絵文字/記号のみ)を除外**。デバイスダンプで検証:

| 除外されたもの | 残ったもの(適格) |
|---|---|
| カテゴリタイル(Button)、商品カード(Button)、商品画像(Image)、ハート(Button/Image)、絵文字単体「📱」 | 見出し・バナー・カテゴリ label・商品名・価格・評価・「セール」「新着」等の StaticText |

**対策2: 省略許容プロンプト**([OcclusionVerifier.swift](../Sources/FTFoundationModels/OcclusionVerifier.swift))。
「末尾の… や折り返しは visible=true。false にするのは 覆い/単色空白/全く別の文字列 の時だけ」に改訂。

### 結果: 実 UI で有害誤反転 0

3 画面・**適格テキスト 36 要素すべてで `visible=true`(ground truth 全可視 → 誤反転 0/36)**。
各画面 26 要素は足切りで FM を呼ばず除外。レイテンシ median≈1.6s。

- **決定的**: §5.7 で誤反転していた ref9「家電・電化製品」(実描画「家電・電化**…**」)が、省略許容で
  `fullyVisible` に是正。価格「¥18,000」評価「(610)」商品名など省略/記号交じりの実テキストも全て正。
- **誤反転率 ≈50% → 0%**(実 UI・同一デバイス)。足切り+省略許容で素の FM の実用不能問題は解消した。

**未取得**: デバイス上での true-positive(実際に覆われた要素を false と当てる)。iOS シミュレータは
ハードウェアキーボード接続時にソフトキーボードが出ず(02≈03 が同一)、狙った occlusion を作れなかった。
occlusion 検知能力の裏付けは現状 §4(合成: 覆い/空/減光を 100% 検知)+低インクゲートに依存。
実オーバーレイ(モーダル/シート/スナックバー)での true-positive 確認が残タスク。

## 5.9 デバイス上で true-positive を取得(2026-07-22)

`occtest` モード(ハーネス Live.swift・§8)で実 occlusion を作って検証:
商品詳細で「カートに追加」→ スナックバー(`カートに追加しました 見る`)が下部を覆う瞬間に snapshot+screenshot し、
覆われた適格テキストを verifier が捕捉できるか確認した。ground truth はスクショ目視。

| 要素 | 追加前 | スナックバー中 | ground truth | 判定 |
|---|---|---|---|---|
| **¥24,000**(111,695) | visible ✓ | **covered** | 追加前=見える / スナックバーが上端を覆う | **true-positive**(可視→覆いを正しく追従) |
| **レビュー**(16,857) | covered | covered | 両方とも sticky「カートに追加」バーの裏で不可視 | **true-positive**(安定・sd=0 でゲートが FM 起動) |
| 他の可視テキスト(価格/評価/商品名/バナー等) | visible | visible | 可視 | 正(誤反転 0) |
| スナックバー自身「カートに追加しました」 | — | visible | 可視 | 正 |

- **狙いどおりの実証**: 同一要素「¥24,000」が、スナックバー出現で `visible→covered` に正しく反転。
  「ツリー上に在るが視覚的に覆われて見えない」誤った緑をデバイス上で捕捉できた。
- **レビュー**は sticky ボトムバーの裏に隠れた要素で、静止画面(追加前)でも安定して covered。
  当初の動機(ツリーに在るが見えない)そのものの実例を、低インクゲート(sd=0)が FM に回して正しく検知。

### 発見した運用上の罠: snapshot と screenshot の非原子性

スナックバー中「見る」(311,717・実際は可視)を sd=0・covered と誤判定。原因は **snapshot(frame)と
screenshot(pixel)を別コールで取得するため、アニメーション中は両者が時間的にズレる**こと。
frame は或る瞬間、pixel は別の瞬間のもので、crop がズレて空領域を掴む。
→ **占有ガードは静止後に実行するか、snapshot/screenshot を極力連続で取得すること**が前提。
StepExecutor.occlusionFlip は既に「操作の settle 後・pass 確定の一点」で1回だけ実行する設計なので、
通常の assert(整定後)では問題になりにくいが、アニメ中の assert では留意が必要(diagnosis に state を残す理由)。

## 5.10 DSL: 全アサーションを既定で可視性確認・`requireVisible: false` でオプトアウト(2026-07-22)

occlusion-guard を利用側の既定挙動にし、**`requireVisible` 引数で統一**した(ユーザー決定)。
`visible()` / `present()` は削除(検討の経緯は本節末尾)。

```swift
exist("#msg")                        // 既定: ツリー存在 + 実際に見えているかを FM で確認(見えなければ失敗)
exist("#icon", requireVisible: false) // ツリー存在のみ(見えているかは問わない・高速・アイコン/画像向け)
select("#msg").textIs("完了")                // 既定: 一致 + 見えている
select("#msg").textIs("完了", requireVisible: false)  // 一致のみ
select("#sw").valueIs("1")                   // 同上(既定ガード)
```

- **既定 ON**: exist/textIs/valueIs は、ツリー一致で pass した直後に occlusion-guard を発火
  (足切り→低インク/幾何→FM→visible)。覆われ/切れ/不在なら誤った緑として失敗に反転。
- **`requireVisible: false`**: occlusion 確認を省く(FM を一切呼ばない・ツリー一致のみ)。
- **配線**: `FlowStep.occlusionGuard: Bool?`([Flow.swift](../Sources/FTCore/Flow.swift))を DSL 引数が true/false で
  立て、[StepExecutor.occlusionFlip](../Sources/FTCore/StepExecutor+Assert.swift) が `step.occlusionGuard ?? executor 既定`
  で判定。executor 既定は false のまま(api/record 経路や raw FlowStep=nil は非ガード)。
- **安全策**: FM 未配線(不可)時は guard も素通り(pass)= `requireVisible: false` と同一挙動。足切りで
  非テキスト型(アイコン/画像/絵文字)は自動除外、高インク領域は FM を呼ばない(コスト最小化)。
- **ソース契約**: 逆写像コード生成(既定=引数省略 / オプトアウト=`, requireVisible: false`)・VSCode パラメータ
  編集(`requireVisible` bool・既定 true)を exist/textIs/valueIs に登録。
- **テスト**: 反転/pass/オプトイン(raw exists=nil は FM 不呼出)を StepExecutorTests、各コマンドの
  パラメータ解析・オプトアウトを StepCommandParamsTests で検証(全 green)。

> **命名の経緯**: 当初 `visible()`(別コマンド)→ デバイス上で誤反転が多く不採用。次に `exist`(既定ガード)+`present`
> (ツリーのみの別コマンド)→ テキスト系に対を持てず非対称。最終的に**全コマンド共通の `requireVisible` 引数**に統一。
> `occlusionGuard` は専門用語で不採用、`visible: false` は「非表示を検証」と誤読されるため不採用。

> **コスト注意**: 既定 ON により各アサーション通過で **スクショ 1 枚**(+疑わしい低インク領域のみ FM ~1.6s)が乗る。
> 可視テキストが多い通常画面は高インクで FM を呼ばないため増分は小さいが、大量に検証を回すシナリオでは
> 可視性が不要な箇所を `requireVisible: false` にするのを推奨。
> 未了: (a)`textIs` 等への guard 付与は未実施。(b)explore 自動生成は guard を明示しない(既定に従う)。

## 5.11 実行時配線の修正 + エンドツーエンドデバイス確認(2026-07-22)

**配線バグ修正(重要)**: 実ランナーの `LazyFMDelegate`([ScenarioRunnerMain.swift](../Sources/FTScenarioRunner/ScenarioRunnerMain.swift))が
新メソッド `verifyElementVisible` を転送しておらず、実シナリオ実行では `exist` のガードが `ReplayDelegate`
既定実装(nil)に落ちて**黙って素通り**していた(=機能が実行時に無効)。転送を追加して実効化。

**E2E デバイス確認**: 実ランナーと同一構成(`StepExecutor` + `FMReplayDelegate`)で、A012ADD8 の
商品詳細画面(sticky「カートに追加」バー裏に「レビュー」= 実オクルージョン)に対し実行:

| 実行 | 結果 | 判定 |
|---|---|---|
| `exist("レビュー")`(既定ガード・覆い) | **failed**「`false positive (occlusion)`: …[covered] 領域が不透明な要素に覆われている」(当時は日本語文言) | ✓ 実オクルージョンを失敗へ反転 |
| `exist("レビュー", requireVisible: false)`(ガード無) | passed | ✓ ツリー存在のみで通過 |
| `exist("在庫あり")`(可視) | passed | ✓ 可視テキストは誤反転せず通過 |

→ **ランタイム経路(StepExecutor.execute → occlusionFlip → FMReplayDelegate.verifyElementVisible →
OcclusionVerifier)がデバイス上で機能することを確認**。これで PoC は 合成 → 実 UI 誤反転修正 → 実 UI TP → DSL →
配線 → E2E まで一通り実証済み。

## 5.12 poll-until-visible: 過渡的オーバーレイでの誤失敗を回避(2026-07-22)

当初は「pass 確定の一点で1回」ガードを実行し、その瞬間に覆われていると即失敗していた。これは
**ローディング表示・スナックバー・遷移アニメ等の過渡的オーバーレイ**で誤失敗を招く(消えるはずの覆いで落ちる)。
`exists`/`textEquals` の poll ループを、要素が見つかっても覆われている間は即失敗せず、既存の
「出現待ち」意味論と同様に **timeout まで可視化を待つ**よう変更([StepExecutor+Assert.swift](../Sources/FTCore/StepExecutor+Assert.swift))。

- 覆い判定は `lastOcclusion` に保持し、可視化されれば pass、timeout まで覆われ続ければ occlusion 失敗を返す。
- コストは足切り+低インクゲートで従来どおり抑制(可視な高インク領域は FM を呼ばず即通過)。覆われ続ける
  場合のみ timeout 窓内で数回 FM(実測 median≈1.6s/回。既定 5s 窓で ~3 回)。
- テスト: 過渡的(covered→visible)は pass、覆われ続けは timeout で occlusion 失敗、を StepExecutorTests で検証。
- デバイス E2E 再確認: sticky バー裏「レビュー」の `exist` は依然 failed(可視化されないため timeout で反転)、
  `requireVisible: false`/可視テキストは pass(無回帰)。

## 5.13 textIs / valueIs も既定 occlusion-guard 化(2026-07-22)

`exist` と揃え、`textIs` / `valueIs` も**既定で可視性確認あり**(一致かつ実際に見えていること)に。
オプトアウトは共通の `requireVisible: false`(§5.10 参照。当初は `exist`=present / テキスト系=引数 と
非対称だったが、最終的に全コマンド共通の `requireVisible` 引数へ統一)。内部の `FlowStep.occlusionGuard`
フィールドは実装名として据え置き。

- 機構は既存の textEquals/valueEquals 経路の occlusionFlip をそのまま使用(poll-until-visible も適用)。
- ソース契約: codegen(オプトアウト時のみ `, requireVisible: false` を付与)・VSCode パラメータ編集
  (`requireVisible` bool・既定 true)を textIs/valueIs に登録。FTElement チェーン(`.textIs`/`.valueIs`)も対応。
- テスト: textEquals 経路のガード反転(StepExecutorTests)、textIs/valueIs のパラメータ解析・オプトアウト
  (StepCommandParamsTests)を検証(全 green)。

これで存在系(`exist`)・一致系(`textIs`/`valueIs`)とも既定で「見えていること」を確認する。
オプトアウトは全コマンド共通の `requireVisible: false`(`present` は削除済み。§5.10 の命名の経緯を参照)。

## 5.14 スクショ再利用: 操作を挟まない連続ガードで往復(~125ms)を回避(2026-07-22)

実測(§crop 検討)で、ガード発火のたびに全画面スクショ往復 **~125ms** を払っていた。**操作を挟まない
連続ガード**(`exist` を並べる等、同一静止画面)では 1 枚を使い回して往復を省く。

- StepExecutor に `cachedScreenshot` を持ち、occlusionFlip の取得を `guardScreenshot()` 経由に。
- **無効化**(古いスクショの再利用防止): ① action(tap/type/swipe 等= executeAction 冒頭)
  ② performCustom(launch/wait/procedure = executor をバイパスするため [FTRuntime](../Sources/FTDSL/FTRuntime.swift) から
  `invalidateScreenshotCache()` を呼ぶ)③ poll 待機ごと(poll-until-visible で画面が変わり得る)
  ④ **200ms TTL**(静止画面前提の staleness 上限)。
- 効果: 連続する N 個の可視ガードが **スクショ 1 回**で済む(従来 N 回)。実測往復 ~125ms/回なので
  例: 連続 5 ガードで ~500ms 削減。FM が絡む場合は FM 側が支配的なので効果は相対的に小。
- テスト: 連続ガードでスクショ 1 回に集約 / 操作を挟むと取り直す、を StepExecutorTests で検証(count で確認)。
  デバイス E2E 無回帰(覆い→failed / 可視→passed)。poll-until-visible とも両立(待機ごとに取り直すため覆いは再判定)。

## 5.15 レビュー反映(別セッションのレビュー・2026-07-22)

- **#1 座標系の食い違い(要修正・中)**: textEquals/valueEquals がフォールバックドライバ(SystemUI/springboard)
  由来の要素に一致した場合、その frame/screen は primary と別座標系なのに occlusionFlip へ primary の
  snapshot/screenshot を渡していた(FM に別アプリのスクショ+システム要素を渡し誤反転しうる)。
  → **fsnap 由来一致はガードをスキップ**(exist の fsnap 経路と同契約)。`fromFallbackDriver` で判定。
- **#2 結合 `, ` 規則の過剰適用(低〜中)**: `select("#x").textIs("Hello, World")` のような正当な句読点入り期待値が
  結合セマンティクス扱いで黙って素通りしていた。→ eligibility に `isUserText` を追加し、**ユーザー期待値
  (textEquals/valueEquals)には `, ` 規則を当てない**(型・絵文字の規則は維持)。
- **#5 stale occlusion(軽微)**: 覆い観測後にテキストが不一致へ変化して timeout すると、実態(不一致)を
  隠して古い occlusion 失敗を返していた。→ **`actual != expected` を観測したら `lastOcclusion` をクリア**。
- **#3 既定 ON の是非(設計判断)**: exist/textIs/valueIs の既定 ON は全既存シナリオの検証意味論を変える。
  三段ゲート+スクショ再利用で実コストは抑制済み。**ユーザー決定で「既定 ON のまま」本流方針とする**
  (2026-07-22 確定)。オプトアウトは `requireVisible: false`。
- **#4 TTL 200ms / 良い点**: 指摘どおり論理健全・コメントに根拠あり(変更不要)。

テスト追加: #1 fsnap 一致でガード不呼出 / #2 eligibility の isUserText / #5 不一致時に occlusion を返さない
(StepExecutorTests・全 green)。

## 5.16 本番実行でのコスト実測(2026-07-22・M2 Ultra)

PoC の懸念は「常時 ON は非現実的」(§4 の速度計測)だったが、**ゲートが効いて実運用の発火は少ない**。
SampleApp 76シナリオを `ios-5-android-5`(10台並列)で実行した実測:

- FM 発火は **8シナリオ / 14回・計53.9秒**(うち occlusion 10回33.5秒、heal 4回20.4秒)
- iOS レーン稼働時間 846秒に対し **6.4%**。律速にはなっていない
- 発火は**失敗シナリオと長時間シナリオに偏る**(heal は失敗時のみ走るため)
- occlusion 1回あたり p50 2,799ms / max 3,294ms(10台並列時。逐次なら約1,200ms)

**ただし FM はホスト全体で共有される資源**(許可枠(既定5)で制限される。当時〔2026-07-22〕は
枠1本で運用しておりスループットは並列度によらず約1回/秒に見えたが、2026-09-01 の実測で
これは FM 自体の天井ではなく枠数の帰結だったと判明した)。実行時間には「FM 呼び出し総数と枠」
から来る下限ができ、**デバイスを増やしても縮まない**(枠はホスト単位のため)。詳細な実測値と、
発火が増えたときの絞り方は [performance-tuning.md §3.5 / §4.2](performance-tuning.md)。

計測は `FMHealth`(Sources/FTCore/FMHealth.swift)が用途別に計上し、結果 JSON の `fm` と
モニターの FM グラフに出る。**ANE 負荷率では測れない**(電源状態の1ビットしか返さないため廃止済み。
performance-tuning.md §6)。

## 5.17 Tier-2: Vision OCR を FM の手前に置く(2026-09-07)

FM(画像添付)は crop 1 枚に p50 1.3〜1.9s(並列時 2.8s)を払い、ホスト単位で直列化される
(FMLock)。**同じ crop を Vision の `VNRecognizeTextRequest` で読むだけなら p50 33ms**なので、
「期待テキストが丸ごと読めた回は FM を呼ばずに可視として通す」段を FM の直前に足した
([RegionText.swift](../Sources/FTCore/RegionText.swift))。crop 矩形は
[OcclusionCrop.rect](../Sources/FTCore/OcclusionCrop.swift)(FM から移設)を共有するので、
**両者は同じ画素を見る**。

### 判定規則(誤った赤を作らない側に倒す)

- 一致は**完全含有だけ**。正規化(NFKC → 空白除去 → 小文字化 → 末尾の `…`/`...` 除去)した
  期待文字列が、読めた行の連結または各行に**丸ごと**含まれるときだけ可視と見なす。
  **先頭一致は採らない** —— 部分的に覆われて残りだけ読めた回を通すと、guard の目的である
  誤った緑を作る。
- **ASCII の期待文字列は語境界も要求する**(2026-09-07 のレビュー指摘)。素の部分一致だと
  `exist("OK")` が覆いの「Cookieの設定」に当たって素通りする —— 短い期待値ほど当たりやすい。
  日本語には語境界が無いので CJK を含む期待値は素の含有のまま。**残る取りこぼし**は
  折り返しの連結がたまたま期待値を作る形(`["row_4", "0 件"]`)で、折り返しの連結は正当な用途
  なので消さない(誤る向きは見逃しだけ)。
- confidence は使わない(Vision の confidence は 0.3/0.5/1.0 に飛び飛びで、根拠のある閾値を
  置けない)。読めた文字列そのもので判定する。
- 読めなかった回は**今までどおり FM が最終判定**する。OCR は FM を置き換えない。

### 実測1: FM が反転した crop 167 枚(`~/Library/Logs/fleetest/occlusion/`)

| | FM | OCR |
|---|---|---|
| 「期待テキストを読めない」 | 167/167(定義) | **167/167 一致** |
| 1 枚あたり | 選別段 p50 1,257ms | **p50 33ms / p90 68ms** |

**FM が反転した側では両者の結論が完全に一致する**。

### 実測2: FM の段に到達した crop 163 枚(measure モード)

`FT_OCCLUSION_OCR=measure` は OCR を撃つが素通りさせず、crop と両者の判定を
`~/Library/Logs/fleetest/occlusion-corpus/` へ書く。この run では手元の FM が死んでいたため
(`fmVisible: null`)、正解は crop の目視で取った。

E2E-iOS で 77 枚(34 本・34/34 緑)、日本語ラベルを持つ E2E-CMP `ios-fm` で 85 枚(36 本・36/36 緑)、
**計 163 枚**。

日本語モデルを常に載せた最初の版では、等倍で読めたのは **116/163(71%)**。読めなかった 47 枚の
大半は **1 文字の誤読**(`swipe=down`→`swipe=aown` / `selected`→`selectea` / `zoom=in`→`ZoomEin`)で、
**本当に描かれていない**のは 2 枚だけ(`画面外テキスト` = 下端で切れて OCR は空を返す)。
この誤読は次節の言語規則で消える(97% が等倍で読める)。

- **OCR 単独(FM の置き換え)は不可**: 誤読した回を「見えていない」と読むと誤った赤になる。
  §5.7 の「素の FM は約50%誤反転」と同型の失敗で、向きだけが違う。**読めなかったことは
  反転の根拠にしない**(判定は必ず FM)。
- 到達した crop は 162/163 が Tier-0 幾何の疑い由来で、インク由来(sd < 12)は 1 枚。
  インク由来の母集団は実測1(167 枚)がそのまま代表する。
- 日本語ラベルはこの母集団に 5 枚しか居らず、うち 3 枚が読めた。**誤読は ASCII の `d`/`a`・`0`/`o`
  に集中**していて、CJK 固有の弱点は出ていない。

### 読ませる言語は期待文字列から決める —— 71% → 97%・p50 208ms → 92ms

等倍での誤読(`swipe=down`→`swipe=aown` / `selected`→`selectea` / `zoom=in`→`ZoomEin`)は
**日本語モデルを載せていたせい**だった。期待文字列が ASCII だけのときは英語だけを載せる
(`RegionText.languages(for:)`)。同じ 163 枚で:

| 言語 | 読めた | 日本語ラベル | はしご込みの所要 |
|---|---|---|---|
| ja-JP + en-US(固定) | 158/163(97%・うち 42 枚は拡大で救出) | 3/5 | p50 208ms |
| en-US のみ(固定) | 155/163(95%) | **0/5** | p50 91ms |
| **期待文字列で決める(採用)** | **158/163(97%)・全部が等倍** | 3/5 | **p50 92ms** |

- **日本語モデルは所要の 6 割**を占める(91ms → 208ms)。ASCII の期待文字列では読み取り結果が
  英語のみと完全に一致するので、載せる意味がない。
- 残る 5 枚は WebView の 3 枚(`WebView` / `wv_result=link`)と `画面外テキスト` 2 枚。
  後者は**本当に描かれていない** = FM に回るのが正しい。

### 拡大の段(はしご)は安全網として残す

読めなければ crop を拡大して読み直す(`upscaleLadder = [1, 2, 3]`)。言語規則を入れた後の
163 枚では**一度も段が上がらない**が、日本語モデルを載せた場合の実測では ×1 71% → ×2 88% →
×3 97% と効き、**×4 では 74% に落ちる**(だから上げ続けない)。固定コーパスがこの両方を等号で
固定している(`RegionTextCorpusTests`)。

- **1行も読めない crop は段を上げない**(実測: ×1 で無読の 3 枚は ×2/×3 でも 1 枚も読めない)。
  効くのは**覆いが消えるのを待つ poll 周回** —— 毎周 3 回撃つと待っている間じゅう払い続ける。
- **合成画像では拡大の効きが再現しない**(6pt の合成文字は ×2 で悪化した)。回帰は実 crop の
  固定コーパス(`Tests/Fixtures/OcclusionCrops/` の 3 枚 + manifest)で、**言語規則の効き目・
  段ごとの読み取り・撃つ回数**を等号で固定する。この 3 枚は認識レベルを fast に落とす・
  日本語の集合で言語補正を切る(§5.18)・拡大を止める・言語規則を壊す、のいずれの変異も落とす。

### 使う API と版

**`RecognizeTextRequest`(Vision の Swift API)** を使う(ユーザー決定 2026-09-07)。
**版(revision)は指定しない** —— OS が既定に選んだものに従う(固定すると新しい OS で改善された
認識器を使えない)。版が動いたときの検出は固定コーパスが担う。

旧 `VNRecognizeTextRequest` との比較(同じ 163 枚・1プロセス1条件・暖機後・等倍のみ):

| | 読めた | p50 | 平均 |
|---|---|---|---|
| 旧 API + ja/en 固定 | 116/163(71%) | **36ms** | 42ms |
| 新 API + ja/en 固定 | 116/163(71%) | 206ms | 217ms |
| 旧 API + 言語規則 | 158/163(97%) | **29ms** | 36ms |
| **新 API + 言語規則(採用)** | 158/163(97%) | 94ms | 107ms |

- **読み取り結果は同じ**。枚数が一致するだけでなく、20 枚の逐一比較で読めた文字列の不一致 0 件。
  モデルも同じで、新 API の `supportedRevisions` は **revision3 の1つだけ**(旧 API の current も 3)。
- **新 API は 1 回あたり 3.2 倍遅い**(言語規則下で 94ms vs 29ms)。呼び出し形を
  `ImageRequestHandler` 経由に変えても 92ms で動かないので、**差は Swift ラッパの固定費**
  (画像変換かセッション設定)で、こちら側で削れない。
- **実運用の影響は +65ms × 発火回数**。E2E-iOS 34 本で 77 回 = **+5 秒/run** で、FM を 2〜4 回
  省ければ帳消しになる(実際 75/77 が OCR で片付いた)。**設計判断は変わらない**。
- **旧 API へ戻す再検討条件**: 1 run で OCR が数百回撃たれるようになったとき(`requireVisible` が
  高頻度に発火する受け手のシナリオ)。それまでは、将来の OS が新しい revision を出したときに
  自動で乗ること・旧 API がいずれ非推奨になることを採る。

### 暖機(2026-09-10 に正体を特定して作り直した)

「モデルがメモリに載っていない」ではなかった。プロセスで最初の 1 回だけ 20〜45 秒かかるのは
**Espresso(認識器の実体)の AOT コンパイル**で、キャッシュは
`~/Library/Caches/<プロセス名>/com.apple.e5rt.e5bundlecache` に**プロセス名とバイナリの素性で**鍵付けされ、
**コンパイルがそのプロセスの生存中に終わったときだけ**コミットされる(途中で死ぬと `.tmp.<pid>` が残るだけ)。
1 シナリオ = 1 プロセスの実行バイナリは 20〜60 秒で終わるのでほぼコミットできず
(E2E-CMP で完了 1 / 放置 53)、**全プロセスが毎回ゼロから払っていた**。xctest ホストや素の CLI では
2 回目が 160ms なので、テストや単発の計測では**見えない**(3 秒時点のスタックをプロセス自身に採らせて
`MILCompilerForE5` → `AneValidator` … を確認)。この初回がステップの締め切り(120 秒)を丸ごと食い、
フリート負荷下では 36〜108 秒まで伸びて全レーン・全機械が同時刻に固まって見えていた。

置き場は3つ: **① run の開始時に同じプロセス名の待てる子**(`fleetest-scenarios-<project> warm-ocr`)を
背景で 1 本起こしてコミットさせる(`ScenarioHost.listForRun`。dry-run / MCP / codegen では起こさない・
機械で同時に 1 本 = `OCRWarmupLock`・親の死を生き延びる)/ **② シナリオ側の探りは同じロックを待ってから**
読む(`RegionText.prewarmIfNeeded`。ガードが既定で効く executor の生成時に始める)/ **③ 探りが 1 行以上
読めるまで近道は撃たない**(`isWarm`。判定は FM が担うので損は無い)。読ませる言語集合は
`languages(for:)` が返しうる 2 つ(モデルが別で、別々にコンパイルされる)。

実測: シナリオ側の暖機 22,980ms → 218ms、実 crop の初回読み 21,830ms → 98〜122ms、
E2E(local)でガード 54 件中 43 件が近道・予算切れ 0・guardMs 最大 456ms(修正前 107,720ms)。
保守者はコミットのたびに SUT ごと 1 回払い直す(背景・供給と並行)。受け手は導入ごとに 1 回。

### run 全体に占める割合

同じ E2E-iOS 34 本(1台・measure モード)で、結果 JSON の `ocrMs` を合計した実測:

| 条件 | OCR 合計 | run に対する割合 |
|---|---|---|
| 旧 API・等倍・ja/en | 31.3s(うち初回ロード 25.2s) | 7.8% → **除くと 1.5%**(6.1s / 399.6s) |
| 新 API・はしご・ja/en | 22.4s(モデル常駐で初回ロード無し) | **4.8%**(22.4s / 464.5s) |
| **新 API・言語規則(現行)** | 55.5s(うち初回ロード 47.0s) | 12.9% → **除くと 2.0%**(8.5s / 429.3s) |

- **初回ロードは API に依らない共通の固定費**なので、API の比較は必ずこれを除いた定常値で見る
  (入るか入らないかで 2% と 13% の間を動く)。
- 定常値では **旧 1.5% → 新 2.0%**(この run で +2.4 秒)。同じ API 内では**言語規則のほうが
  効きが大きい**(4.8% → 2.0%)。
- 比較対象の FM は、この run で OCR が片付けた 75 枚を訊いていたら 2.3s/回 × 75 = **約 170 秒
  ≒ run の 40%** だった。**2% と 40%** の差がこのゲートの意味。

### 既定と殺しスイッチ

**利用者の口は実行プロファイルの `ocrTextVisualCheck`**(既定 true。拡張のプロファイルタブ
「Advanced Features(Experimental)」セクションの「OCRを使用したテキストの視覚検証を有効にする」)。
他のキーを経由せずこの値がそのまま効く。`false` にすると `--no-occlusion-ocr` がランナーへ渡り、
**環境変数より優先**して OCR を止める(従来どおり FM だけで判定)。`textVisualCheck` が false の
run では guard 自体が走らないのでこの値は効かない。

環境変数は保守者用の口:
`FT_OCCLUSION_OCR`: 未設定/その他 = **on(既定)** / `0`・`off` = 殺しスイッチ(従来どおり FM だけ)/
`measure` = 訊くが必ず FM にも回しコーパスを書く(採取用。**書き出しは measure のときだけ** ——
既定で書くとガードのたびに PNG を1枚落として run を遅くする)。

**残るリスクは見逃し(誤った緑)の側だけ** —— 覆いが不透明なら画素が消えるので OCR も読めないが、
「読めたのに実は覆われていた」回が無いことは 330 枚(167 + 163)の範囲でしか確かめていない。
誤った赤は構造上増えない(読めなかった回の判定は必ず FM)。


## 5.18 FM には期待文字列を渡さない —— 転写 + ホスト照合(2026-09-15)

日本語 UI での精度確認(iOS 標準アプリ 20 画面を ja/en で撮り、実要素に覆い・空白・別の文字を合成。
正解付き ja 70 / en 80 要素 × 5 変種 + 実 run の crop 158 枚)で、FM 段の**おうむ返し**が見つかった:
期待文字列を prompt に入れて「見えるか」を訊く形は、**空白や別の文字の crop でも observedText に
期待文字列をそのまま写して visible=true** と答える。同じ crop を OCR は正しく読めていた(画像が
読めないのではなく、手本を写している)。

| 見えない形 | 旧方式の見逃し ja / en |
|---|---|
| 空白 | 23% / 20% |
| 別の文字 | 56% / 33% |
| 左 65% の覆い(末尾だけ見える) | 84% / 81% |
| 全面の覆い | 3% / 5% |

欄順を「転写 → 可否」に替えるだけでは直らない(別の文字の見逃しがむしろ 100%)。**期待文字列を
FM に見せない**ことが本質だった。

### 方式

1. FM への問いは定数 `What text is drawn in this image?` と転写 1 欄(`DrawnTextTranscript`)だけ。
   prompt にも instructions にも期待文字列を入れない(`OcclusionTranscriptTests` がソース走査で守る)
2. 可否は `FTCore.TranscriptMatch`(純粋関数)が期待文字列と突き合わせて決める: 含む / 先頭 2 文字以上の
   切り詰め / 5 文字以上なら len÷5 文字の誤読(開始自由の編集距離。**期待の先頭文字が転写に在るときだけ** ——
   「About」→「bout」を誤読で通さない)/ 末尾だけ = covered / 空 = notRendered / 無関係 = textMismatch
3. 等倍で読めなければ 2 倍に拡大して読み直す(`RegionText.enlarged` を共有。拡大は文字を作らないので
   覆い・空白は空のまま = 見逃しは増えない)
4. 転写が期待文字列に**惜しい**(1 文字差)ときだけ、同じ crop を OCR にも読ませ、**丸ごと読めたときだけ**
   見える側へ倒す(`nearMissOCRBudget` = 60 秒。Vision の初回コンパイル 25〜47 秒を待ち切る。
   align 直後の最初のシナリオで実際に踏んだ)。読めなかったことは反転の根拠にしない

### 結果(同じ 922 crop に本番の検証器を当てた)

| | 誤った赤 | 見逃し |
|---|---|---|
| ja 合成(70 要素 × 5 変種) | 1/70(旧字体「擴張」) | 2/280(合成の余白への写り込み) |
| en 合成(80 要素 × 5 変種) | 0/80 | 1/328(同上) |
| 実 run の crop(158 枚) | 0/153 | 0/5 |

空白・別の文字・全面の覆いの見逃しは **0/152** ずつ。所要 p50 は見える回 0.89s(旧 1 段目 1.1s)・
反転する回 1.75s(拡大の読み直しぶん)。照合の閾値(÷5・先頭文字)は候補 8 規則を同じ転写に
オフラインで当てて選んだ(÷6 は「ユーザ辞書→辞典」が誤った赤、÷4 は別の文字を 1 件見逃す)。
2 段化(`VisibilityScreening` / `FT_FM_OCCLUSION_TWO_STAGE`)は撤去。

### 露見した 2 つ(方式の誤りではない)

- **探索側の穴**: E2E-iOS の `#txt_offscreen` はタブバーの裏に 2pt しか描かれておらず、旧 FM は
  おうむ返しで通していた(真陽性)。`clippingContainer` の深さ規則が「可視域に兄弟 1 つ」で nil →
  viewport が画面全体 → 容器の下端を 18pt はみ出した要素を found と判定していた。nil のときは
  申告 scrollable の祖先へ倒す形に直した(docs/maintainer-notes.md §19)
- **字形**: en ロケールのシミュレータは「単」(U+5358)を中国語フォントの字形で描き、補正なしの Vision も
  FM も「单」(U+5355)と読む。言語の選択(`languages(for:)` = ja+en)は既にあったが
  `usesLanguageCorrection` を切っていたのが効いていた。**日本語モデルを載せる集合でだけ補正を有効**にする
  (`RegionText.usesLanguageCorrection(for:)`。補正は Vision の設定で FM には無い)。合成 ja 79 要素で
  補正ありは見えている要素の読みが 76→78、覆い・空白・別の文字で読めた回は 0 のまま = 覆われた語を
  補完しない。ASCII は従来どおり切る(欠けを推測で埋めさせない)

計測ハーネス(捕獲・合成・方式の差し替え)はリポジトリに残していない(セッションの scratchpad)。
再計測するなら: XCUITest ブリッジで画面を撮り(snapshot + screenshot)、`OcclusionEligibility` を通る
要素の frame に変種を合成し、`OcclusionCrop.rect` で切った crop を `OcclusionVerifier.verifyCropped`
(frame = 画像全体)に当てる。

## 5.19 FM が判定を返さないときの OCR だけの判定と、先頭だけ描かれた形(2026-09-26)

それまで FM が判定を返さないとき(macOS 26・実呼び出しの失敗・ブレーカ開)はガードが素通りし、
OCR の有無で合否は変わらなかった(OCR は「見える」側にしか倒さないため)。ユーザー要望で、
**FM が使えないときは OCR だけでも「見えない」を判定する**ことにした。あわせて、先頭だけ描かれた形の
扱いをユーザー決定で3つに分けた。

### 方式

- **判定は `TranscriptMatch.judge` の1か所**(FM の転写と OCR の読みの両方に当てる)。値の正しさは木が
  保証するので、ガードが見るのは「期待に近い文字がその位置に描かれているか」だけ —— 1文字程度の誤読は
  許す(値だけ違う `tap=0` と `tap=3` も緑)
- **先頭だけ描かれた形**: 末尾に省略記号 → 緑 + `text-ellipsized`(アプリの意図した省略)/ 読めた割合 >
  `mostlyHiddenRatio`(0.5)→ 緑 + `text-partially-hidden` / 以下 → 赤(`mostlyHidden`)
- **OCR だけの判定**(`OCROnlyVisibility`): 近道が既に読んだ行を `judge` に通す。行が無ければ領域のインク量
  (`occlusionInkThreshold` = 12)で「描かれていない / 判定不能(何か描かれているが読めない)」を分ける。
  **追加の OCR は撃たない**。不可視なら FM の反転と同じく赤(警告から始め、下の確認を経て上げた)
- 陽性対照の注入口: `FT_FAKE_FM_NO_VERDICT=1`(FM を呼ばずに「答えが無い」扱いにする)

### コーパス

| 種類 | 件数 | 正解の付け方 |
|---|---|---|
| 実 crop(E2E-iOS / E2E-CMP の measure 採取・§5.17 と同じ) | 163 | 目視(5 件は撮った瞬間に期待の文字が出ていなかった: WebView の空白・DOM が絵より先に進んだ `wv_result=link`・画面外テキスト) |
| **実画面の日本語**(ja ロケールの Simulator・iOS 27 の設定とリマインダー 34 画面) | 134(うち日本語 119) | 型の足切りを通り・画面内に収まり・全画面の容器を除いて他と重ならない要素。赤の候補は1件ずつ目視(2 件は本当に見えていなかった: 浮いた検索バーの下・木と絵のずれ) |
| FM が反転した実 crop(`~/Library/Logs/fleetest/occlusion/`) | 39 | 目視で全件が見えていない(空白・図形・別の行) |
| 合成(見えている実 crop から) | 空白・全面の覆い・左 65% / 右 35% の覆い・半透明 50% / 80%・別の文字 × 各 295 | 変種の定義どおり |
| 合成の省略表示(期待の先頭 30/60/80% + `…` をヒラギノ角ゴ W3・SF で描画) | 1386 | 見えている(意図した省略) |

**iOS 27 の設定アプリは木の末尾に全画面の `toolbar` を持つ**ため、幾何の疑い(`OcclusionSuspicion`)では
ほぼ全要素が「覆われている疑い」になり、OCR/FM の段へ回る(本番でもそう動く = OCR の母集団として本物)。

### 結果(本番の Swift 実装で集計)

| 標本 | 緑 | 赤 | 判定不能(素通り) |
|---|---|---|---|
| **見えている実 crop 290(日本語 119)** | 289 | **0** | 1(`0` 1文字・高インク) |
| 見えていない実 crop 7 | 0 | 5 | 2(画面外テキスト・高インク) |
| FM が反転した実 crop 39 | 0 | 36 | 3(紫の図形・高インク) |
| 空白 / 全面の覆い | 0 / 0 | 295 / 295 | 0 / 0 |
| 左 65% の覆い | 4 | 241 | 50 |
| 右 35% の覆い | 229(うち `text-partially-hidden` 192) | 57 | 9 |
| 別の文字 | 35(全件が1文字違いの値) | 247 | 11 |
| 半透明 50% / 80% | 288 / 288 | 6 / 6 | 1 / 1 |

- **判定不能は全件「見逃す側」**(読めない × インクが多い = 図形・WebView・短すぎる残り)
- 半透明の覆いは文字が透けて読めるので緑(FM も同じ扱い)
- インク量の閾値は 6〜20 のどこでも結果が1件も変わらない(空白は 0 付近・文字は 40 以上)

**省略表示**(合成 1386): 先頭が 5 文字以上なら **682/684 が緑**。1〜4 文字だと 702 件中 62 件(9%)が赤・7 件が判定不能
—— 短い先頭の誤読(`フ…` → `7・・・`)と、Vision が短い文字列を 180° 逆向きに読む形(`zoo…` → `...OOZ`)。
**実画面の省略表示は1枚も採れていない**(自前 SUT に省略表示の画面が無い)。

### コーパスで見つけて直した規則(いずれも FM の経路にも効く)

| 見つかった誤り | 件数 | 直し方 |
|---|---|---|
| 期待(木)は全角の `“…”`、読みは半角の `"…"`(NFKC で揃わない) | 見えている実 crop 3 件が赤 | `RegionText.normalize` で全角引用符を半角へ畳む |
| 同じ長さで先頭1文字だけ誤読(`iCloud` → `¡Cloud`)が「先頭の文字が読みに在る」条件で弾かれる | 1 件が赤 | この条件は左から覆われて**短くなった**読みを防ぐためのものなので、読みが期待と同じ長さ以上なら課さない |
| ヒラギノの `…` は点が字の中央の高さに並ぶ字形で、OCR は `•••`・`・・・`・`⋯・`・`：・・` と読む | 省略表示の ja の約 9 割が「別の文字」で赤 | 点の類(`. • ・ · * : ⋯`)が2つ以上続くか `⋯` を含む末尾を省略記号と見る(`RegionText.ellipsisTailLength`)。1つだけの `.` `:` と `。` は除く |
| 長い省略で1文字を読み落とす(`全体的な設定や自…` → `全体な設定や自…`) | 省略表示の数% | **省略記号があるときだけ**先頭一致に誤読(長さ ÷5)を許す。無いときに許すと値だけ違う読み(`tap=3`)が「一部が隠れている」に化けた(別の文字 30 件で確認) |

### 警告から赤へ(デバイス実行)

FM を答えなかったことにする注入(`FT_FAKE_FM_NO_VERDICT=1`)を付けたフル E2E(環境変数はリモートへ
転送されないので、検証になるのは手元のレーンだけ・全プロファイルに手元のレーンがある):

| 段階 | 結果 |
|---|---|
| 警告のみ | 8/8 緑。ただし `ocr-only-would-flip` が 71 件(うち 65 件が手元の Android Emulator 2 台) |
| うち 6 件 | **実装の抜け**: OCR を読んでいない(暖機前)のにインク量だけで判定していた → 読みが無ければ判定不能に直した |
| 残りの crop(計測モードで採取) | 空白・一様な灰色の板・別の行 —— **撮った絵に期待の文字が写っていなかった**。見えている文字の誤読は 0(同じ run の 142 件は全部正しく見える) |
| 赤にした版・配信あり | 7/8 緑。赤 1 本 = スクショに**前のシナリオの画面が写ったまま**(木は正しい画面)。以前の「灰色の板」もこの画面の地図領域 |
| 赤にした版・**モニターの配信を pause** | Android 4/4 緑・OCR だけの判定の赤 0(手元の FM 呼び出し 0・OCR まで届いたステップ 61 件がすべて通過) |

**赤の原因はモニターの配信が張られた Android Emulator のスクショの古さ**で、FM の経路でも同じ絵を見て
同じく赤になる(OCR 固有ではない)。コーパスの誤った赤 0 と合わせて、警告から赤へ上げた。

### 残る限界

- 判定不能(読めない × インクが多い)は確かめずに通る = 見逃す側
- 短い省略表示(先頭 1〜4 文字)は約 9% が赤になる(合成)。実画面の省略表示では未測定
- FM の経路で `text-partially-hidden` が誤って立つ率(FM が見えている文字の末尾を落として転写する率)は、
  FM の転写付きのコーパスが手元に無く未測定

計測ハーネスはリポジトリに残していない(セッションの scratchpad)。再計測するなら: 実画面は XCUITest
ブリッジで snapshot + screenshot を撮り、`OcclusionEligibility` を通る要素を `OcclusionCrop.rect`(余白 24)で
切って `RegionText.read`(言語は `languages(for:)`・拡大の段ごと)で読み、`TranscriptMatch.judge` と
`OCROnlyVisibility.judge` に当てる。

## 5.20 文字以外の要素に FM を当てる案(不採用・2026-09-26)

「FM を使用した UI の視覚検証」として、文字以外の要素(アイコン・画像・ボタン・スイッチ)にも
可視判定を広げる案を PoC で測り、**新設しないと決めた**(ユーザー決定)。

- 標本: iOS 27 の設定アプリの実画面から、文字以外の要素 49 個(ボタン 37・画像 11・スイッチ 1)を
  要素の枠そのもの(余白 0)で切り出し、空白・全面の覆い・左 65% の覆い・別の絵・半透明の覆いを合成
- FM への問い: 期待は渡さず(おうむ返しを避けるため)、短い説明と `drawn / partlyCovered / blankOrPlain` の分類だけ。
  M1Max のオンデバイスモデル、p50 2.7 秒

| 標本 | FM | インク量(< 12 を空白) |
|---|---|---|
| 見えている実物 49 | 49 すべて drawn | 48 が描かれている・**1 件を空白と誤判定**(薄い色のアイコン・ink 11.7) |
| 空白 49 | 36 だけ blankOrPlain(13 件は何も無いのに「横線」と答えた) | 49 すべて空白 |
| 全面の覆い 49 | 48/48(1 件は FM の失敗) | 49 すべて空白 |
| 左 65% の覆い 49 | 4/47 だけ partlyCovered | 判定できない |
| 別の絵 49 | 0/49 | 判定できない |

FM が拾えるのは空白と全面の覆いだけで、どちらもインク量のほうが確実だった。文字の検証で FM が効くのは
転写を木のラベルと突き合わせられるからで、アイコンには突き合わせる相手が無い。再検討するなら、
見た目の正解(見本・前回の見た目)を持つ形に限る。

## 5.21 OCR で不可視と言い切れた回は FM を呼ばない(2026-09-26)

ユーザー決定: **FM の有無に関わらず**、OCR の読み(読めなければインク量)だけで `OCROnlyVisibility` が
「見えていない」と言い切れた回は FM に回さず赤にする。役割は「a11y の木が在ることを保証し、見えているかを
安い OCR で決め、OCR が言い切れない形だけ FM で精度を足す」。§5.17 の「読めなかった回の判定は必ず FM」は
「**丸ごとは読めなかった**だけを反転の根拠にしない」に改める(1文字誤読の問題は言語規則で解消済み・
判定は §5.19 の `TranscriptMatch.judge` を通る)。

- 根拠は §5.19 のコーパス: 見えている実 crop 290 件(日本語 119)で OCR だけの判定の誤った赤 0。
  FM が生きている構成でこの判定が赤にするのは、FM が使えないときに赤にしていたのと同じ形だけ
- FM へ上げるのは **判定不能**(読めない × インクが多い = 図形・WebView・一部だけ残った文字。
  OCR が判定不能にした見えない標本 72 件を FM は 72/72 赤・見えている 31 件は全部緑)と、
  **見えるが丸ごとは読めなかった**回(1 文字誤読・先頭だけ読めた形)
- `measure` では短絡しない(FM と並べて採取するのが目的)。OCR が off・暖機前で読んでいない回は
  judge が判定不能を返すので従来どおり FM
- 失敗文言は `judged by OCR`(FM が使えないときは従来どおり `judged by OCR alone because FM gave no verdict`)。
  FM に訊いていないので `visibility-guard-skipped` は立てない
- 配線は `OCROnlyVisibilityWiringTests.testOCRNotVisibleShortCircuitsBeforeFM` が FM より前にあることを固定
  (変異 2 件 = measure に限定・skipped を立てる、を検出)

## 6. 既知の限界

1. **合成フィクスチャでの計測**。実アプリのスクショ(半透明シート、影、アンチエイリアス、動的コンテンツ)は
   未検証。次段で sut-ec-mobile の実オーバーレイ画面で再計測すべき。→ §5.7〜5.9 の実 UI 計測で解消済み。
2. **Compose iOS の frame クランプ画面では無力**。クロップ先の frame 自体が嘘([compose-ios-ax-frame-clamp])
   なので、退化 frame は `occlusionFlip` がスキップする実装にした(素通り=従来動作)。この画面群は
   本ガードの対象外と割り切る。
3. **判定不能時(FM 不可・画像不正)は素通り**。ガードは「積極的に誤った緑を潰す」だけで、可用性は落とさない。
4. **coveredPartial の閾値は主観**。65% 覆いを「見えない」と定義。どこまでを許容するかは要件次第。
5. **poll 挙動**。現状は「ツリー一致の一点で1回照合し、不可視なら即失敗」。過渡的オーバーレイ
   (スピナー等)を待ちたいなら「可視になるまで poll、timeout で失敗」に変える余地(FM コール増)。
   → §5.12 の poll-until-visible で解消済み。

## 7. 推奨(実 UI 計測を踏まえ改訂)

実 UI で **unconditional FM は約50%誤反転**する事実が判明したため、当初の楽観的推奨(cropped を広く採用)
を**取り下げる**。改訂版:

1. **unconditional FM は不可**。全 assert 通過で FM を呼ぶ運用は実 UI で誤反転が多すぎる。
2. **FM は必ずゲート越し**。`occlusionInkThreshold`(既定 12)未満の低インク領域だけ FM に回す。
   これで実運用の誤反転をほぼ 0 にできる(デバイス上で確認)。当初はここで既定 OFF・限定 ON を
   推奨したが、三段ゲート+スクショ再利用でコストが抑えられたため**既定 ON が確定**
   (2026-07-22 ユーザー決定・§5.15 #3。オプトアウトは `requireVisible: false`)。
3. **FM に回す前に要素種別・省略で足切りする**(次段の必須タスク):
   - 対象は「label が verbatim 描画されるテキスト要素」に限定。**アイコン/画像/結合セマンティクス
     (label に区切りの `, ` を含む合成ノード)/絵文字単体は除外**。
   - **省略対策**: 期待テキストが末尾省略され得る場合、完全一致でなく「先頭一致/省略記号許容」で
     判定するか、そもそも FM 判定から外す。
4. **本採用の前提**: 上記 3 の足切り実装 → 再計測 → 実オーバーレイ(モーダル/シート/キーボード)での
   true-positive 確認(今回キーボードが上がらず未取得)。DSL 露出(`assertVisible` 等)はその後。
5. state 分類は判定に使わない(diagnosis ログのみ)。判定に使うのは `visible` bool と 2 段ゲートのみ。

**結論(2026-07-22 更新)**: 「疑い時だけ FM」ゲート + **要素足切り(非テキスト型/結合/絵文字を除外)**
+ **省略許容プロンプト**の3点を揃えれば、実 UI で有害誤反転 0%・レイテンシ median≈1.6s を達成できた。
これらは全て実装・結線済み([OcclusionEligibility](../Sources/FTCore/OcclusionEligibility.swift)・
[RegionInk](../Sources/FTCore/RegionInk.swift)・[OcclusionSuspicion](../Sources/FTCore/OcclusionSuspicion.swift)・
[OcclusionVerifier](../Sources/FTFoundationModels/OcclusionVerifier.swift)・StepExecutor.occlusionFlip)。

**デバイス上での true-positive も取得済み(§5.9)**: スナックバー occlusion で「¥24,000」が visible→covered に
正しく反転、sticky バー裏の「レビュー」も安定検知。実 UI で **誤反転 0 かつ 実 occlusion 捕捉**を両立できた。

残タスクは **①高インク下の部分覆いの取りこぼし**(Tier-0 幾何で一部補完・原理的限界あり)、
**②snapshot/screenshot の非原子性**(アニメ中は整定後に実行・§5.9 の罠)。
**③DSL 露出は実装済み**(§5.10 の `requireVisible` 引数。`visible()` は削除済み)。これらを踏まえれば実採用の目処は立った。
単体の FM を無差別に当てる案は引き続き非推奨。

**採用形のまとめ(確定)**: DSL の `requireVisible` 引数で露出し**既定 ON**(2026-07-22 ユーザー決定・
§5.15 #3。オプトアウトは `requireVisible: false`)、判定は
`eligible(足切り) → 低インク or Tier-0幾何(疑い) → FM(cropped・省略許容) → visible判定`の順。
整定後の assert 一点で1回だけ実行。state は診断ログのみ。

## 8. 再現手順(計測ハーネス)

計測に使った単体ハーネス `ftester-poc-occlusion`(合成フィクスチャ計測+デバイス駆動の
live/dump/explore/occtest/e2e モード)は **main の履歴に保存**され(追加コミットで導入 → cleanup
コミットで除去)、working tree(現行の main)には含めない(デバイスを hardcoded フローで駆動する調査
足場のため)。再計測が必要なら履歴から復元する:

```
git log --oneline -- Sources/ftester-poc-occlusion/         # 追加/除去コミットを探す
git checkout <追加コミット> -- Sources/ftester-poc-occlusion/ Package.swift
swift build --product ftester-poc-occlusion
.build/debug/ftester-poc-occlusion <出力ディレクトリ>          # 合成計測
.build/debug/ftester-poc-occlusion e2e <udid> <bundleID> <port>   # デバイス E2E
```

[compose-ios-ax-frame-clamp]: ./design.md
