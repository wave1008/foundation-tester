...
発行側                            ランナー機(<base>)
  api monitor(親)  ── ssh ──▶  api monitor --device-machine local(子)
      ▲ monitorLock                    │ 毎周期 <base>/.fleetest/dispatch.lock/info.json を読む
...
- **拡張へは `dispatchWaiting`(NDJSON・ProtocolVersion 19)で位置と総数を配り、実行ログビューの
  全体レーンに出す**。ログとイベントは**同じ刻みの式を1つ**通す(数字が2箇所で食い違わない)。
...
**残る窓(受け入れている限界。2026-09-21 ユーザー決定)**: 複数機械にまたがる run は、機械ごとに
子が別々にロックを取りに行くので、**「A が機械①にチケットを置いたが機械②にはまだ」の数秒**に
B が機械②を取ると、互いに相手の機械を待つ。**永久には固まらない**(`--wait-lock` の上限で
両者が諦める)が、2 つの run が待ち時間を無駄にする。塞ぐには**親が全機械のロックを辞書順に
取り切ってから子を起こす**(二相ロック = resource ordering)しかなく、**複数機械 run すべてに
ssh 往復ぶんのレイテンシを課す**ので採らなかった。**再検討条件**: 共有が日常化し、この窓に
当たった実例が出たとき。
