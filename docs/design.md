...
- **`textContains` / `textMatches`** は `textIs` と同じ経路(可視性ガードつき)。`textMatches` は
  **部分一致の正規表現**(全体一致は `^...$`)。occlusion-guard には**実際に一致した部分文字列**を
  渡す(パターン文字列は画面に出ないため。`StepExecutor.matchedText` が唯一の判定者)
