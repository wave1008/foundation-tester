...
│                                                        alice の run     │
│ ▾ M1Ultra  sui-ec-mobile / local+remote   空き                          │
│     iPhone 17 Pro(iOS 27.0)-01                                          │
│     iPhone 17 Pro(iOS 27.0)-02                                          │
│ ▸ M1mini   sui-ec-mobile / local+remote   —                             │
└─────────────────────────────────────────────────────────────┘
```
...
- **階層はインデントで示す**: ヘッダ 8px → run 行 24px → レーン 64px。**レーンには縦のガイド線**を
  引く(ユーザー指摘 2026-09-20)—— インデントだけだと、下に続く「走っていない機械」の行と
  見分けが付かず、機械が run の子に見える。**機械の一覧は run 行と同じ 24px**(run と同列)
...
- **走っていない機械は本体に1行ずつ並べる**(ユーザー決定 2026-09-20。展開時のみ = 折りたたむと
  ボードごと隠れる)。**ヘッダに機械の要約は置かない** —— 同じ情報が2箇所に出るため。
...
- **プラットフォームの表示フィルタ**(ヘッダの「全て展開を維持」の右。iOS / Android / すべて の
  **ラジオ3択・既定は「すべて」**。ユーザー決定 2026-09-22): 選んでいない側のデバイスを
