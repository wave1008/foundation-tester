...
- **MCP(`ft_*`)は実行プロファイルのエンジンに追従する**(2026-08-04 ユーザー決定。
  ...
  **揃える理由は探索と実行で見えるものを一致させること**: snapshot の内容もジェスチャの成否も
  ...
  - **ghost の拒否は「実際に別の要素が重なっている」ことまで確かめる**(2026-08-06 に修正)。
    ...
    判定は `RefGuard.occluder`(中心を覆う別要素。**自分を丸ごと包む相手は数えない** ——
    それは遮蔽ではなく容器で、設定アプリの検索で「閉じる」が全画面の減光レイヤー
    `#AdditionalDimmingOverlay` に弾かれた(2026-08-06 の報告。座標タップでは正しく閉じたので
    誤検知と確定)。本物の遮蔽は**一部しか重ならない**。除くのは**自分の祖先と子孫だけ** ——
    「自分より深いものだけ」に絞ると、残像に重なる下部タブが**浅い**ので外す)。
