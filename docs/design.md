...
(FM を呼ばない)。crop 矩形は `FTCore.OcclusionCrop` を FM と共有するので、両者は同じ画素を見る。
実 run で FM の段に届いた crop 163 枚のうち **97% がここで片付き**、FM に回ったのは本当に
描かれていない 2 枚だけだった(p50 92ms。FM は 1.3〜2.8s)。守る規律は4つ:
**①読めなかったことを反転の根拠にしない**(判定は必ず FM。日本語モデルを載せた版では可視な
テキストの 29% を 1 文字誤読した)/ **②一致は完全含有だけ**(先頭一致を許すと、部分的に覆われて
...
2026-09-15)/
**④読めなければ拡大して読み直す**(`upscaleLadder = [1,2,3]`。×4 で悪化するので上げ続けない。
**1行も読めない crop は段を上げない** = 覆いが消えるのを待つ poll 周回で毎周 3 回払わない)。
...
が期待文字列と突き合わせて決める(含む / 先頭 2 文字以上の切り詰め / 5 文字以上なら len÷5 の誤読・先頭文字
必須 / 末尾だけ = covered / 空 = notRendered / 無関係 = textMismatch)。等倍で読めなければ 2 倍で読み直し、
転写が 1 文字差で惜しいときだけ OCR に丸ごと読めたかを確かめる(素通りの向きだけ)。期待文字列を渡して
「見えるか」を訊く旧形は、空白・別の文字の crop でも期待文字列を写して visible=true と答えていた
(おうむ返し。docs/poc-fm-occlusion-guard.md §5.18・maintainer-notes §19)。
...
- **`refresh()` の戻り値を見る**(2026-09-20 追加)。false は「取り直せなかった」で、
  **ノードの中身は古いまま残る**。その読みを「値がまだ入っている」の根拠にすると、
  反映済みの欄を期限切れで落とす(refresh を入れる前と同じ誤りが、refresh 済みの顔で残る)。
  注入の4経路(`setTextAppendingAt` / `clearTextAt` / `setTextAppending` / `clearFocused`)は
  **false の周回では読まず・撃たず**、次の周回で木から引き直す。`SnapshotBuilder` は対象外 ——
  あちらの仕事は「今ある情報を返す」ことで、失敗の判定はしない
- **期限切れの失敗には判別できるだけの事実を載せる**(`failureFacts`): 最後に読み返した値
  (**マスク欄は長さだけ**・40 文字で切る)/ 直前の `refresh()` の成否 / `ACTION_SET_TEXT` が
  受理された回数 / IME ウィンドウが前面か。これが無いと「読みが古かった」のか
  「アプリが本当に値を戻した」のかが記録から割れない(2026-09-20 の誤った赤 →
  maintainer-notes §39.1)
- **Android の 409 / 500 に受け皿は無い**。ホストが `typeDriver` へフォールバックできるのは
  iOS の hybrid だけ(`ScenarioRunnerMain` が iOS の分岐でしか渡さない)なので、
  ブリッジのこの判断がそのままシナリオの合否になる
...
- **inapp の tap は activate 不発時に「整定待ち→要素取り直し→再 activate」で粘る**(2026-07-27)。
  ...
  **撃ち直すのは自前描画(Compose / Flutter)のアプリだけ**(v109。`AppUIFramework.retriesUnfiredActivate`)。
  ...
  8/8 の `freshTapPoint`)。**合成タッチの前の整定待ちは省かない**(v110): v109 で「保存時の frame から
  動いていたときだけ待つ」にしたところ、E2E-RN の回転直後の tap が吸われた(要素の位置は保存時のまま = 回転の
  遷移中はタッチが捨てられる)。省くのは 250ms の小休止と 2 回目の取り直し・activate だけ。
  語彙が読めなければ従来どおり撃ち直す。
...
- **`scrollFrame` でスクロール領域を指定できる**(2026-08-02。Shirates と同じくセレクタ式で受ける)。
  ...
  **in-app は座標を「対象 + 移動量」として読む**(始点で UIScrollView を特定し、始点と終点の差を
  ...
  へ広げてはいけない** —— 1回で端まで飛んで途中の要素を全部飛び越す。
  **Compose/Flutter は「枠が一致するスクロール容器」だけを in-app で送る**(v112。`FTSwipePath.region` =
  scrollFrame の要素の枠を `ScrollRegionMatch` で突き合わせ、一致した容器を根に AX の scroll を走査で撃つ)。
  2 点(始点・終点)からは hitTest も AX も領域を絞れず、指定領域の外を指しても画面本体が動いた
  (2026-08-02 に E2E-Flutter で実測)ので、**一致しなければ従来どおり 501 で XCUITest へ回す**
  (witness = E2E-Flutter 05 scene 11 / E2E-CMP 05 scene 9 の固定ヘッダ)。実装で踏んだ2点:
  ①Compose の容器そのものは scroll を断り、受理するのは内側の要素(容器を根に走査する)
  ...
  **scrollFrame 無しは画面中央に揃える**(v113): XCUITest / Android は画面中央を払うが、in-app は UIKit 系が
  ...
  (`ScrollPointReach`。枠 0 の入れ物は通す)。witness = 05_スクロール S0091 scene 5。
  時間指定は持たない(上記の承認済み差分)。
...
- **閉じたことは必ずステップの注記に出す**(`割り込み … を閉じました`)。黙って閉じると
  「毎回出続けている」異常に気付けなくなる。1ステップで閉じるのは最大10回まで(`maxDismissals:`。
  閉じても消えない相手で無限に回らないため)
...
- **鍵は `"<scenarioID>|<platform>|<file>:<line>|<selector>"`**(`LocatorFingerprintCache.key`)。
  **OS を含める**: iOS と Android で回す共通のシナリオは同じ行を共有するが、同じ要素でも型名が
  OS で違う(CMP のボタンは iOS `button` / Android `Cell`)。OS を跨いで1つの鍵を上書きし合うと、
  直前にもう一方の OS で採った指紋と一致せず、交互に回す限り直らない
- **失効はシナリオ単位の置き換え**(時間の定数は使わない)。鍵が `file:line` とセレクタを含む
  ...
  `scenarioID` の鍵のうち今回**触れなかった**ものを刈る。**「触れた」= lookup または record** ——
  鍵はソースの行+セレクタなので、引かれた = その行は今も在る。record だけを数えていた版は、
  指紋で直ったステップ(record しない)の鍵を、同じシナリオでプライマリが通る別ステップが
  record した run の終わりに刈っていた = **run2 は緑・run3 で指紋を失って赤に戻る**
  (2026-09-15 にコードの読みで発見。`94_指紋照合` は2周しか回さないので踏まなかった。
  `LocatorFingerprintRecordingTests` の3周テストが守る)。守る条件3つ ——
  **①通ったときだけ**(失敗・中断した run は後続ステップに到達していないので、刈ると
  生きている指紋を落とす)/ **②そのシナリオの鍵に1件以上触れていたときだけ**(触れた集合が空の
  run で刈ると、そのシナリオの鍵を根こそぎ失う。**消してはいけないガード**)/
  **③接頭辞 `"<scenarioID>|<platform>|"`(`LocatorFingerprintCache.scope`)で自分のぶんだけ**
  (`--scenario` の部分実行で他シナリオの指紋を巻き込まない・iOS の run が同じシナリオの
  Android の指紋を刈らない)
...
v1 で採取 → v2 で2周 → `heal=false` で赤、を1台に固定して判定する。docs/verification.md)。
...
- **GUI(SwiftUI 版 `fleetest-gui`)は 2026-07-10 に削除**。対話的 UI は VSCode 拡張
  ...
  **実行/デバッグ実行は `fleetest.profile` 未指定なら実行せず、「デバイスモニター」タブでの指定を促す通知
  (ボタン「デバイスモニター」タブを開く = `fleetest.showDeviceMonitor`)を出す**(未指定だとブリッジ自動供給の無い
...
- **実行中の凍結による結果取り消し+別デバイス再実行**(`RunOrchestrator.runWorker`。2026-07-17):
  ...
  - **供給フェーズの進行は NDJSON の `log` にも流す**(2026-09-09): 供給(デバイス起動・インストール・
    凍結 triage)の行は stderr だけに出ていたため、拡張の OUTPUT には出るが「デバイスモニター」タブは無音だった。
    ...
    **実行ログに出すのは選択した台だけ**(§12.6。2026-09-21 の3ペイン化。それ以前は「選択中は
    レーン欄にログを置かない」)なので、worker を持たない log は見出しの状況行
    (`#lanes-run-status`)にも出す(LaneAction の `status`)—— 18台選択のまま実行していると、
    レーン欄には拡大表示だけが並び進行がどこにも出ていなかった(実害 2026-09-09)。
    **全体レーンは選択に関わらず実行ログビューに残す**(§12.6)ので、供給の進行はこの行と
    実行ログビューの両方で読める
  - **「テストを実行」ボタンは先に「デバイスを全て起動」と同じ処理を通す**(ユーザー決定 2026-09-09。
...
- **「プロセス」タブ(常駐プロセス一覧・停止)**(2026-07-19): **起動時は出さない** —— 設定タブ「ツール」の
  ...
  host-metrics/stream を停止し **iOS シミュレータと Android エミュ本体(qemu)は残す**(「デバイスモニター」タブの領域。
  ...
  **この workspace 由来のみ**=workspaceRoot/binaryDir を含むコマンド。machine-wide 巻き込み回避)、
  ...
  モニターパネルを閉じる**(モニターを止めたままタブを開いておくと「デバイスモニター」タブが状態更新を失い
...
- **MCP と run の協調(mcp-lease)**(2026-09-11・ユーザー決定「避けて、足りなければ警告して使う」):
  ...
  `<pid> <開始時刻>` を書く。**生死は pid + 開始時刻**(時間の閾値を置かない —— run は印を避けるだけで断らないので、
  使い終わった後も MCP が生きている間は残る印の損は「その台を避ける」に収まる。開始時刻まで見るのは pid の再利用で
  印を生き返らせないため)。MCP は stdin EOF で自分の印を消し、死んだ印は `sweepStaleLeases` が掃除する。
  run(`run` / `api run` の両方)は、ほかの台で本数ぶん足りるなら印のある台を**予備にも残さない**
  (`ResolvedProfile.limitingDevices(avoiding:)`。残した台はシナリオを早い者勝ちで取り合うので、予備でも結局そこで走る)。
  本数が分からない(絞らない)ときも、ほかに台があれば外す。それでも使う台は「MCP(pid N)が操作中。この run が奪う」と
...
> セパレーターは手動だけで、保存値が無いときの初期位置は表示エリアの 20%(`splitter.js` の
...
- **罠(実害 2026-07-31)**: `devices` のポーリングは「デバイスモニター」タブが**非表示の間も届き続ける**
  ...
  **レイアウトが無いとき(`clientHeight===0`)は書かずに抜ける** — 非表示中は直前の正しい値が保たれる。
  ...
  `switchTab` → `reapplyPaneHeights` が測り直す(3ペインぶん。§12.6)
...
- **「実行中」と「デバイス」の間にもセパレーター**(`#devices-separator`。ユーザー決定 2026-09-22)。
  ...
  **②はみ出しは箱で止め、行は `#run-board-rows` が自前でスクロールする**(`min-height: 0` が無いと
  flex の既定で中身の高さまで伸びて `overflow` が効かない)/ **③畳んでいる間は出さない**
  (伸ばす中身が無い = 掴めない仕切りを見せない。CSS の `display: none` で `#splitter` /
  `#splitter-log` と同じ書き方。`offsetHeight` が 0 になるので高さの勘定からも自動で外れる)/
  **④畳みの反映は `runBoard.js` が
...
- **両ペインに出すのはラインビューで選択した台だけ**(ユーザー決定 2026-09-21)。**全体レーン
  (`__overall__`)は台ではないので選択に関わらず実行ログビューに残す** —— 供給フェーズの進行
  (worker を持たないイベント)の受け皿で、消すと run の進みがどこにも出ない(§12.4 の `status` 行と対)
- **ちょうど1台選択のときだけ**、グリッドビューの中に「拡大表示 | 実行ログの複製(ミラー)」を
  `.lane-pair` で並べる。**ログ本体の DOM は動かさない**(選択を外せば `#lanes-grid` 側で続きが読める)
  ので、新しい行は本体とミラーの両方へ書く(`laneLog.js` の `appendLaneLine`。行数の上限も両方で効かせる)。
  この間は**実行ログビューを自動で畳む**(同じログを上下に出さない)—— 旗は `splitter.js` が持ち、
  利用者の設定(`logViewVisible`)は書き換えない(選択が1台でなくなれば旗を下ろすだけで元へ戻る)。
  **「畳む理由が生きているか」(= 複製が見えているか)は旗と別に毎回導出する**(`autoFoldActive` =
  旗 ∧ グリッドビューが開いている)—— グリッドビューを畳んだときに旗のほうを捨てると、開き直した
  瞬間に同じログが上下へ出たまま次の選択変更まで戻らない。**見出しクリックは「見えている状態」を
  反転する**(旗が効いているかで分岐を変えると、押しても何も起きない空振りができる)
- **選択0台は両ペインに「デバイスを選択して下さい」**。実行ログビュー側は**全体レーンに行があるときは
...
- 見出しの文言は **「デバイス一覧」/「選択したデバイス」/「実行ログ」**(ユーザー決定 2026-09-22)。
  ...
  **ラインビューの見出し行にこの順で並べる** —— どちらもタイルの見え方を操るので隣り合わせる
  ...
  台数(`.header-count`)は**ラインビューが全台・グリッドビューが選択した台数**
  (ユーザー決定 2026-09-22。数える対象は違うが形は揃える)で、**0台でも出し、幅を固定**する
  (欄が消える / 桁が変わると右のトグルが動くため。等幅数字 + 言語ごとの `min-width`)。
  **全台のほうは選択の経路(`updateSelectionUi`)ではなく `applyDevices` で書く** ——
  台が減っただけのときはあちらを通らない。守る規律3つ:
...
## 18. デバイスモニターの run ボード(フリート横断の実行状況。2026-09-20 設計・2026-09-21 実装済み = §18.6 の段1〜6)
...
- **run の段階を `phase` で持つ**(`"building"` → `"preparing"` → `"running"`。ユーザー指摘
  ...
  (同じ pid = 同じファイル)。**`"building"` は実際に `ScenarioHost.build` を呼んでいる間だけ**
  (ユーザー決定 2026-09-22)—— 入口から立てると、`--skip-build`(**機械分担のローカル子は常にこれ**。
  親が1回ビルド済み)では**1バイトもビルドしないまま「ビルド中」**が出て、ビルドが終わってからも
  供給が始まるまで「ビルド中」のままだった。入口は `"preparing"` で書き(書かないとその間ボードから
  消える)、build を挟む間だけ `"building"` にして**直後に戻す**
  (`RunProgressLedgerWiringTests` が2経路とも走査で固定)。**書き手は段階ごとに場所が違う** —— `"building"` は
...
- **詰まりの事実だけを載せる**(段6): run 単位の `requeued`(結果を捨てて振り直した回数)と
  ...
  **`expectedSeconds` は記録用で画面には出さない**(ユーザー決定 2026-09-21)—— 経過の隣に
  並べると、超えた瞬間は同じ値が2つ並ぶだけ(`0:08(中央 0:08)`)で情報が無い。
  後から「いつもと比べてどうだったか」を分析するために**台帳と NDJSON には残す**
...
- **`monitorLock` と情報源が別**なので、ロックを取らない run(`--force-lock` で押し切った run)も
  ここには出る —— remote-runner.md §18.7 末尾の「ロックに写らない run」がこの経路で見えるようになる。
  **`monitorLock` も手元を出すようになった**(2026-09-21)が、こちらは「本数と進捗」であちらは
  「占有」なので**別の軸のまま**(片方を他方から導かない)
...
- 材料は LPT がすでに使っているもの —— `RunResultsStore.scanRecords` → `LPTScheduler.durations`
  (`(scenarioID, platform)` ごとの中央値)。**2つ目の実装を作らない**。読むのは run 開始時に1回
- **値は makespan の下界**(新しい定数を置かない):
  ...
  並列スケジューリングの makespan は「最長ジョブ」と「総和 ÷ 台数」のどちらも下回れない ——
  **下界だから `~` 付きで出す**(実際はこれ以上かかる)。早い者勝ちのキューを LPT で再現したり、
  係数で水増ししたりしない(根拠のない数字を作らない)
- **実績が1件も無い run は `etaSeconds` を省く**(欄ごと省く = 表示は「—」)。推測値を出さない。
  **一部のシナリオにだけ実績が無いときは、実績のある中央値の中央値で埋める**(FleetRunner の
  `unknownDurationMs` と同じ考え方 = 既にある手口を流用し、新しい定数を持ち込まない)
- **分母(`total`)は run 開始時に確定した本数**で、再キューでも失敗でも動かさない
- **eta も記帳時点の値**(経過と同じ)。読み手は受信からの経過を差し引いて秒読みする
  (`liveRemaining`。0 を切ったら「残り ~0:00(+M:SS 超過)」)
...
```
┌ ▾ 実行中 2 ────────────────────────────────────────────────┐
│ ▾ ec-mobile / ios-smoke           ███████░░░  7/12 ✕2   4:21 / 残 ~3:10 │
│   ▾ local                                                               │
│       ▯ iPhone 17-01  ▶ 05_検索          1:12                           │
│       ▯ iPhone 17-02  ▶ 07_決済          0:48                           │
│       ▯ iPhone 17-03  ⏹ 待機              —                             │
│   ▸ M1Max                                                               │
│ ▸ ec-mobile / android-regress     ███░░░░░░  3/20       1:48 / 残 ~9:40 │
│                                                        alice の run     │
│ ▾ sui-ec-mobile / local+remote                                          │
│   ▾ M1Ultra                                             空き            │
│       ▯ iPhone 17 Pro(iOS 27.0)-01                                      │
...
- **ヘッダ行はどこを押しても開閉する**(ユーザー決定 2026-09-20)—— 三角だけを当たり判定にすると
  小さすぎて「開ける」と気づかれない。三角は状態の表示で、**文字は常に ▶・向きは CSS の回転**
  (字を差し替える形は細くて伝わらなかった)。個々の run の行の chevron も同じ規律
- **準備中の run は本数でなく「準備中」を出し、進捗バーは隠す**(本数も割合もまだ意味を持たない)。
  経過時間は出す —— どれくらい待っているかが分かる。**件数(「実行中 N」)には数える**
- **並びは常に機械の順**(local → 登録簿の順。ユーザー決定 2026-09-20)—— run が始まっても
  **その機械の位置は動かさない**(行の種類が「空き」から run に変わるだけ)。動くと目が追えない。
  機械分担の run は最初に現れた機械の位置に1行だけ置く。**行の高さは固定する**
  (和文の「空き」と記号の「—」は既定の行高が違い、揃えないと上下に揺れる)
- **詰まりは事実の並置だけ**(段6。ユーザー決定 2026-09-20): run 行に `⟳N`(再キュー)と
  `レーン離脱 N` を、**0 のときは出さない**。**語は中立に**(「遅い」「刺さっている」と書かない)
  —— ツールにはアプリが重いのか機械が混んでいるのか分けられない(失敗の記録に分類を置かないのと
  同じ理由)。**実績中央値は画面に出さない**(ユーザー決定 2026-09-21。記録には残す = §18.1)
- **台の行にはデバイスのアイコンを名前の左に置く**(ユーザー決定 2026-09-22)。**色は
  プラットフォームのバッジと同じ値**(iOS / Android。`platform` が無い台は色を付けない ——
  知らないものをどちらかに見せない)
- **レーンのデバイス名は省略しない** —— 末尾で見分けるので、途中で切ると同じ見た目が並ぶ
- **ツリーは3段**(ユーザー決定 2026-09-22): **根**(run 行 = その run の `project / profile`、
  または走っていない機械をまとめる行 = モニターが台を並べる範囲)→ **機械の枝**(マシン名の
  バッジだけ。**`project / profile` は枝に出さない** —— 根が出しているので、置くと同じ文字が
  機械の数だけ並ぶ。同日ユーザー決定)→ **台**。**どの行の種類でも同じ作り**にする ——
  枝だけを素の div にすると、インデントと2カラムの境目が行ごとに割れてツリーに見えない。
  **機械が1台の run でも枝を作る**(run 行にバッジを出さない = 同じ名前が2行に並ばない)
- **機械の枝は機械ごとに畳める**(ユーザー決定 2026-09-22)。**既定は開いた状態**
  (run 行の既定は閉じ、と逆)なので、**畳んだものを覚える**形で持つ。畳んでも**見出しは残す**
  —— その機械が居ることは消さない。寿命は run 行の展開と同じ(webview の `getState` だけ)
- **階層はインデントで示す**(16px 刻み): ヘッダ 8px → 根 24px(名前 48px)→ 機械の枝 40px
  (名前 64px)→ 台 80px。**縦のガイド線**は根の子(= 機械の枝)の側に 32px で引く
  (ユーザー指摘 2026-09-20)—— インデントだけだと、機械が run の子であることが読めない
- **行は2カラム**(ユーザー決定 2026-09-22): 左 = ツリー(機械・台の名前)、右 = ステータス
  (機械の「空き / —」・run の進捗と経過・台の実行中シナリオ)。**右は左寄せ**。
  境目は**マウスで動かせる**。**既定はいちばん長いラベルがちょうど収まる幅**
  (ユーザー決定 2026-09-22。比率ではない)—— 測る一瞬だけ左カラムを中身なりの幅にして
  全行の最大を採る(flex-basis が効いたままだと今の幅しか返らない)。**行を作り終えてから測る**
  (揃っていないと短い側に決まる)。守る規律4つ: **①幅は px で持つ**
  (比率だと丸めのたびに境目が滑る。既定は「まだ動かしていない」間だけ毎回引き直すので、
  台が増えて名前が伸びれば追従する)/
  **②境目を全行で1本に見せる**ために、行の左右 padding を 8px に揃え・階層のインデントは
  左カラムの padding で表し・**レーンのガイド線は背景で引く**(border/margin は箱の幅を変えるので
  行ごとに境目がずれる)/ **③左右とも 80px は残す**(引き切って片方を潰さない。
  **保存する値もクランプ済み** —— 表示だけ直して控えに範囲外を残すと復元値が意味を持たない)/
  **④見出し行には掛けない**(掴む相手ではないうえ表示フィルタに重なる。top は測ったヘッダの高さ)。
  **寿命はこのパネルそのもの**(ユーザー決定 2026-09-22。`tilePaneHeight` とはここが違う)
  —— **タブを閉じたら解放し、開き直したら既定へ戻す**。隠す/再表示では保つ
  (`retainContextWhenHidden` で webview が生き続けるため)。そのため
  **webview のモジュール変数だけが持ち主**で、**host へ送らない・`getState` へ書かない**
  (どちらも閉じても残るのでリセットされなくなる)。ワイヤも持たない
- **台のツリーは run の有無に関わらず出す**(ユーザー決定 2026-09-22)—— 認識した台が1枚も
  見えないボードでは、フリートに何が居るのかがここから分からない。守る規律4つ:
  **①台の一覧と並びはラインビュー(`monitorDevices`)から採る**(run のレーンからではない)。
  **ただし「起動中のデバイス」の表示フィルタは通さない**(ユーザー決定 2026-09-22)—— 通すと
  **ビルド中の run の下から台が丸ごと消える**(供給前なのでどの台もまだ起動していない)。
  停止中・観測できない台もツリーには出す。そのため `monitorDevices` は**表示フィルタ前の全台**を
  webview へ送り(`devicesToWebviewMessage`)、落とすのは `deviceTiles.js` の入口でタイル側だけ/
  **②run の行にもその機械の全台を並べ、run が使っている台にだけシナリオを添える** ——
  run に出ていない台も見える / **③レーンの側にしか無い台は落とさない**(消えたタイル・観測窓の
  外でも run の事実は残す。台の一覧のあとに続ける)/ **④台の一覧が変わったら描き直す**
  (`refreshRunBoardDevices`。run ボードは `monitorRuns` でしか描き直さないので、呼ばないと
  古い台が残る)。**走っていない機械の行も run 行と同じ作り**(`.run-board-row`)で、
  行の高さ・インデント・展開の記録を共有する —— 見出しの1行だけ控えめにし、**下の台のツリーは
  薄くしない**。行に出す `project / profile` は**モニターが台を並べる範囲**
  (`api monitor --project <P> [--profile <run>]` = ツールバーの選択)で、run 行の scope
  (その run の project/profile)とは出どころが違う
- **走っていない機械は根の1行にまとめる**(ユーザー決定 2026-09-22。それまでは1機械 = 1行だった)
  —— 根はモニターが台を並べる範囲(ツールバーの選択)で、その下に機械の枝が機械の順で並ぶ。
  **根は最初の空き機械の位置に置く**ので、run が始まって機械が根から抜けても並びは崩れない。**ヘッダに機械の要約は置かない** —— 同じ情報が2箇所に出るため。
  **run のある機械はここに出さない**(その機械は run の行として出ているので二重になる)。
  状態は**空き / 不明の2値**。空きは語、**不明は「—」**(ユーザー決定 2026-09-20。
  `remote status` の LOCK/FM 欄が判定不能に使うのと同じ記法)。**どちらもグレーで、警告色は
  使わない** —— 観測できていないのは異常ではない。ダッシュの意味は title で言う。
  **不明を空きに見せない**のがこの行の目的で、区別は語と「—」が担う
...
- 行のクリックでその run の台を**ラインビューで選択**(既存の `selectedDeviceIds`)。機械の枝の
  見出しはその機械の台だけ。**台の行は押しても何も起きない**(ユーザー決定 2026-09-22)——
  ここは実行状況を読む場所なので、cursor も hover の地色も付けない
- **ヘッダ右端の「全て展開を維持」はトグル(モード)**(ユーザー決定 2026-09-20)—— 一度きりの操作では
  なく、**ON の間はあとから始まった run も展開して出る**。**アイコンだけを置き、名前は tooltip と
  aria-label が持つ**(ユーザー決定 2026-09-21。ネイティブ `title` は使わず `setHoverTip`)。
  **文言は ON/OFF で入れ替えない** —— これは「いま開く」ではなく「開いた状態を保つ」モードのスイッチで、
  ON/OFF は押し込み(`.toggled`)で見せる。
  モードなので host の workspaceState に持つ(groupKey ごとの展開は webview のみ、と対)。
  ON のまま1行を閉じたらモードを抜け、**他の行は開いたまま**(抜けた瞬間に全部畳まない)
- **プラットフォームの表示フィルタ**(ヘッダの「全て展開を維持」の右。iOS / Android / すべて の
  **3択・既定は「すべて」**。ユーザー決定 2026-09-22): **見た目はバッジ**(同日ユーザー決定。
  ラジオボタンではない)—— 選んでいるものだけ色付き(iOS / Android は**タイルのデバイス名ピルと
  同じ色**・「すべて」は白)で、**選んでいないものは灰色**。3択であることは role="radiogroup" /
  role="radio" + aria-checked が持つ。選んでいない側のデバイスを
  **4つのセクション(実行中・デバイス一覧・選択したデバイス・実行ログ)から同時に隠す**。
  **知らない値は「すべて」へ倒す**(webview も host も)—— 台が黙って消えるより出しすぎるほうが安全。
  守る規律3つ: **①落とすのは `applyDevices` の入口1箇所**(4つともこの一覧から作るので、
  ここで落とせば同時に消える。**次の監視サイクルで戻ってこない**のもこれが理由)/
  **②生の一覧を控えて切り替えたその場で描き直す**(次のサイクルを待たない)/
  **③run のレーンは別経路**なので、台の一覧に無いレーン(消えたタイル)は `lane.platform` で
  別に落とす。永続化は `tilePaneHeight` と同じ契約(`setPlatformFilter` / `platformFilter`)
...
- ~~**空き機械の行をボードに並べる**~~: 当初は「行が台数ぶん伸びて run が埋もれる」として却下し
  ヘッダの要約に畳んでいたが、**2026-09-20 に反転**(ユーザー決定)—— 要約と本体で同じ情報が
  2箇所に出るほうが邪魔だった。いまは本体に並べ、ヘッダには件数だけを置く(§18.5)
