...
- **撃つ前に言える「たぶん何も起きない」は注記にする**(2026-08-07。`TapTargetGeometry`。
  MCP と共有)。2形とも実測由来: **無効な要素**(木には `disabled` と印字しているのに操作経路が
  `enabled` を見ていなかった)/ **中心が中身のどこにも乗らない容器**(`#layers_fab_button` を
  叩くと中心が地図の上なので海上にピンが落ちた)。
  **失敗にはしない** —— 無効な要素をわざと叩いて反応しないことを確かめる書き方は正当で
  `enabledIsFalse` もある。注記なら後段の失敗から原因へ辿れる。
  両方に当てはまるときは「そもそも無効」を先に言う。
  **2つは別の関数に分けてある**(2026-08-08): `disabledAdvisory` は**撃つ座標に依らない**ので
  どの経路でも言えるが、`missedContentAdvisory` は **frame の中心を撃つときにしか言えない** ——
