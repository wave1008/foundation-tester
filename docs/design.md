...
- **MCP(`ft_*`)は実行プロファイルのエンジンに追従する**(2026-08-04 ユーザー決定。
  ...
  **揃える理由は探索と実行で見えるものを一致させること**: snapshot の内容もジェスチャの成否も
  ...
  - **ghost の拒否は「実際に別の要素が重なっている」ことまで確かめる**(2026-08-06 に修正)。
    ...
    **誤検知が4形続いた**のは、最初に「容器の外に居る」だけで**拒否**まで踏み込んだのが
    設計の誤りだったため。この判定は DSL では「掴み直して送り直す」合図で、外しても次の周回で
    回復する。同じ閾値を拒否へ格上げすると、外れがそのまま機能の喪失になる。
    **これ以上誤検知が出るなら、拒否をやめて警告に落とす**(黙って誤操作するよりはましだが、
    押せるものを押せなくするのは別種の害)
