...
- 型名は `SelType` の静的メンバ(OS 共通契約の5型 + エイリアス + 頻出型)。語彙に無いものは
  `.custom("...")`(先頭小文字へ正規化)
- **生成側(`ScenarioCodeGen` / explore 生成 / `/ftester-scenario`)は文字列版を出す。
...
- **容器の推測に依存する補正は3層で止められる**(上位から `FT_CONTAINER_INFERENCE=off` の殺しスイッチ /
  ...
  まとめて無効化できるようにしてある。**見えている部分を撃つ補正には床(8pt)**もあり、
  わずかな重なりへは突っ込まない
  ...
  **揃える理由は探索と実行で見えるものを一致させること**: snapshot の内容もジェスチャの成否も
  ...
  - **`ft_scroll_to` は DSL と同じ `StepExecutor` に委ねる**(2026-08-06)。整定待ち・キャッシュ回避・
    ...
    **同じ知見の2つ目の実装を作ると必ず割れる**。MCP は FlowStep を1つ組んで投げるだけ =
    MCP で届く要素はシナリオでも届く
  - **ghost の拒否は「実際に別の要素が重なっている」ことまで確かめる**(2026-08-06 に修正)。
    当初は `isOutsideContainer` だけで拒否していたが、**ホーム画面の dock が押せなくなった**
    —— dock のアイコンは容器の推測から外れるが、その座標にはそれ自身しか無い。
    `isOutsideContainer` は DSL では「掴み直して送り直す」= やり直しの合図で、外しても次の周回で
    回復する。MCP はそれを**拒否**へ格上げしたので、同じ閾値では強すぎた。
    判定は `RefGuard.occluder`(中心を覆う別要素)。**遮蔽と数えない相手が3種類**ある:

    | 除外するもの | 理由 | 実例(すべて 2026-08-06 の報告) |
    |---|---|---|
    | ① 相手自身が残像 | **描かれていないものは何も覆えない** | 設定アプリの検索で「閉じる」を弾いていた `clickable (16,484 370x52)` は、スクロールで画面外へ出たリスト行の容器 |
    | ② 自分を丸ごと包む相手 | 遮蔽ではなく容器 | 同じ「閉じる」を弾いた全画面の `#AdditionalDimmingOverlay` / `Toolbar` |
    | ③ 自分の祖先と子孫 | 自分を含む容器を数えない | 「自分より深いものだけ」に絞ると、残像に重なる下部タブが**浅い**ので外す |

    **①を②より先に見る** —— ②の包含判定は 1pt の差で外れるほど際どい
    (実測: 閉じる y483..521 対 残像 y484..536)。閾値では守り切れない。
    本物の遮蔽は**一部しか重ならない**(残像 `#row_11` (16,790 370x56) に重なる
    下部タブ `#tab_controls` (134,792 134x48))。
    副産物として精度も上がった: 3件あった残像のうち**実際にタブへ当たる1件だけ**を指すようになった。

    **拒否はやめた**(2026-08-06。誤検知が5形続いたので、直前に引いた撤退線を実行した)。
    いまは**撃ったうえで「何に当たったかもしれないか」を応答に添える**(`RefGuard.ghostWarning`)。
    上の除外規則は、その文面の精度を上げるために残してある。

    **後退した理由**: 木の幾何だけでは「実際に描かれているか」を決められない。相手が残像でなく、
    包含でもなく、それでも描かれていない形が最後に出た —— **キーボードの下**にあるリスト行。
    キーボードはスナップショットから丸ごと除外している(キー1つ1つが Button として写り込むため)
    ので frame すら取れず、**原理的に判定材料が無い**。特殊ケースを足し続ける競争になっていた。

    5形の内訳: ① dock(何も重ならない)/ ② 浅いタブが重なる残像 / ③ 全画面の容器・減光レイヤー /
    ④ 残像そのものが覆っている / ⑤ キーボードに隠れた行が覆っている。
    **すべて利用者からの報告で見つかった** —— 自分の掃討(4 SUT 全画面)では1件も出せていない。

    根本は、**やり直しの合図に使う判定を拒否へ格上げした**こと。DSL 側は外しても次の周回で
    回復するが、拒否は外れがそのまま機能の喪失になる。**同じ判定を別の強さで使うときは、
    外れたときの損害が同じかを確かめる**
