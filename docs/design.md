...
- **「書けるセレクタ」は1箇所で決める**(`MCPServer.SelectorNaming`。2026-08-09)。
  優先順は **一意な `#id` > 一意なラベル > スコープ記法 `#容器 >> .型[n]` > 書けない(nil)**。
...
- **MCP(`ft_*`)は実行プロファイルのエンジンに追従する**(2026-08-04 ユーザー決定。
  ...
  **揃える理由は探索と実行で見えるものを一致させること**: snapshot の内容もジェスチャの成否も
  ...
  - **`ft_batch` は StepExecutor に委ねる**(2026-08-10)。MCP の dispatch を順に回す案は採らない ——
    ...
    **steps は「DSL の行そのもの」**(2026-08-10 に14キーのオブジェクト形から改めた —— `{"command":
    "tap","selector":"#a"}` のような翻訳は「そのまま貼れる」という主目的そのものを裏切ると外部
    指摘があった)。1要素 = `tap("#a")` のような呼び出し1行。改行が入っていれば複数手に分割し、
    行末の `;`・前後の空白は落とす。**限定文法のパーサ**(`BatchLineParser`。純粋関数・デバイスにも
    MCPServer の状態にも依存しない)が `name [ "(" arg ("," arg)* ")" ]` / `arg := [label ":"] value`
    / `value := string | number | bool | .ident` だけを解釈する —— 入れ子呼び出し・配列・演算子・
    クロージャは構文的に受け付けず明確なエラーにする。**文字列は `'…'` も `"…"` と等価に受ける**
    (2026-08-10。MCP の引数は JSON 文字列なので `"` は `\"` の二重エスケープになり生産性が低い、
    という外部指摘。Swift に無い形なのでシナリオへ貼るときは `"` へ戻す —— draft への記録は
    FlowStep からの再生成なので契約は崩れない)。位置引数の名前は**シグネチャ文字列
    ...
    すら無いラベルは別の文言(`ft_dsl_commands` へ誘導)で拒否する。**ref は書けない** —— 行の文法に
    ref を表す形が無いので、`ref:` はどのコマンドの signature にも無いラベルとして自動的に拒まれる
    (専用の**検証**分岐は不要になったため削除。旧オブジェクト形だけが必要としていた)。
    ただし**文言だけは ref を特別扱いする** —— 「そんな引数は無い」で終えると渡し方を探して
    もう1往復するので、理由と `tap("#id")` への書き換え方まで返す(2026-08-10 のデバイス確認)。
