...
**⚠️ ただし「残した」3つのうち2つは production で動いていない**(2026-08-05 に経路を追って確認。
着手する前にここを読むこと):

| 残したつもりのもの | 実体 |
|---|---|
| 実移動量がビューポートを超えたら刻みを詰める | `spanScale` は **`scrollPath` の中でしか読まれない**。`scrollPath` は `scrollFrame` 未指定なら nil を返すので、**既定の `scrollTo` では計算されるだけで一切効かない** |
| 座標スワイプが動かなかったら従来経路へ戻す | `coordinateSwipeIneffective` は**書き込みとリセットだけで読み取り箇所が無い**(完全な dead。代入されているのでコンパイラも黙る) |
| `implicitScrollTarget` の規則 | production からの呼び出し無し(単体テストのみ) |

さらに**動く条件下でも閾値の基準がずれている**: 詰めるかの判定に使う `extent` は
[StepExecutor の探索ループ](../Sources/FTCore/StepExecutor.swift)で**画面の高さ**だが、
行き過ぎの実害が起きるのは**リストの可視高**。§3.18(f)の実測を当てると
SwiftUI は 1 スワイプ 681pt に対し閾値が 0.8×874=699pt で**発火せず**(可視高 492pt = 1.38 倍の超過)、
Flutter だけが発火する —— **いちばん取りこぼす SUT でガードが素通り**している。

**直すなら「実際に動いた要素の外接矩形」を extent にするのが筋**(`measuredTravel` が既に共通要素の
差分を計算しているのでゼロコストで採れる)。これは 2 度の撤回理由(Compose が `scrollable` を
申告しない / in-app の到達距離が縮む)を**どちらも踏まない** —— 刻みではなく閾値だけを直すため。
**未着手・ユーザー判断待ち**(直すと SwiftUI の探索スワイプが増えて遅くなる方向のため)。
