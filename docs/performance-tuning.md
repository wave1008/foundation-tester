...
**再検討の条件**: この判定に使える witness が無い —— 自前 SUT の一番長い画面(40 行リスト)は
**加速が始まる前に端に着く**ので、長文で得られるはずの読み節約が測れない。
**10 ページ以上の実文書(受け手の規約画面など)で測れるようになったら**、
実ジェスチャのエンジンに限って再評価する価値がある(in-app UIKit/SwiftUI は §3.22 で
往復自体が消えたので対象外)。
