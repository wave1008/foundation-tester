...
|---|---|
| `type("文字列", replace: false)` | Types into the **currently focused** element (focus it first with `tap(入力欄)`). The argument is text, not a selector — passing something that looks like a selector (starts with `#`, or contains `\|\|` / `>>`) fails before execution rather than silently typing the literal string. To actually type such a string, use the two-argument form `type("#field", "#email")`. `replace: true` clears the field (like `clearInput`) before typing. |
| `type(sel, "文字列", timeout:scroll:maxSwipes:replace:)` | Resolves the element, then types into it. Japanese text is typed as-is (no IME switching needed). `replace: true` clears the field before typing. |
...
- **Line breaks (`\n`) follow the OS default.** On iOS this arrives as a Return key press: it
  ...
  [`pressEnter()`](./press_enter_hide_keyboard.md) instead — `type("腕時計\n")` does not tell the
  reader whether a newline or a submit was intended.
- **`tap(input field)` → `type("文字列")` is supported.** On Android, input fields are often a
  ...
  `type(sel, "文字列")` resolves, focuses, and reads the value back in one step.
