...
| 関数 | 説明 |
|---|---|
| `select(sel, waitSeconds:requireVisible:scroll:maxSwipes:)` | 要素を掴みます。`exist` と違い**検証ではない**ので、レポートに検証ステップとして残りません。値の読み出し(`.text`/`.value`/`.id`)や検証コマンドへのチェーンの起点に使います。掴めなければ失敗させず**空要素**を返すので、呼び出し側は `.isEmpty`/`.isNotEmpty` で分岐します。在ることを保証したいなら `exist` を使ってください。`requireVisible: false` で可視性照合自体を外せます。 |
