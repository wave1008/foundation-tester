...
| 関数 | 説明 |
|---|---|
| `tap(sel, holdSeconds: 0, waitSeconds:scroll:maxSwipes:containerInference:)` | セレクタにマッチする最初の要素をタップします。`holdSeconds` を 0 より大きくすると長押しになります(既定 0 = 通常タップ)。タップ前に対象が操作可能になるまで待ちます(後述)。 |
