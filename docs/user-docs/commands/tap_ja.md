...
| `tap(x: Int, y: Int, holdSeconds: 0, maxGestureSeconds:)` | 座標を直接タップします。座標は snapshot の `screen` と同じ座標系です(iOS = pt / Android = px。dp ではありません)。セレクタで指せるならそちらを優先してください。iOS の in-app エンジンでは、画面外とソフトキーボードの上の点は失敗になります(in-app はキーを押せません。先に `pressEnter` でキーボードを閉じてください)。スクロール容器で切れて見えていない要素は、frame が点を含んでも押しません。 |
...
- **`tap` は対象が操作可能になるまで待ってから撃ちます。** 画面が出た直後は、要素は木に
  ...
  `tap` はステップの `waitSeconds:`(省略時は約5秒)まで、対象が `enabled` になるまで解決を
