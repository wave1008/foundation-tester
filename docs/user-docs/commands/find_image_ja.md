...
- `findImages` もラベルのフォルダの見本を全部使います。同じ要素に複数の見本が当たったときは、最も近い距離で1件にまとめます(Shirates の `findImages` は見本を1枚だけ使います)。
...
- **見本は、テストを回す端末の OS の版・画面の倍率ごとに採ってください。** 同じ行でも OS の版が違うと、文字の
  描き方の違いで距離が 0.04〜0.12 離れます(実測: Android 15 / 13 / 12 の同じ行。見た目では分からない差でも離れます)。
  同じ OS の版の端末どうしなら 0.001〜0.003 です。`findImage` / `existImage` はラベルのフォルダの見本を全部試すので、
  採った見本を同じフォルダへ足すだけで両方の端末で掴めます(`threshold` を絞ったまま使えます)。
  `findImages` も同じです。オン / オフのように状態で見た目が変わる部品も、両方の見本を置けば両方を返します。
