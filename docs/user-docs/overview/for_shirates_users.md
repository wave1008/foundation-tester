...
| `displayedIs` | `requireVisible:` argument + a run profile's `fmTextOcclusionCheck` / `ocrTextOcclusionCheck` | visibility checking is a parameter on the relevant command rather than its own assertion |
