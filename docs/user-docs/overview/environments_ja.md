...
- FM が使えない環境では、`screenLooksLike` は失敗ではなく**スキップ**されます。テキストの視覚検証は
  FM の代わりに端末の OCR(`ocrTextVisualCheck`)の読みで判定し、「見えていない」と読めたステップには
  注記 `ocr-only-would-flip` を残します(**今は失敗にしません**。緑のまま注記だけが付きます)。
  どちらも run は緑のままになりうるので、FM が実際に使えているかは `fleetest doctor --fm-only` で確認して
