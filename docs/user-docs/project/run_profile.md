...
| `ocrTextVisualCheck` | bool | `true` | Let the occlusion guard read the element with on-device OCR (Vision) before asking FM. When the expected text is read in full the step passes without an FM call; anything else goes to FM. When FM gives no verdict (macOS 26, or FM failing), the guard judges from this reading instead and leaves the note `ocr-only-would-flip` on a step whose element it reads as not visible (not failed for now). Turning this off makes the check slower and drops that fallback. It has no effect when `textVisualCheck` is `false`, because the guard itself does not run |
