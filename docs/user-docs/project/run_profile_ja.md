...
| `ocrTextVisualCheck` | bool | `true` | occlusion guard が FM に訊く前に、端末の OCR(Vision)で要素を読む。期待テキストが丸ごと読めた回は FM を呼ばずに通り、読めなければ FM が判定する。FM が判定を返せないとき(macOS 26・FM の不調)は、この読みで判定し、見えていないと読めたステップに注記 `ocr-only-would-flip` を残す(今は失敗にしない)。切ると検査が遅くなり、FM が使えないときの判定も無くなる。`textVisualCheck` が `false` の run では guard 自体が走らないので効かない |
