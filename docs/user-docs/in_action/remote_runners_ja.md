...
| モニターの「テスト実行」タブでリモートデバイスの状態・映像を表示 | ✅ |
...
| ファイアウォールの「すべての着信接続をブロック」が OFF(ON だと sshd も遮断される。ファイアウォール自体は ON でよい) | `sudo /usr/libexec/ApplicationFirewall/socketfilterfw --getblockall` が `disabled` |
...
ランナー機は**同じ LAN に居る必要はありません**。`ssh <宛先> 'echo ok'` が通る Mac なら
ディスパッチできます。遠隔化されているのは run 単位のジョブだけなので、往復の遅延にも
比較的強い構成です。

- **開くのは発行側 → ランナー機の SSH 1本だけ**です。手元の Mac に着信は要りません
  (転送・進行表示・成果物回収・ライブ映像はすべてこの接続の中を通ります)。
- **ポート番号や踏み台は `~/.ssh/config` に書きます。** 宛先に `host:2222` のようには書けません
  (`:`・空白を含む宛先は拒否されます)。`Host mac2` / `HostName` / `Port` / `ProxyJump` を
  書いたエイリアスを作り、そのエイリアスを宛先として使ってください。
- **素のインターネットに SSH ポートを晒さないでください。** VPN(Tailscale 等)でリンクを
  張るのが想定している形です。ポート転送する場合は1ポートだけにし、送信元 IP を絞り、
  ランナー機の sshd はパスワード認証を無効にします。
- **画面共有(5900)はルーターに開けません。** `ssh -L 5900:localhost:5900 <宛先>` で
  トンネルして `vnc://localhost:5900` に繋ぎます。
- **回線が細いときは壊れずに退化します** —— ライブ映像が張れなければ静止画に落ち、瞬断中の
  タイルは `unknown`(=「観測できていない」であって「空き」ではありません)になります。
  重いのは録画の回収です。

**実機をランナー機で使う場合**、端末と手元の経路は無関係です(ランナー機の LAN の話)。
USB なら設定は要りません。LAN 経由の iPhone は端末側が待ち受けて Mac が繋ぎに行くので、
Mac のファイアウォール設定は不要ですが、**ランナー機と端末が同じサブネットに居ること**と、
**アクセスポイントのクライアント隔離(プライバシーセパレータ)が切ってあること**が必要です。
