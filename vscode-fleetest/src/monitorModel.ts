...
// 純粋関数群。monitorPanel.ts と test/ の両方から同じロジックを使うための方針):
...
//   - monitorProfileForms.ts:     実行/アプリプロファイルのフォーム解析・検証・デバイスカタログ
// 拡張側の import はこのファイル経由でよい(このファイル自体は re-export のみで挙動を持たない)。
