...
export class MonitorProfilesController {
  ...
  private resolveProjectOrWarn(): string | undefined {
    ...
    if (resolution.kind !== "resolved") {
      void vscode.window.showWarningMessage(`fleetest: ${t("profiles.error.projectUnresolved")}`);
      return undefined;
    }
  ...
  async handleProfileAdd(): Promise<void> {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.runProfileAddFailed", { name })}`);
    }
  }
  ...
  async handleProfileCopy(source: string): Promise<void> {
    ...
    if (!fs.existsSync(sourcePath)) {
      void vscode.window.showWarningMessage(`fleetest: ${t("profiles.msg.runProfileNotFound", { name: source })}`);
      this.postProfileInfo();
      return;
    }
    const existing = listRunProfileNames(this.deps.workspaceRoot, project);
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.runProfileCopyFailed", { name })}`);
    }
  }

  /**
   * 「削除」ボタン: モーダル確認で「削除」が選ばれたときのみ削除する。削除対象が現在選択中の
   * プロファイル(fleetest.profile)であれば selectProfile("") で戻す。
   */
  async handleProfileDelete(name: string): Promise<void> {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.runProfileDeleteFailed", { name })}`);
    }
    this.postProfileInfo();
  }

  /**
   * 「✏」ボタン: runs/<name>.json をリネームする。fleetest.profile が旧名を指していた場合は
  ...
  async handleProfileRename(profile: string): Promise<void> {
    ...
    if (!fs.existsSync(oldPath)) {
      void vscode.window.showWarningMessage(`fleetest: ${t("profiles.msg.runProfileNotFound", { name: profile })}`);
      this.postProfileInfo();
      return;
    }
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.runProfileRenameFailed", { name: profile })}`);
    }
  }

  // ---- アプリプロファイルの追加/コピー/名前変更/削除(プロファイルタブ中段のアイコンボタン) --------
  // fleetest.* から直接参照されないため selectProfile 相当の追随は無い(壊れた参照の検出は
  ...
  async handleAppProfileAdd(): Promise<void> {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.appProfileAddFailed", { name })}`);
    }
  }
  ...
  async handleAppProfileCopy(source: string): Promise<void> {
    ...
    if (!fs.existsSync(sourcePath)) {
      void vscode.window.showWarningMessage(`fleetest: ${t("profiles.msg.appProfileNotFound", { name: source })}`);
      this.postProfileInfo();
      return;
    }
    const existing = listAppProfileNames(this.deps.workspaceRoot, project);
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.appProfileCopyFailed", { name })}`);
    }
  }

  /** 「削除」ボタン: モーダル確認で「削除」が選ばれたときのみ削除する(fleetest.* 設定への追従は不要)。 */
  async handleAppProfileDelete(name: string): Promise<void> {
    ...
    try {
      ...
      void vscode.window.showErrorMessage(`fleetest: ${t("profiles.msg.appProfileDeleteFailed", { name })}`);
    }
    this.postProfileInfo();
  }
  ...
  async handleAppProfileRename(profile: string): Promise<void> {
    ...
    if (!fs.existsSync(oldPath)) {
      void vscode.window.showWarningMessage(`fleetest: ${t("profiles.msg.appProfileNotFound", { name: profile })}`);
      this.postProfileInfo();
      return;
    }
