...
//   platform, file, segments }], clipsAttempted?, clipsFailed?, sourcesFailed? }(Sources/FTCore/
...
// - clipsAttempted/clipsFailed は任意(エンコーダ不調時の切り出し失敗を
...
//   true になる)。どのフィールドも欠落・型不一致は「無い」として扱う(呼び出し側が数値/真偽値
 ...
 * recordings は空配列でも通す(全滅 run の契約)。clipsAttempted/clipsFailed は
