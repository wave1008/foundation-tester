// 起動時プレフライト: `fleetest api version` を単発実行し、CLI と拡張のプロトコル版
...
import { FLEETEST_PROTOCOL_VERSION } from "./protocolVersion";
...
/**
 * `fleetest api version` の結果と FLEETEST_PROTOCOL_VERSION を比較し、不一致なら警告する。
 * バイナリが無い(未ビルド)場合と spawn 失敗は黙ってスキップする(fresh clone のセットアップ前に
 * 誤警告を出さないため)。activate() を1回だけ呼ぶ想定。
 */
export async function checkFleetestCompat(
  binaryPath: string,
  ...
  if (cliProtocol === FLEETEST_PROTOCOL_VERSION) {
    return;
  }
  if (cliProtocol < FLEETEST_PROTOCOL_VERSION) {
    void vscode.window.showWarningMessage(
      t("compat.mismatch.cliOld", { cli: String(cliProtocol), ext: String(FLEETEST_PROTOCOL_VERSION) }),
    );
    return;
  }
  void vscode.window.showWarningMessage(
    t("compat.mismatch.extOld", { cli: String(cliProtocol), ext: String(FLEETEST_PROTOCOL_VERSION) }),
  );
}
