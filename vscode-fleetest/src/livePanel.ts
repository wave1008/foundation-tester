// livePanel.ts
// 独立ライブ操作パネルの WebviewPanel(コマンド `fleetest.showLiveControl`)。ロジック本体は
...
//
// 開き方は2経路:
// - 手動: fleetest.showLiveControl コマンド(ステータスバー常駐ボタンからも)。既存パネルなら
...
const VIEW_TYPE = "fleetestLiveControl";
...
export class LivePanelController implements vscode.Disposable {
   ...
   * initLive() が refreshDevices を送るため不要)。fleetest.showLiveControl(手動)と runStarted
   * (自動、既存パネル時)の両方から使う。 */
...
export function registerLivePanel(
  ...
  statusItem.command = "fleetest.showLiveControl";
  statusItem.show();

  context.subscriptions.push(
    controller,
    statusItem,
    vscode.commands.registerCommand("fleetest.showLiveControl", () => controller.show(true)),
  );
