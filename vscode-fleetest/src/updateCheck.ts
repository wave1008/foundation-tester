...
// /fleetest-update に委ねる)。
//
// 入口は2つあり、**性格が違う**:
//   - checkFleetestUpdate(自動): activate から fire-and-forget(compatCheck.ts と同型)。
//     更新があるときだけ喋る。1日1回・却下済みの版は黙る。
//   - checkFleetestUpdateNow(手動: fleetest.checkForUpdate コマンド): 押されたら必ず実行し、
...
const STATE_LAST_CHECKED = "fleetest.updateCheck.lastCheckedAt";
const STATE_DISMISSED_HEAD = "fleetest.updateCheck.dismissedHead";
 ...
 */
export async function checkFleetestUpdate(deps: UpdateCheckDeps): Promise<void> {
...
/**
 * fleetest.checkForUpdate コマンドの実体。**間隔・却下・設定 off をすべて無視して実行し、
 * 必ず結果を返す**(最新でも「最新です」と答える)。自動チェックの静けさは自動側だけの性質。
 */
export async function checkFleetestUpdateNow(deps: Omit<UpdateCheckDeps, "enabled">): Promise<void> {
...
async function notifyAvailable(
  ...
  if (picked === open) {
    ...
    await vscode.commands.executeCommand("fleetest.showDeviceMonitor", "settings");
