...
// 設定タブ「常駐プロセス」一覧の中核。`ps -axo pid=,ppid=,state=,command=` の出力から fleetest 関連の
...
//
// 「fleetest 関連」の判定はコマンド文字列のみに依存する。実行時の kill 経路(monitorPanel.ts)は
// ここで得た pid を使うが、実際の停止は原則 `fleetest devices down`(confirmDeaths→simctl
...
export type ResidentType =
  | "bridge" // iOS ブリッジ本体: xcodebuild test-without-building(FleetestRunner の xctestrun)
  | "sim-runner" // シミュレータ内の XCUITest ランナー: FleetestRunnerUITests-Runner.app
  ...
  | "monitor" // fleetest api monitor(デバイスモニター常駐)
  | "host-metrics" // fleetest api host-metrics
  | "live-serve" // fleetest api live serve(ライブ操作の配信)
  | "stream" // fleetest-simstream / fleetest-androidstream(画面ストリーム helper)
  | "run" // fleetest api run / fleetest run(実行。非常駐だが孤児化するとブリッジを占有)
  | "mcp" // fleetest-mcp(MCP サーバ)
  | "fleetest"; // その他の fleetest CLI(上のどれにも当たらない常駐)
...
export interface ResidentProcess {
   ...
   *  bridge は xctestrun 名(FleetestRunner-<port>.xctestrun)から、sim-runner は自分のコマンドに
...
export interface ParseResidentOptions {
  ...
  /** fleetest のビルド成果物ディレクトリ(絶対パス、例 .../.build/debug)。この配下の実行ファイルは
   *  名前を問わず fleetest 由来として拾う(新規追加バイナリのもれ防止)。monitorPanel が config から渡す。 */
  readonly binaryDir?: string;
  /** in-app ブリッジの UDID(大文字)→ ポート。`.fleetest/bridge-<port>.inapp`(ファイル名=ポート、
...
function typeLabel(type: ResidentType, locale: Locale): string {
  ...
  return key ? rt(key, locale) : "fleetest";
}

// fleetest CLI をパス前置(相対/絶対)を問わずサブコマンド位置で判定する土台。fleetest-mcp /
// fleetest-simstream / fleetest-androidstream は "fleetest" の直後が "-" なので (?:\s|$) に当たらず、
// この正規表現には一致しない(それぞれ専用の前置チェックで先に捕まえる)。
const FLEETEST_CLI_RE = /(^|\/)fleetest(?:\s|$)/;

const SIM_UDID_RE = /CoreSimulator\/Devices\/([0-9A-Fa-f-]{36})/;

// iOS ブリッジ(xcodebuild)は BridgeLauncher が注入したポート専用の xctestrun を回す
// (Sources/FTBridgeClient/BridgeLauncher.swift の FleetestRunner-<port>.xctestrun)。この2つで
// ブリッジのポートと相手シミュレータの UDID を1本のコマンドから取れる(destination は
// "platform=iOS Simulator,id=<UDID>")。
const BRIDGE_XCTESTRUN_RE = /FleetestRunner-(\d+)\.xctestrun/;
...
export function classifyResident(
  ...
  // 1. helper バイナリ(専用プロセス名。fleetest CLI 判定より先に確定させる)
  if (/(^|\/)fleetest-mcp(?:\s|$)/.test(cmd)) {
    return { type: "mcp", detail: "" };
  }
  if (/(^|\/)fleetest-(?:sim|android)stream(?:\s|$)/.test(cmd)) {
    const serial = extractArg(cmd, "--serial") || extractArg(cmd, "--device") || extractArg(cmd, "--udid");
    return { type: "stream", detail: serial };
  }
  // 1b. その他の fleetest-<name> helper を汎用に拾う(専用判定の後。新規 helper のもれ防止)。
  //     detail に helper 名を出して種別「fleetest」の中で区別できるようにする。
  const genericHelper = cmd.match(/(^|\/)(fleetest-[A-Za-z0-9][A-Za-z0-9._-]*)(?:\s|$)/);
  if (genericHelper) {
    return { type: "fleetest", detail: genericHelper[2] ?? "" };
  }

  // 2. fleetest CLI サブコマンド
  if (FLEETEST_CLI_RE.test(cmd)) {
  ...
  // 3. iOS ブリッジ本体: FleetestRunner の xctestrun を回す xcodebuild
  if (/\bxcodebuild\b/.test(cmd) && /test-without-building/.test(cmd) && /FleetestRunner/.test(cmd)) {
  ...
  // 5. Android エミュレータ(qemu)。fleetest のフリートはここに含まれる。
  //    `fleetest devices down` が `pkill -f sdk/emulator/qemu` で全 qemu を落とすのと対象範囲を揃える。
...
export function describeParent(
  ...
  // 親自身が fleetest 関連なら種別ラベル(+識別子)で示す
...
/** ps 出力(`ps -axo pid=,ppid=,state=,command=`)から fleetest 関連の常駐プロセスを抽出し、
...
export function parseResidentProcesses(
  ...
  for (const line of psOutput.split("\n")) {
    ...
    // FleetestRunner-<port>.xctestrun を持つのは iOS ブリッジの xcodebuild のみ(誤検出しない)。
