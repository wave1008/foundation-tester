// debugAdapter.ts
// fleetest シナリオ用の Debug Adapter Protocol (DAP) 実装。
//
...
//
// スレッドは常に1本(THREAD_ID)。`fleetest api run --debug` は --scenario を1件だけ
// 受け付ける前提に合わせている(Sources/fleetest/ApiRunCommand.swift)。
//
...
type FleetestProcess = ChildProcessByStdio<Writable, Readable, Readable>;

export interface FleetestDebugSessionOptions {
  /** fleetest CLI バイナリの絶対パス。 */
  binaryPath: string;
...
export interface FleetestLaunchRequestArguments extends DebugProtocol.LaunchRequestArguments {
  project: string;
  scenario: string;
  dryRun?: boolean;
  stopOnEntry?: boolean;
...
export class FleetestDebugSession extends DebugSession {
  ...
  protected override launchRequest(
    ...
    args: FleetestLaunchRequestArguments,
  ): void {
  ...
  protected override stackTraceRequest(
    ...
    const name = paused.description ?? paused.scenario ?? "fleetest";
  ...
  protected override threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    response.body = { threads: [new Thread(THREAD_ID, "fleetest scenario")] };
    this.sendResponse(response);
  }
  ...
  private startDebuggee(): void {
    ...
    // --profile と --platform/--port/--serial は fleetest api run 側で同時指定不可なので、
  ...
  private handleNdjsonValue(value: unknown): void {
    ...
    if (value.kind === "scenarioFinished") {
      ...
      this.sendEvent(new Event("fleetest.scenarioFinished", body));
    }
  }
