...
/** モニターのデバイスタイル表示範囲(設定 fleetest.monitorDeviceFilter)。既定は "all"。 */
export type MonitorDeviceFilter = "all" | "running";

/** 起動時の更新チェック(設定 fleetest.updateCheck)。既定は "auto"。 */
export type UpdateCheckMode = "auto" | "off";

export interface FleetestConfig {
  ...
  liveFps: number;
  /** iOS シミュレータのライブ映像ストリーミング(fleetest-simstream)を使うか。true でも helper が
   * 未ビルド(resolveSimStream が undefined)なら自動でポーリングにフォールバックする。 */
  iosStreamEnabled: boolean;
  /** Android 実機/エミュレータのライブ映像ストリーミング(fleetest-androidstream)を使うか。
...
export function readConfig(workspaceRoot: string): FleetestConfig {
  const configuration = vscode.workspace.getConfiguration("fleetest");
  const rawBinaryPath = configuration.get<string>("binaryPath", ".build/debug/fleetest");
...
/** binaryPath を見つけた fleetest-simstream のパスをキーにキャッシュ(見つかった正の結果のみ)。 */
const simStreamCache = new Map<string, string>();

/**
 * fleetest バイナリと同じディレクトリにある fleetest-simstream(iOS ライブ映像 helper)の絶対パス。
 * 実行可能ファイルが無ければ undefined(呼び出し側はポーリングにフォールバック)。
 * 正の結果だけキャッシュする(未検出はキャッシュしない=後から helper をビルドすれば Reload 無しで有効化される)。
 */
export function resolveSimStream(config: FleetestConfig): string | undefined {
  const cached = simStreamCache.get(config.binaryPath);
  if (cached !== undefined) {
    return cached;
  }
  const candidate = path.join(path.dirname(config.binaryPath), "fleetest-simstream");
...
/** binaryPath を見つけた fleetest-devicepoll のパスをキーにキャッシュ(resolveSimStream と同じ方針)。 */
const devicePollCache = new Map<string, string>();

/**
 * fleetest バイナリと同じディレクトリにある fleetest-devicepoll(**実機**のライブ映像 helper)の絶対パス。
 * 実機はスクリーンショットのポーリングで配信する: iOS 実機は simstream が CoreSimulator 私有 API の
 * ため不可、Android 実機は screenrecord だと静止画面でフレームが流れない(Sources/fleetest-devicepoll
 * の冒頭コメント参照)。resolveSimStream と同じ方針(未検出は undefined・正の結果のみキャッシュ)。
 */
export function resolveDevicePoll(config: FleetestConfig): string | undefined {
  const cached = devicePollCache.get(config.binaryPath);
  if (cached !== undefined) {
    return cached;
  }
  const candidate = path.join(path.dirname(config.binaryPath), "fleetest-devicepoll");
  if (isExecutableFile(candidate)) {
...
/** binaryPath を見つけた fleetest-androidstream のパスをキーにキャッシュ(resolveSimStream と同じ方針)。 */
const androidStreamCache = new Map<string, string>();

/**
 * fleetest バイナリと同じディレクトリにある fleetest-androidstream(Android ライブ映像 helper)の絶対パス。
 * resolveSimStream と同じ方針(実行可能ファイルが無ければ undefined、正の結果のみキャッシュ)。
 */
export function resolveAndroidStream(config: FleetestConfig): string | undefined {
  const cached = androidStreamCache.get(config.binaryPath);
  if (cached !== undefined) {
    return cached;
  }
  const candidate = path.join(path.dirname(config.binaryPath), "fleetest-androidstream");
  if (isExecutableFile(candidate)) {
 ...
 * /opt/homebrew/bin→/usr/local/bin(Sources/fleetest-androidstream/main.m・FTAndroid/AndroidDriver.swift の
