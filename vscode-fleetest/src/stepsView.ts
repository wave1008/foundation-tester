// stepsView.ts
// 読み取り専用の「ステップ一覧」TreeView(ビュー id: fleetestSteps)。
//
// `fleetest api steps --project <P> --scenario <ID>` を叩いて dry-run 相当のステップ表を取得し、
// stepsModel.buildStepTree()(vscode 非依存)でノードモデルに変換して表示する。
// 表示対象シナリオはアクティブエディタ追従(既定)またはコマンド `fleetest.showSteps` で決まる。
//
...
const OPEN_STEP_LOCATION_COMMAND = "fleetest.stepsView.openStepLocation";
...
export function registerStepsView(
  ...
  const treeView = vscode.window.createTreeView("fleetestSteps", {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("fleetest.showSteps", (item?: vscode.TestItem) => {
      if (!item) {
        return;
      }
      ...
      provider.setScenario(scenario.id, project);
      void Promise.resolve(vscode.commands.executeCommand("fleetestSteps.focus")).then(
        undefined,
        () => {
    ...
    vscode.commands.registerCommand("fleetest.refreshSteps", () => {
  ...
  getTreeItem(element: ViewNode): vscode.TreeItem {
    switch (element.type) {
      ...
      case "scene": {
        ...
        item.contextValue = "fleetestStepsScene";
        return item;
      }
      case "step": {
        ...
        item.contextValue = "fleetestStepsStep";
        item.command = {
