// testTree.ts
// fleetest のシナリオを VS Code TestController ツリーとして表示する。
...
import { type CliResult, CliSupersededError, type FleetestCli } from "./cli";
...
export class FleetestTestTree implements vscode.Disposable {
  ...
  constructor(
    ...
    this.controller = vscode.tests.createTestController("fleetest", "fleetest");
  ...
  private reportInvocationError(error: unknown, binaryPath: string): void {
    ...
    this.outputChannel.appendLine(`[fleetest] ${message}`);
