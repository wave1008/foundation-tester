...
export class MonitorDeviceStreamController {
  ...
  applyDevices(devices: readonly MonitorDevice[]): void {
    ...
    const showStreamDuringRun = this.deps.isShowStreamDuringRun();
    ...
    for (const device of devices) {
      ...
      // 「配信を表示する」が ON のとき、occupiedMachines は自分の run のぶんを含まず inRun も見ない。
      ...
      if (!showStreamDuringRun && device.inRun === true) {
        // 「配信を表示する」が ON(既定)なら run 中も配信を続ける(利用者の選択。配信を張ったままの run は赤くなり得る =
