...
import { type CliResult, FleetestCli } from "./cli";
import { checkFleetestCompat } from "./compatCheck";
import {
  type FleetestConfig,
...
import { FleetestTestTree, unhideAllTests } from "./testTree";
import { checkFleetestUpdate, checkFleetestUpdateNow } from "./updateCheck";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const rawOutputChannel = vscode.window.createOutputChannel("fleetest");
  ...
  if (!hasProjectsDirectory(workspaceRoot)) {
    // fleetest のテストプロジェクトを持たないリポジトリでは登録しない。
  ...
  void checkFleetestCompat(getConfig().binaryPath, workspaceRoot, outputChannel, (proc) => {
    ...
    outputChannel.appendLine(`[fleetest] ${error instanceof Error ? error.message : String(error)}`);
  });

  // upstream の更新有無の確認(updateCheck.ts)。同じく fire-and-forget。通知するだけで取り込まない。
  void checkFleetestUpdate({
    workspaceRoot,
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (!e.affectsConfiguration("fleetest.language")) {
        return;
      }
...
function registerTestTree(
  ...
  const testTree = new FleetestTestTree(cli, () => workspaceRoot, getConfig, outputChannel);
...
function registerCommands(
  ...
  context.subscriptions.push(
    // 対象プロジェクト切替(fleetest.selectProject コマンド、または設定の直接編集)に追従。
    ...
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (!e.affectsConfiguration("fleetest.showOnlyFailedTests")) {
        return;
      }
      syncFailedFilterContext();
    ...
    vscode.commands.registerCommand("fleetest.enableFailedTestsFilter", () => {
      void setFailedFilter(true);
    }),
    vscode.commands.registerCommand("fleetest.disableFailedTestsFilter", () => {
      void setFailedFilter(false);
    }),
    vscode.commands.registerCommand("fleetest.refreshScenarios", () => {
      void testTree.refresh();
    }),
    // 手動の更新確認。自動チェック(activate 冒頭)と違い、間隔も却下も設定 off も無視して必ず答える。
    vscode.commands.registerCommand("fleetest.checkForUpdate", () => {
      void checkFleetestUpdateNow({
        workspaceRoot,
        outputChannel,
        globalState: context.globalState,
        registerChild: (proc) => {
    ...
    // fleetest api delete-scenario。テストアイテム id は class:<folder>/<class> か <Class>.<Method>。
    vscode.commands.registerCommand("fleetest.deleteScenario", async (item?: vscode.TestItem) => {
    ...
    vscode.commands.registerCommand("fleetest.selectProject", async () => {
      ...
      // 設定更新で onDidChangeConfiguration("fleetest.project") が発火し、そちらで refresh する。
      await vscode.workspace
        .getConfiguration("fleetest")
    ...
    vscode.commands.registerCommand("fleetest.selectProfile", async () => {
      const config = getConfig();
      const resolution = resolveProjectName(workspaceRoot, config);
      if (resolution.kind !== "resolved") {
