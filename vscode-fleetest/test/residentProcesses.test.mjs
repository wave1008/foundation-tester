...
test("classifyResident: 各種別を正しく判定する", () => {
  const cases = [
    [
      "/Applications/Xcode_27_beta_3.app/Contents/Developer/usr/bin/xcodebuild test-without-building -xctestrun /Users/w/foundation-tester/.fleetest/DerivedData/Build/Products/FleetestRunner_iphonesimulator27.0.xctestrun -destination id=2C4FBE2E-5358-48CD-B32A-964B8EB817B4",
      "bridge",
    ],
    [
      "/Users/w/Library/Developer/CoreSimulator/Devices/E38DCA93-95F2-4DDF-B1FE-29527205D3EE/data/Containers/Bundle/Application/99534FFE/FleetestRunnerUITests-Runner.app/FleetestRunnerUITests-Runner",
      "sim-runner",
    ],
    [
    ...
    [".build/debug/fleetest api run --project SampleApp --scenario Foo", "run"],
    [".build/debug/fleetest-mcp", "mcp"],
    [".build/debug/fleetest bridge status --port 8123", "fleetest"],
  ];
...
test("parseResidentProcesses: ps 出力(pid ppid state command)を分類・整列して抽出する", () => {
  const psOutput = [
    "  222     1 S   .build/debug/fleetest-mcp",
    "  111  4321 S+  .build/debug/fleetest api monitor --project SampleApp --interval 1",
...
test("classifyResident: 新規 fleetest-<name> helper を汎用に fleetest 種別で拾う", () => {
  const r = classifyResident(".build/debug/fleetest-newhelper --serve");
  assert.ok(r);
  assert.equal(r.type, "fleetest");
  assert.equal(r.detail, "fleetest-newhelper"); // helper 名を識別子に出す
  // 専用 helper は従来どおり個別種別が勝つ
  assert.equal(classifyResident(".build/debug/fleetest-mcp").type, "mcp");
  assert.equal(classifyResident(".build/debug/fleetest-androidstream --serial x").type, "stream");
});
...
test("classifyResident: in-app ブリッジ(注入アプリ)を .inapp の UDID で拾う", () => {
  ...
  const runnerCmd = `/x/CoreSimulator/Devices/${udid}/data/Containers/Bundle/Application/G/FleetestRunnerUITests-Runner.app/FleetestRunnerUITests-Runner`;
...
test("parseResidentProcesses: ポートを bridge=xctestrun名 / sim-runner=同一UDIDのbridge / inapp=.inapp から解決する", () => {
  ...
  const psOutput = [
    // 実機採取形式: -xctestrun .../FleetestRunner-<port>.xctestrun -destination platform=iOS Simulator,id=<UDID>
    `  36328     1 S /Applications/Xcode.app/Contents/Developer/usr/bin/xcodebuild test-without-building -xctestrun /Users/w/foundation-tester/.fleetest/DerivedData/Build/Products/FleetestRunner-8128.xctestrun -destination platform=iOS Simulator,id=${udid}`,
    `  36795 35891 S /Users/w/Library/Developer/CoreSimulator/Devices/${udid}/data/Containers/Bundle/Application/AA/FleetestRunnerUITests-Runner.app/FleetestRunnerUITests-Runner`,
    ...
    "  50000     1 S .build/debug/fleetest-mcp",
  ].join("\n");
  const got = parseResidentProcesses(psOutput, {
...
test("parseResidentProcesses: 親PID を launchd_sim 経由でシミュレータ名に解決する", () => {
  ...
  const psOutput = [
    ...
    `  40000 35891 S /Users/w/Library/Developer/CoreSimulator/Devices/${udid}/data/Containers/Bundle/Application/AA/FleetestRunnerUITests-Runner.app/FleetestRunnerUITests-Runner`,
    "  50000     1 S .build/debug/fleetest-mcp",
  ].join("\n");
  const got = parseResidentProcesses(psOutput, {
...
test("parseResidentProcesses: シミュレータ名が未知なら UDID 短縮にフォールバック", () => {
  ...
  const psOutput = [
    ...
    `  200   100 S /x/CoreSimulator/Devices/${udid}/y/FleetestRunnerUITests-Runner.app/FleetestRunnerUITests-Runner`,
  ].join("\n");
...
test("parseResidentProcesses: state Z / <defunct> をゾンビとして立てる", () => {
  const psOutput = [
    "  555     1 Z   .build/debug/fleetest-mcp",
    "  666  4321 S   .build/debug/fleetest api host-metrics --interval 1 <defunct>",
    "  777  4321 S   .build/debug/fleetest api monitor --project SampleApp",
  ].join("\n");
