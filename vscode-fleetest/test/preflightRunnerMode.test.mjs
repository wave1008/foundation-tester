...
//   3. --runner を受け付ける引数解析と、--base の既定値 ~/fleetest-runner がある
//      (Sources/fleetest/Fleetest.swift 等の --remote-dir 既定と揃える契約。CLAUDE.md 参照)。
//
...
test("引数解析: --runner を受け付け、--base の既定値が ~/fleetest-runner である", () => {
  const source = readFileSync(PREFLIGHT_PATH, "utf8");
  ...
  assert.match(
    source,
    /BASE=(["'])~\/fleetest-runner\1/,
    `${PREFLIGHT_SH} の --base 既定値が "~/fleetest-runner" になっていません` +
