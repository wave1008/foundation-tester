...
test("run profile changes restart only for the selected profile", () => {
  assert.equal(monitorRestartNeeded("ios-basic", "ios-basic"), true);
  assert.equal(monitorRestartNeeded("android", "ios-basic"), false);
  assert.equal(monitorRestartNeeded("ios-basic", ""), true, "選択中の実行プロファイルが無いときは全実行プロファイルの和集合が対象");
});
...
test("hand edits always restart; form saves restart only on scope change or right after a hand edit", () => {
  ...
  assert.equal(change(true, false, "k", "k"), false, "フォームの保存で devices が同じなら再起動しない");
