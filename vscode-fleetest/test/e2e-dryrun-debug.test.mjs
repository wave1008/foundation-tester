...
// FleetestDebugSession(src/debugAdapter.ts)を実バイナリ(.build/debug/fleetest)相手に駆動する
// エンドツーエンドテスト。node:test で実行するが、実バイナリ・実リポジトリ(SampleApp)に
// 依存するため、FLEETEST_E2E=1 が設定されているときだけ実行する(それ以外は skip され、
...
const RUN_E2E = process.env.FLEETEST_E2E === "1";
...
test(
  ...
  { skip: !RUN_E2E && "FLEETEST_E2E=1 のときだけ実行します" },
  async () => {
    ...
    try {
      ...
      const finishedEvent = await driver.waitForEvent("fleetest.scenarioFinished", 30000);
