...
//   vscode-fleetest/src/webview/monitor/hostChartScale.js HM_VISION_MAX_RATE
//
...
test("VN グラフの下限が Swift 側の upscaleLadder の段数と一致する", () => {
  ...
  const chartsMatch = charts.match(/HM_VISION_MAX_RATE\s*=\s*(\d+)/);
...
test("FM と VN は同じ入力でも下限が違えば結果が違う", async () => {
  ...
  assert.equal(hmCountScale([0, 1], HM_VISION_MAX_RATE), HM_VISION_MAX_RATE);
});
