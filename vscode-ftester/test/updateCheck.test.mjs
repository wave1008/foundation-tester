...
function fakeClone(verdictLines) {
  const root = mkdtempSync(path.join(tmpdir(), "ftester-clone-"));
...
test("checkFtesterUpdate: スクリプトを実行し、最終チェック時刻をクローン単位で記録する", async () => {
  const root = fakeClone("verdict=up-to-date");
  const globalState = fakeState();
  await checkFtesterUpdate({
    workspaceRoot: root,
    enabled: true,
...
test("checkFtesterUpdate: off・未導入・間隔内では実行も記録もしない", async () => {
  const root = fakeClone("verdict=up-to-date");
  ...
  const lonely = mkdtempSync(path.join(tmpdir(), "ftester-noclone-"));
  await checkFtesterUpdate({
  ...
  const key = `ftester.updateCheck.lastCheckedAt:${root}`;
...
test("resolveToolRoot: clone 構成 / 外部構成 / 未導入", () => {
  const base = mkdtempSync(path.join(tmpdir(), "ftester-toolroot-"));
