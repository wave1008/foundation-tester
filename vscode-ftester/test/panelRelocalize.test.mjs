...
// 各 webview パネルの relocalize()(ftester.language 変更時に extension.ts が呼ぶ)の実行時検証。
...
const getConfig = () => ({ binaryPath: "/usr/local/bin/ftester", project: "P", profile: "" });

function newLiveController() {
  const cli = new FtesterCli(outputChannel);
  // FtesterTestTree(../src/testTree)はモジュール読み込み時に vscode.TestTag(...) を呼ぶため、
