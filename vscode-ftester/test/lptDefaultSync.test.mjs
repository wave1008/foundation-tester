...
//
// 同期相手:
//   Sources/ftester/LPTOrdering.swift  defaultHistoryRuns   … CLI の既定
//   vscode-ftester/package.json        ftester.lptHistoryRuns.default … 設定の既定
//   vscode-ftester/src/monitorPanel.ts sendInitialState の default … 設定タブの初期値
//
// process.cwd() は npm test 実行時に vscode-ftester ルート。
...
test("LPT の実績 run 数の既定値が Swift / package.json / 拡張で一致する", () => {
  const swiftSource = readFileSync(
    path.join(REPO, "Sources/ftester/LPTOrdering.swift"), "utf8");
  ...
  const configDefault =
    pkg.contributes?.configuration?.properties?.["ftester.lptHistoryRuns"]?.default;
  assert.equal(configDefault, swiftDefault,
    "package.json の ftester.lptHistoryRuns.default が Swift 側とズレています");
