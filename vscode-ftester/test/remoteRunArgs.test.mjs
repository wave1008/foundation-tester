...
const HOSTS = [
  { name: "mac-01", host: "user@mac-01", dir: "" },
  { name: "mac-02", host: "mac-02", dir: "/Users/ci/ftester-runner" },
  { name: "broken", host: "", dir: "" },
];
...
test("normalizeRemoteHosts: name 空なら host を name に補完", () => {
  assert.deepEqual(
    ...
    [{ name: "user@mac-01", host: "user@mac-01", dir: "" }],
  );
});
...
test("normalizeRemoteHosts: 型不正フィールドは空文字扱い(dir/host が string でない)", () => {
  assert.deepEqual(
    normalizeRemoteHosts([{ name: "x", host: 123, dir: null }]),
    [{ name: "x", host: "", dir: "" }],
  );
});
