...
export const panelsStrings = {
  ...
  "panels.settings.updateHint": {
    ja: "更新があるときは右上に「更新する」が出ます。更新は git pull・再ビルド・拡張の再インストールまで行い(数分)、完了後はウィンドウの再読み込みが必要です。",
    en: "When an update is available, an \"Update now\" button appears at the top right. Updating runs git pull, rebuild, and extension reinstall (a few minutes), and requires a window reload afterwards.",
  },
