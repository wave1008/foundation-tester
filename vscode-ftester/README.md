...
- **更新する**: **更新があるときだけタブバーの右端に現れます**(どのタブを見ていても目に入る位置。
  押せないボタンを常時見せない)。確認ダイアログのあと `<TOOL_ROOT>/Scripts/update.sh` を実行し、
  設定タブへ切り替えて**出力を1行ずつ流します**(git pull → 再ビルド → 拡張の再インストール →
  プラグイン更新。数分)。完了後は **Developer: Reload Window** が必要です(拡張自身を入れ替える
  ため、それまでは旧版が動きます)。
