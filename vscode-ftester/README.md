...
- ライブ操作(`src/livePanel.ts` / `src/liveModel.ts`)。コマンド **「ftester: ライブ操作を表示」**
  ...
  起動/終了/インストールを `ftester api list-devices` / `ftester api live <sub>` 経由で行う
  (詳細は下記「ライブ操作」)
...
- **上部のデバイスセレクタ**: パネルを開くと `ftester api list-devices --project <project>` を
  ...
  1件にフォールバックし、上部にエラーバナーで理由を表示します。
- **左: スクリーンショット**: 「更新」ボタンで `ftester api live snapshot` を呼び、取得した
  JPEG 画像(base64)を表示します。**画像をクリックするとその位置をタップします**
  (クリック位置を画像の表示pxからデバイスのポイント座標へ比例変換して
  `ftester api live tap --x --y` を呼ぶ。詳細は下記「座標変換について」)。未取得時はプレースホルダー
  が表示され、「更新」を促します。
- **右: 操作パネル**(GUI 版 `LiveView` の `controlPane` と同構成):
  - **アプリ**: bundle ID / パッケージ名の入力欄 + 「起動」(`api live launch`)・
    「終了」(`api live terminate`)。
  - **インストール**: iOS(`.app`)・Android(`.apk`)それぞれのパス入力欄 + 「選択...」
    ...
    (`api live install --path`。パスが空、またはファイルが存在しない場合は CLI を呼ばずに
    エラー表示します)。
  - **「更新」ボタン**: snapshot を再取得します。
  - **スワイプ**: ↑↓←→ ボタン(`api live swipe --direction`)。
  - **テキスト入力欄 + 「入力」**(`api live type --text`): 要素一覧でタップした要素があれば
    `--ref` を付けて送信します(未選択、または snapshot を再取得すると選択状態はクリアされ、
    その場合はフォーカス中の要素に入力されます)。
  - **要素一覧**: `[ref] type「label」id=identifier =value` 形式(GUI 版 `elementLine` と同じ。
    空フィールドは省く)。**クリックでその要素をタップします**(`api live tap --ref`)。
    行にマウスを乗せると、画像上に該当要素の `frame`(ポイント座標)を表示pxに変換した枠を
    オーバーレイ表示します。
  - **エラー表示欄**: 直近の `{"ok":false,"error":...}` のエラーメッセージを表示します。
- **操作後の自動更新**: タップ/スワイプ/入力/起動/終了/インストールのいずれかが成功すると、GUI 版
  `AppModel.liveAction` と同じく 700ms 待ってから自動的に snapshot を再取得します(失敗時は
  再取得せず、エラー表示欄にエラーを出すだけです)。操作中は全ての操作ボタン・デバイスセレクタが
  無効化され、「処理中...」と表示されます。

### 座標変換について

`ftester api live snapshot` の `screen`(デバイス全体のサイズ)と各要素の `frame` はいずれも
**ポイント座標**です。スクリーンショット画像は screen のアスペクト比のままレターボックス無く
...
`ftester api list-devices`/`ftester api live <sub>` は、Test Explorer の「実行」等が使う
...
受け入れられません。一方 `api live`/`api list-devices` はいずれもドライバ直叩きの操作で
`swift build` を一切行わないため、専用 spawn にしても実行中の `swift build` と競合する心配は
ありません(`src/monitorPanel.ts` が `devicesUp`/`devicesDown` を専用 spawn しているのと同じ方針)。
...
│   ├── liveModel.ts             # `ftester api list-devices`/`ftester api live <sub>` の検証・座標変換・要素行フォーマット・CLI引数組み立て・webviewメッセージプロトコル(vscode 非依存)
│   ├── livePanel.ts             # ライブ操作の WebviewPanel(ftesterLiveControl)。list-devices/live の専用spawn(FtesterCliのキューを使わない)・座標変換の適用・700ms後の自動snapshot再取得
...
└── test/
    ...
    ├── liveModel.test.mjs       # liveModel のユニットテスト(検証・座標変換・要素行フォーマット・CLI引数組み立て・webviewメッセージプロトコル) + mock-live を使った統合テスト + 実バイナリ(存在すれば)確認
    ...
    └── fixtures/
        ...
        ├── mock-live.mjs         # `ftester api list-devices`/`ftester api live <sub>` を模したダミースクリプト(--fail で ok:false を模せる)
