...
- デバイスモニター(`src/monitorPanel.ts` / `src/monitorModel.ts`)。コマンド
  **「ftester: デバイスモニターを表示」**で開く Webview パネルで、`ftester api monitor` を
  ...
  `ftester.autoRepairBridge`/`ftester.autoRepairDeviceHealth`)も動く
- ライブ操作(`src/monitorLiveController.ts` / `src/liveModel.ts`)。独立した Webview パネル
  (`src/livePanel.ts`。コマンド **「ftester: ライブ操作を表示」**(`ftester.showLiveControl`)、
  view type `ftesterLiveControl`)で、画像上のタップ/ドラッグ(スワイプ)/長押し・要素一覧タップ・
  テキスト入力・ホーム/タスク切替の操作と、操作を記録して Swift シナリオを生成するレコーディング
  (`ftester api gen-scenario`)を、`ftester api list-devices`(デバイス一覧のみワンショット)と
  `ftester api live serve`(常駐プロセス。選択デバイスごとに1つ spawn し、NDJSON でコマンド送信・
  観測イベント受信を行う)経由で行う。設定 `ftester.liveControlOnRun`(既定 `true`)が有効な間は
...
- 結果ダッシュボード(`src/dashboardPanel.ts` / `src/dashboardModel.ts`)。コマンド
  **「ftester: 結果ダッシュボードを開く」**(`ftester.showResultsDashboard`)で開く独立 Webview
  パネルで、`ftester api results --project <名> ...` の集計結果(シナリオ別成功率・不安定
...
- 失敗テストの再実行・レポート導線(`src/lastResults.ts` / `src/lastResultsSync.ts` /
  ...
  **「失敗したテストを再実行」**(`ftester.rerunFailedTests`)・**「名前をコピー」**
  (`ftester.copyTestName`)・**「レポートを開く」**(`ftester.openScenarioReport`、実行結果の
  Markdown レポートを開く)・Test Explorer ツールバーの **「失敗したテストのみ表示」**
  (`ftester.enableFailedTestsFilter`/`ftester.disableFailedTestsFilter`、設定
  `ftester.showOnlyFailedTests` と同期)を提供する
...
| `ftester.streamCodec` | `"h264"` \| `"mjpeg"` | `"h264"` | デバイス画面ストリーミングのコーデック。`h264`(既定)は WebCodecs によるハードウェアデコードを使う。問題が出る環境では `mjpeg`(従来方式)に切り替える |
| `ftester.liveControlOnRun` | boolean | `true` | テスト実行(Run Test)開始時に、ライブ操作パネルをエディタの右側へ自動表示する |
| `ftester.autoRepairBridge` | boolean | `true` | ブリッジ無応答(接続済みだったデバイスが起動中のまま復帰しない状態が続く)を検出したら、実行中のレーンが無い間に限り device-up で自動修復を試みる |
...
- **実行中のインジケーター**: 状態行とタブバーのボタンにスピナーが出て、VSCode の進捗通知にも
  現在の工程(`==>` の見出し行)が出ます。**詳細ログは OUTPUT の ftester チャンネル**へ出力し、
...
拡張の UI 文字列は日英切替対応です。`ftester.language`(auto/ja/en)で切り替えられ、コマンド名・
...
- パネルを開くと `ftester api monitor --project <project> --interval <秒> --max-width <max-width>` を
  裏で起動し(`<秒>`=`ftester.monitorInterval`、`<max-width>`=`ftester.monitorMaxWidth`(既定 960))、
...
`ftester api device-catalog`(利用可能な機種/OSの一覧取得)・`ftester api installed-devices`
(導入済みシミュレータ/AVDの一覧取得)・`ftester api create-device`(新規デバイス作成)を
...
- **ブリッジ無応答の自動修復**(`src/monitorBridgeWatchdog.ts`、設定 `ftester.autoRepairBridge`
...
- **Android ゲスト OS 健全性の自動修復**(`src/monitorHealthWatchdog.ts`、設定
  `ftester.autoRepairDeviceHealth` 既定 `false`): Wi-Fi 無効化・時計のずれ等の異常が Swift 側の
...
独立した Webview パネル(`src/livePanel.ts`。コマンド `ftester.showLiveControl`、view type
`ftesterLiveControl`)。ロジック本体はデバイスモニターと共通の `src/monitorLiveController.ts`、
webview 資産は `src/webview/live/main.js`(UI 本体はデバイスモニターと共有する
`src/webview/monitor/liveTab.js` をそのまま呼び出す)です。macOS GUI 版(`ftester-gui`)の
「ライブ操作」タブ(`Sources/ftester-gui/LiveView.swift` + `AppModel.swift` の
`refreshLive`/`liveAction`)に相当する機能です。デバイスモニターパネルとは別パネル・別シングルトン
ですが、ポーリングモード設定(`workspaceState` の `monitor.pollingMode`)だけは共有します。

パネルを開く経路は3通りあります。

- コマンドパレットから **「ftester: ライブ操作を表示」** を実行する(既に開いている場合は前面に
  出るだけ)
- エディタ左下のステータスバーに常駐するボタンをクリックする
- デバイスモニターパネルでタイルを右クリックして**「ライブ操作」**を選ぶ(パネルが開き、
  対象デバイスを選択したうえで接続済みなら画面も取得します)

さらに設定 `ftester.liveControlOnRun`(既定 `true`)が有効な間は、Test Explorer の「実行」
...
のと同じ方針)。例外として `ftester api gen-scenario`(レコーディング終了時の生成)は生成コードの
ビルド検証(`swift build`)を伴うため、あえて `FtesterCli` の直列キュー経由で実行します
(詳細は上記「レコーディングとシナリオ生成」)。

## 結果ダッシュボード

コマンドパレットから **「ftester: 結果ダッシュボードを開く」**(`ftester.showResultsDashboard`)を
実行すると開く独立 Webview パネルです(`src/dashboardPanel.ts`。monitorPanel.ts/healReviewPanel.ts
と同じ、1拡張につきパネル1枚のシングルトン)。

- データは `ftester api results --project <project> --since 90d --min-runs 3` を1発叩いて得る
  1行 JSON(`src/dashboardModel.ts` の `ApiResultsPayload`。ビルドを伴わない読み取り専用コマンドなので
  `FtesterCli` の直列キューには乗せず `src/oneShotCli.ts` の `runOneShot()` で単発 spawn します)。
...
35. デバイス(シミュレータ/エミュレータ)を `ftester devices up` 等で起動した状態で、
    コマンドパレットから **「ftester: ライブ操作を表示」** を実行する。独立したライブ操作パネルが
...
| `npm test` | `esbuild.mjs --tests` で `test/*.test.mjs` を `out-test/` にバンドルし、`node --test out-test/*.test.mjs` で実行する(NDJSON パーサ・実行結果 reducer(並列実行ケース含む)・ログレーン変換・DAP アダプタ・デバイスモニター(タイル個別起動/停止含む)/ライブ操作/プロファイル/i18n の変換・検証のユニットテスト。E2E テストは `FTESTER_E2E` 未設定時は自動的に skip される) |
...
│   ├── binaryPathResolve.ts     # ftester バイナリパスの解決(clone 構成: .build/debug/ftester、外部パッケージ構成: PATH。vscode 非依存)
│   ├── compatCheck.ts           # 起動時プレフライト。`ftester api version` を単発実行し CLI↔拡張のプロトコル版を照合
│   ├── protocolVersion.ts       # 拡張側のプロトコル版定数(Sources/FTCore/ProtocolVersion.swift と一致必須)
│   ├── orphanSweep.ts           # activate 時、孤児化した ftester 常駐プロセス(PPID=1)を掃除する(vscode 非依存)
│   ├── residentProcesses.ts     # 設定タブ「常駐プロセス」一覧の中核。ps 出力から ftester 関連プロセスを分類(vscode 非依存)
...
│   ├── lastResults.ts            # .ftester/last-results/ の読み取り(Swift 側 LastResultsStore.swift と対。vscode 非依存)
│   ├── lastResultsSync.ts        # ターミナルからの `ftester run` 実行分を last-results から Test Explorer アイコンへ反映
│   ├── scenarioReports.ts        # シナリオ実行レポート(Markdown)の探索(vscode 非依存)
│   ├── reportCodeLens.ts         # 失敗テストの「レポートを開く」を CodeLens として常設表示
│   ├── copyTestName.ts           # ftester.copyTestName のキーボード起点時、カーソル位置から対象 TestItem を逆引き(vscode 非依存)
│   ├── monitorModel.ts           # `ftester api monitor` の NDJSON → webview メッセージへの変換・検証(vscode 非依存)
│   ├── monitorPanel.ts           # デバイスモニターの WebviewPanel(ftesterMonitor)。monitor プロセスの spawn/中継・サブコントローラの束ね役
...
│   ├── monitorDeviceStreamController.ts # タイル向け画面ストリーミング制御(iOS: ftester-simstream / Android: ftester-androidstream)
...
│   ├── monitorBridgeWatchdog.ts  # ブリッジ無応答の自動検出・device-up による自動修復(設定 ftester.autoRepairBridge。vscode 非依存)
│   ├── monitorHealthWatchdog.ts  # Android ゲストOS異常の自動検出・Wi-Fi再有効化/再起動による自動修復(設定 ftester.autoRepairDeviceHealth。vscode 非依存)
...
│   ├── monitorLiveController.ts  # ライブ操作の中核サブコントローラ。list-devices の専用spawn+live serve の常駐spawn(いずれもFtesterCliのキューを使わない)・座標変換の適用・serveの観測イベント反映・画面ストリーミング/自動フレームの供給元切替・レコーディング→gen-scenario
│   ├── livePanel.ts              # 独立ライブ操作パネルの WebviewPanel(ftesterLiveControl)。コマンド ftester.showLiveControl・ステータスバー常駐ボタン・実行開始時の自動オープン(ftester.liveControlOnRun)
│   ├── livePanelHtml.ts          # ライブ操作パネルの webview HTML 生成(本文は #panel-live。CSS/座標変換ロジックはデバイスモニターと共用)
│   ├── dashboardModel.ts         # 結果ダッシュボードの vscode 非依存の型・ペイロード型ガード(`ftester api results` の契約)
│   ├── dashboardPanel.ts         # 結果ダッシュボードの WebviewPanel(ftesterResultsDashboard)。コマンド ftester.showResultsDashboard
│   ├── i18n/
│   │   ├── index.ts              # 拡張側 i18n ランタイム。t()・initI18n()・locale 解決(ftester.language)
