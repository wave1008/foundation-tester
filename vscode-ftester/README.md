...
- ライブ操作(`src/livePanel.ts` / `src/liveModel.ts`)。コマンド **「ftester: ライブ操作を表示」**
  で開く Webview パネルで、macOS GUI 版(`ftester-gui`)の「ライブ操作」タブに相当する
  スクリーンショットのクリックタップ・要素一覧タップ・スワイプ・テキスト入力・アプリの
  起動/終了/インストールを、`ftester api list-devices`(デバイス一覧のみワンショット)と
  `ftester api live serve`(常駐プロセス。選択デバイスごとに1つ spawn し、NDJSON でコマンド送信・
  観測イベント受信を行う)経由で行う(詳細は下記「ライブ操作」)
...
macOS GUI 版(`ftester-gui`)の「ライブ操作」タブ(`Sources/ftester-gui/LiveView.swift` +
`AppModel.swift` の `refreshLive`/`liveAction`)に相当する機能です。コマンドパレットから
**「ftester: ライブ操作を表示」**(`ftester.showLiveControl`)を実行すると、エディタの横
(`ViewColumn.Beside`)に Webview パネルが開きます(デバイスモニターと同じくシングルトン。
既に開いている場合は既存のパネルを前面に出すだけです)。

- **上部のデバイスセレクタ**: パネルを開くと `ftester api list-devices --project <project>` を
  実行し、マシンプロファイルの全デバイスと現在状態(接続済み/起動中/未起動)を
  取得してプルダウンに表示します。**「デバイスを一覧を更新」**ボタンで再取得できます。
  `connected` 以外のデバイスも選択自体はできますが、選択中に「⚠ 接続されていません」という
  注意表示が出ます。`list-devices` が失敗した場合(マシンプロファイル未設定、対象プロジェクト未解決
  等)は、`ftester.platform`/`ftester.port`/`ftester.serial` 設定から作った「設定のデバイス」
  1件にフォールバックし、上部にエラーバナーで理由を表示します。デバイスを選択すると、そのデバイス
  向けの `ftester api live serve` 常駐プロセスが起動します(詳細は下記「常駐プロセス(`api live
  serve`)について」)。
- **左: スクリーンショット**: 「更新」ボタンで serve へ `{"cmd":"refresh"}` を送り、返ってきた
  JPEG 画像(base64)を表示します。**画像をクリックするとその位置をタップします**
  (クリック位置を画像の表示pxからデバイスのポイント座標へ比例変換して `{"cmd":"tap","x":..,"y":..}`
  を送る。詳細は下記「座標変換について」)。未取得時はプレースホルダーが表示され、「更新」を
  促します。
- **右: 操作パネル**(GUI 版 `LiveView` の `controlPane` と同構成):
  - **アプリ**: bundle ID / パッケージ名の入力欄 + 「起動」(`{"cmd":"launch","bundle":..}`)・
    「終了」(`{"cmd":"terminate"}`)。
  - **インストール**: iOS(`.app`)・Android(`.apk`)それぞれのパス入力欄 + 「選択...」
    (`vscode.window.showOpenDialog` で拡張側がファイル選択ダイアログを開く)。
    「インストール」ボタンは選択中デバイスの platform に応じた方のパスを使います
    (`{"cmd":"install","path":..}`。パスが空、またはファイルが存在しない場合は serve へ送らずに
    エラー表示します)。
  - **「更新」ボタン**: snapshot を再取得します。
  - **スワイプ**: ↑↓←→ ボタン(`{"cmd":"swipe","direction":..}`)。
  - **テキスト入力欄 + 「入力」**(`{"cmd":"type","text":..,"ref":..}`): 要素一覧でタップした要素が
    あれば `ref` を付けて送信します(未選択、または snapshot を再取得すると選択状態はクリアされ、
    その場合はフォーカス中の要素に入力されます)。
  - **要素一覧**: `[ref] type「label」id=identifier =value` 形式(GUI 版 `elementLine` と同じ。
    空フィールドは省く)。**クリックでその要素をタップします**(`{"cmd":"tap","ref":..}`)。
    行にマウスを乗せると、画像上に該当要素の `frame`(ポイント座標)を表示pxに変換した枠を
    オーバーレイ表示します。
  - **エラー表示欄**: 直近の失敗(`actionResult` の `ok:false`、または snapshot 取得失敗)の
    エラーメッセージを表示します。
- **操作後の自動更新**: タップ/スワイプ/入力/起動/終了/インストールが成功すると、serve が
  操作直後に返す観測イベントをそのまま画面へ反映します(Phase 4 高速化: 旧実装にあった固定700ms待ち
  +別プロセスでの snapshot 再取得は廃止。ブリッジの操作応答時点で UI が整定済みのため追加待ちは
  不要)。失敗時は画面を再取得せず、エラー表示欄にエラーを出すだけです。操作中は全ての操作ボタン・
  デバイスセレクタが無効化され、「処理中...」と表示されます。

### 常駐プロセス(`api live serve`)について

Phase 4 高速化(2026-07)より前は、タップ等の操作1回ごとに `ftester api live tap` 等をワンショット
spawn し、操作後に固定700ms待ってから `ftester api live snapshot` を別プロセスで spawn していました
(1操作あたりプロセス起動2回+700ms、実測1タップ0.8〜1.0秒)。ブリッジ側の改修(操作応答=UI整定済み)
により固定待ちが不要になったのに合わせ、選択デバイスごとに `ftester api live serve` を常駐 spawn する
方式に変更しました。ドライバは serve プロセス起動時に1回だけ生成され、以降の全操作で使い回されます。

- コマンドは stdin へ NDJSON で1行ずつ送ります: `{"cmd":"tap","ref":<Int>}` /
  `{"cmd":"tap","x":..,"y":..}` / `{"cmd":"type","text":..,"ref":..|null}` /
  `{"cmd":"swipe","direction":"up"|"down"|"left"|"right"}` / `{"cmd":"launch","bundle":..}` /
  `{"cmd":"terminate"}` / `{"cmd":"install","path":..}` / `{"cmd":"refresh"}`(操作なし、観測のみ)。
- 応答は stdout から NDJSON で届きます。`refresh` 以外はまず
  `{"kind":"actionResult","ok":true|false,"error":..|null}` を出し、続けて(操作の成否を問わず)
  観測イベント `{"kind":"snapshot","ok":true,"platform":..,"screen":..,"image":..,"elements":..}`
  (失敗時は `{"kind":"snapshot","ok":false,"error":..}`)を出します。`refresh` は観測イベント1行
  だけです。拡張側は `actionResult` が `ok:false` のとき、続く `snapshot` イベントは画面へ反映しません
  (直前の表示を保持したままエラーを表示する。旧ワンショット版と同じ挙動)。
- プロセス管理は `src/monitorPanel.ts` の host-metrics プロセス管理パターン(v0.0.65)をそのまま
  踏襲しています: stdin パイプ保持(EOF が終了指示)・SIGTERM 送信後2秒で SIGKILL・予期しない終了は
  5秒後に自動再起動・起動10秒未満の異常終了が3連続したら諦めます。host-metrics と違い serve は
  デバイスごとの状態を持つプロセスなので、デバイス選択が変わるたびに明示的に再バインド(停止→
  新デバイスで起動)し、その際は諦め状態もリセットします。専用の「再起動」ボタンは無く、
  デバイスを選び直す(または一覧を再取得する)操作そのものが復帰の手段になります。
- 単一実装原則により、旧ワンショットサブコマンド(`api live snapshot`/`tap`/`type`/`swipe`/
  `press`/`launch`/`terminate`/`install`)は削除済みです(`press`[長押し]はこのパネルの UI に
  無く、旧実装時点から未使用でした)。
...
クリック位置→タップ座標への変換自体は拡張ホスト側(`src/livePanel.ts`)で行い、webview は
...
`frameToDisplayRect` と同じ小さな計算式だけを webview 内の素の JS に複製しています
(`healReviewPanel.ts` が `healModel.ts` の一部ロジックを複製しているのと同じ方針)。

### CLI 呼び出しについて(FtesterCli のキューを使わない理由)

`ftester api list-devices`/`ftester api live serve` は、Test Explorer の「実行」等が使う
`src/cli.ts`(`FtesterCli`)の直列実行キューには乗せず、`src/livePanel.ts` が専用に spawn します。
`FtesterCli` のキューは `ftester api run`(シナリオ実行。内部で `swift build` を伴い得るため、
SPM のビルドロック対策として同時に2プロセス走らせない設計)と共有されているため、もしライブ操作も
同じキューに乗せると、実行中(数分かかることもある)はライブ操作パネルの全ての操作がその実行完了まで
...
(`src/livePanel.ts`)やデバイスモニターの devices up/down のような専用 spawn ではなく、
...
24. デバイスが起動しブリッジ接続まで完了すると、対象タイルが「接続済み」(緑バッジ)に変わり、
    ...
    体感で確認する)。ブリッジ未接続の間は「起動中」(黄バッジ)になることも
    確認する。
...
35. デバイス(シミュレータ/エミュレータ)を `ftester devices up` 等で起動した状態で、
    コマンドパレットから **「ftester: ライブ操作を表示」** を実行する。エディタの横に
    Webview パネルが開き、上部のデバイスセレクタに起動済みのデバイスが一覧表示されることを
    確認する(ブリッジ接続済みのデバイスは「接続済み」と表示される)。もう一度実行しても
    新しいパネルが増えず、既存のパネルが前面に出る(reveal される)ことを確認する。
36. マシンプロファイルが見つからない状態(`ftester.project` を存在しないプロジェクト名にする等)
    でパネルを開くと、上部にエラーバナーが表示され、デバイスセレクタに
    `ftester.platform`/`ftester.port`/`ftester.serial` 設定から作られた「設定のデバイス」が
    1件だけ表示されることを確認する。
37. 接続済みのデバイスを選択した状態で「更新」ボタンを押すと、スクリーンショットが表示され、
    右下に要素一覧(`[ref] type「label」...` 形式)が表示されることを確認する。
38. 表示されたスクリーンショットをクリックすると、その位置がタップされ、700ms 程度後に
    画面が自動的に更新されることを確認する(クリック位置と実際にタップされた位置が対応して
    いることを、ボタン等のUI要素を狙ってクリックして反応することで確認する)。
39. 要素一覧の行にマウスを乗せると、スクリーンショット上にその要素の枠がオーバーレイ表示される
    ことを確認する。行をクリックするとその要素がタップされ(枠がハイライトされる)、700ms 程度後に
    画面が自動的に更新されることを確認する。
40. スワイプボタン(↑↓←→)を押すと、対応する方向にスワイプされ、画面が自動的に更新される
    ことを確認する。テキスト入力欄にテキストを入力して「入力」を押すと、直前にタップした
    要素(テキストフィールド等)にテキストが入力されることを確認する。
41. bundle ID / パッケージ名を入力して「起動」「終了」を押すと、対象アプリが起動/終了し、
    画面が自動的に更新されることを確認する。
42. 「選択...」ボタンを押すとファイル選択ダイアログが開き、選択したパスが対応する入力欄
    (iOS は `.app`、Android は `.apk`)に反映されることを確認する。パスを空にしたまま、または
    存在しないパスで「インストール」を押すと、CLI を呼び出さずにエラーがエラー表示欄に
    表示されることを確認する。有効なパスを指定してインストールが成功すると、画面が自動的に
    更新されることを確認する。
43. 操作(タップ/スワイプ/入力/起動/終了/インストール)の実行中は、デバイスセレクタと全ての
    操作ボタンが無効化され、「処理中...」と表示されることを確認する。存在しない bundle ID
    を指定して「起動」を押す等、意図的に失敗する操作を行うと、エラー表示欄にエラーメッセージが
    表示され、画面の自動更新は行われない(直前のスクリーンショットのまま)ことを確認する。
44. シナリオを「実行」している最中(実行に数秒以上かかるもの)に、ライブ操作パネルで
    「更新」やスワイプ等の操作を行うと、実行の完了を待たずに応答が返る(FtesterCli のキューに
    乗らず専用 spawn している)ことを確認する。
...
│   ├── liveModel.ts             # `ftester api list-devices`/`ftester api live serve` の検証・NDJSONコマンド組み立て・座標変換・要素行フォーマット・CLI引数組み立て・webviewメッセージプロトコル(vscode 非依存)
│   ├── livePanel.ts             # ライブ操作の WebviewPanel(ftesterLiveControl)。list-devices の専用spawn+live serve の常駐spawn(いずれもFtesterCliのキューを使わない)・座標変換の適用・serveの観測イベントをそのまま反映
