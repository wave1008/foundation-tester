        ...
        scenario {
            ...
            scene(3, "スクロールして連絡帳を確認する") {
                action {
                    ...
                    // 一覧はかな行ブロック→Latin ブロックの順で、連絡帳(かな末尾)は Chrome(Latin 先頭)より
                    // 上に並ぶため、scene 2 の続き位置から下方向に探すと見つからない。上方向スワイプ
                    // (direction: .down)はリスト先頭で通知シェード/ホームジェスチャに化けて設定アプリを
                    // 離脱する実害があり使えない(2026-07-16 実測)。開き直して先頭から下方向に探す
                    relaunchApp()
                    tap("アプリ||Apps")
                    tap("アプリをすべて表示||See all")
