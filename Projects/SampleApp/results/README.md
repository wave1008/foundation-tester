...
- **コミット単位**: 実行結果はコード変更と混ぜず、`results/` だけの独立コミットにする
  (例: 「実行結果を記録(<マシン名> <日付>)」)。レビュー不要・revert しやすくするため。
- **頻度の目安**: 日次、または意味のある実行バッチ(リグレッション一巡・リリース前確認)ごと。
  push 前の rebase/merge でコンフリクトは発生しない(runID にマシン名+乱数を含む
  追加専用レイアウトのため。2 ブランチ同時刻実行→マージで検証済み 2026-07-17)。
- **消してよいもの**: 検証・実験目的で作った run はコミット前に run ディレクトリごと削除してよい。
  コミット済みの結果は原則残す(分析の母数になる)。
- **間引き**: 肥大化してきたら古い月ディレクトリごと削除する
  (例: `git rm -r 'Projects/SampleApp/results/runs/2026-07'`)。月単位以外の部分削除はしない。
- **見ない・共有されないもの**: `reportPath` が指すレポート(.md/PNG)は gitignore のままなので
  他マシンからは開けない。失敗調査はレコード内の `failedSteps` / `errorLogs` を一次情報にする。
