...
class スクロールで折り返し下の要素に到達できること {
    ...
    ///
    /// **既知の残存フレーク: 約2%落ちる**(2026-08-03 実測。累計 93 回中 2 回 =
    /// 無負荷 52 回で 0 / 別スイート並走下 40 回で 1 / フルスイート中に 1)。
    /// 落ちたときの値は `selected=-` = **タップが飲まれている**(誤った行が選ばれるのではない)。
    /// **新しい退行ではなくこの修正の取りこぼし** —— 修正前は決定的に落ちていた。
    /// **計装つき 42 回(うち 30 回は負荷下)では一度も再現せず**、原因は未特定。
    /// 追い方は docs/verification.md「Compose の探索直後タップ」原因1 の「残存(未解決)」
