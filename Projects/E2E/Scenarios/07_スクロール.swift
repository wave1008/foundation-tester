...
class スクロールで折り返し下の要素に到達できること {
    ...
    /// のが根因で、探索の snapshot をキャッシュ迂回にして解決した(1/5 → 10/10)。
    /// **`scrollFrame` を指定する形だけ置く** —— 未指定(全画面スワイプ)は
    /// ios-xcuitest で決定的に外れたままで、こちらは frame クランプという別要因
    /// (docs/verification.md「Compose の探索直後タップ」)。他 3 SUT の S0080 は未指定形で通る
    ...
    // **CMP で残る制限は iOS の frame クランプだけ**(領域を指定しない探索 = 全画面スワイプで
    // 移動量がビューポートを超えると、飛び越した行が「見えている」ように報告され別の行を掴む。
    // **領域指定で塞げる** —— 実測 0/3 → 3/3)。だから S0080 は領域指定の形で置いてある。
    // Android 側の間欠ずれ(慣性と見ていたもの)は **a11y ツリーの公開遅れ**が根因と分かり、
    // 2026-08-03 に解決済み(docs/verification.md「Compose の探索直後タップ」)
