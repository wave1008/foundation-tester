...
- **HTML コメントにバッククォートを書かない**。RN の写しは TS のテンプレートリテラルなので
  **文字列が途中で終端してビルドが落ちる**(2026-08-13 に実際に落とした)

**この格子は Android では木に出ない**(2026-08-13 に 4 SUT で実測)。**値のセルも見出しも1つも
公開されない** —— 直前の `WebView 画面外テキスト`(`<p>`)は4 SUT とも取れるので、
ページが読めていないのではなく**表だけが落ちている**。

| Android SUT | 表の値 | 見出し | 直前の `<p>`(対照) |
|---|---|---|---|
| E2E-Android(素の View/XML) | × | × | ○ |
| E2E-Flutter | × | × | ○ |
| E2E-RN | × | × | ○ |
| E2E-CMP | × | × | ○ |

**ホスト側の作りに依らない** = Compose の interop でも Flutter のセマンティクスでも RN でも同じなので、
**`android.webkit.WebView` の a11y の性質**。`role="table"/"row"/"cell"` を明示しても変わらない
(一時的な探りで確認)。**`<div>` の格子なら Android でも公開される**ので、
**ページを自分で書ける場合はテスト可能性のために表要素を避けられる**。
