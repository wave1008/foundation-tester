...
4. キーボードに覆われた要素は `exist`/`textIs` の可視性判定(requireVisible 既定 true)で
   誤った緑になり(失敗文言は `false positive (occlusion)`)、検証不能になる。
...
| `#txt_drag_count` | Text | `drag=<n>` 初期 `drag=0` | `#pad_map` で完了した**ドラッグ**(全指が離れるまでの一連のタッチ)の回数。パンと同じしきい値以上動いてから離したときだけ +1。**単タップ・ダブルタップでは増えない** |
...
- **`#txt_drag_count` は「指を離さず折れ線状に動かした連続ジェスチャ(`gesture` コマンド)」と
  「途中で指を離して撃ち直した複数回のジェスチャ」を見分ける witness**(`#txt_pan` の移動量の
  累積だけでは1回のドラッグか複数回かを区別できない)。前者は+1・後者は動かした回数ぶん+n。
  2本指のピンチでも+1されうるが、これは規定しない(検証対象は1本指の区別だけ)
...
OCR 段に届いたのは 2 件だけだった —— **`ocrTextOcclusionCheck` を切り替えても結果が変わらず、
...
- **iOS は中身が出るまで約 2.3 秒かかる**(WebContent プロセスの a11y 起動待ち。内蔵 HTML で
  この値なので、実ページ+通信ではさらに延びる)。`waitSeconds: 0` のアサーションは書かない。
